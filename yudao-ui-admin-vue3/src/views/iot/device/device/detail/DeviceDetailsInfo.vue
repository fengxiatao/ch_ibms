          ...
          <el-card class="h-full">
            ...
            <el-descriptions :column="2" border>
              ...
              <el-descriptions-item label="设备类型">
                <dict-tag v-if="product?.deviceType" :type="DICT_TYPE.IOT_PRODUCT_DEVICE_TYPE" :value="product.deviceType" />
                <span v-else>--</span>
              </el-descriptions-item>
              <el-descriptions-item label="定位类型">
                <dict-tag v-if="device?.locationType" :type="DICT_TYPE.IOT_LOCATION_TYPE" :value="device.locationType" />
                <span v-else>--</span>
              </el-descriptions-item>
              ...
              <el-descriptions-item label="创建时间">
                {{ device?.createTime ? formatDate(device.createTime) : '--' }}
              </el-descriptions-item>
              <el-descriptions-item label="当前状态">
                <dict-tag v-if="device?.state" :type="DICT_TYPE.IOT_DEVICE_STATE" :value="device.state" />
                <span v-else>--</span>
              </el-descriptions-item>
              <el-descriptions-item label="激活时间">
                {{ device?.activeTime ? formatDate(device.activeTime) : '--' }}
              </el-descriptions-item>
              <el-descriptions-item label="最后上线时间">
                {{ device?.onlineTime ? formatDate(device.onlineTime) : '--' }}
              </el-descriptions-item>
              <el-descriptions-item label="最后离线时间">
                {{ device?.offlineTime ? formatDate(device.offlineTime) : '--' }}
              </el-descriptions-item>
