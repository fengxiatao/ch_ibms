<template>
  ...
  <template v-if="data.type === IoTThingModelTypeEnum.PROPERTY && data.property">
    ...
    <div
      ...
      "
    >
      取值范围：{{ data.property.dataSpecs ? `${data.property.dataSpecs.min}~${data.property.dataSpecs.max}` : '-' }}
    </div>
    <!-- 非列表型：文本 -->
    <div v-if="data.property.dataType && IoTDataSpecsDataTypeEnum.TEXT === data.property.dataType">
  ...
  <!-- 服务 -->
  <div v-if="data.type === IoTThingModelTypeEnum.SERVICE && data.service">
    调用方式：{{ getThingModelServiceCallTypeLabel(data.service.callType) }}
  </div>
  <!-- 事件 -->
  <div v-if="data.type === IoTThingModelTypeEnum.EVENT && data.event">
