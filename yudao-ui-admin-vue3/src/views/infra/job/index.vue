  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" @selection-change="handleRowCheckboxChange">
      ...
      <el-table-column label="业务类型" align="center" prop="businessType" width="140">
        <template #default="scope">
          <el-tag v-if="scope.row.businessType" type="info" size="small">
            {{ getBusinessTypeLabel(scope.row.businessType) }}
          </el-tag>
          <span v-else style="color: #909399">-</span>
        </template>
      </el-table-column>
      <el-table-column label="任务分组" align="center" prop="jobGroup" width="100">
        <template #default="scope">
          <el-tag v-if="scope.row.jobGroup" size="small">{{ scope.row.jobGroup }}</el-tag>
          <span v-else style="color: #909399">default</span>
        </template>
      </el-table-column>
      <el-table-column label="优先级" align="center" prop="priority" width="80">
        <template #default="scope">
          ...
          <span v-else style="color: #909399">5</span>
        </template>
      </el-table-column>
...
const getBusinessTypeLabel = (businessType: string): string => {
  const typeMap = {
    IOT_DEVICE_OFFLINE_CHECK: '设备离线检查',
    IOT_DEVICE_HEALTH_CHECK: '设备健康检查',
    IOT_DEVICE_DATA_COLLECT: '设备数据采集',
    IOT_DEVICE_STATUS_SYNC: '设备状态同步',
    IOT_OTA_UPGRADE: 'OTA升级',
    IOT_PRODUCT_STATISTICS: '产品统计',
    IOT_PRODUCT_DATA_AGGREGATE: '产品数据汇总',
    LIGHTING_SCHEDULE: '照明定时',
    LIGHTING_ENERGY_STAT: '照明能耗统计',
    LIGHTING_SCENE_PRESET: '照明场景',
    SPATIAL_INSPECTION: '空间巡检',
    SPATIAL_ENV_COLLECT: '环境采集',
    ENERGY_STATISTICS: '能耗统计',
    ENERGY_OPTIMIZATION: '能源优化',
    SECURITY_VIDEO_CLEANUP: '录像清理',
    SECURITY_PATROL: '安防巡检',
    SYSTEM_DATA_CLEANUP: '数据清理',
    SYSTEM_DATA_BACKUP: '数据备份'
  }
...
const getPriorityType = (priority: number): string => {
  if (priority <= 2) return 'danger' // 关键任务
  if (priority <= 4) return 'warning' // 高优先级
  if (priority <= 6) return '' // 普通优先级
  if (priority <= 8) return 'info' // 低优先级
  return 'info' // 后台任务
}
