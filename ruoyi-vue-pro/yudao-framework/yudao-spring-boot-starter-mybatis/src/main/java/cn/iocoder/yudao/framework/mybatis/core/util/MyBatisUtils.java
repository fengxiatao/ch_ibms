...
public class MyBatisUtils {
     ...
     * 
     * 注意：使用 CopyOnWriteArrayList 保证线程安全，避免并发访问时出现 ConcurrentModificationException
     *
     * @param interceptor 链
     * @param inner       拦截器
     * @param index       位置
     */
    public static synchronized void addInterceptor(MybatisPlusInterceptor interceptor, InnerInterceptor inner, int index) {
        // 使用 CopyOnWriteArrayList 保证线程安全
        List<InnerInterceptor> inners = new CopyOnWriteArrayList<>(interceptor.getInterceptors());
