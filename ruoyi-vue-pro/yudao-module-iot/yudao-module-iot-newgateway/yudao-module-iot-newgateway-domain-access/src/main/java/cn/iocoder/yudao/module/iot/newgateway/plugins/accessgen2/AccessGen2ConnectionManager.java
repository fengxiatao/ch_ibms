 ...
 * 门禁二代连接管理器
 * 
 * <p>管理门禁二代设备的连接状态，实现设备ID与登录句柄的映射。</p>
 * 
 * <p>主要功能：</p>
 * <ul>
 *     <li>设备连接的注册和注销</li>
 ...
 * 
 * <p>连接类型说明：</p>
 * <p>门禁二代设备使用大华 NetSDK 进行通信，登录成功后返回一个 Long 类型的登录句柄（loginHandle）。
 * 后续所有操作（开门、授权下发、人脸下发等）都需要使用这个句柄。</p>
 * 
 * <p>与门禁一代的区别：</p>
 * <ul>
...
public class AccessGen2ConnectionManager implements ConnectionManager<Long> {

    /**
     * 日志前缀
     */
    private static final String LOG_PREFIX = "[AccessGen2ConnectionManager]";
     ...
     * 设备ID -> 连接信息
     */
    private final Map<Long, AccessGen2ConnectionInfo> connectionInfoMap = new ConcurrentHashMap<>();
    ...
    public void register(Long deviceId, String identifier, Long loginHandle) {
        ...
        // 创建连接信息
        AccessGen2ConnectionInfo info = new AccessGen2ConnectionInfo();
    ...
    public void unregister(Long deviceId) {
        ...
        // 移除标识符映射
        AccessGen2ConnectionInfo info = connectionInfoMap.remove(deviceId);
    ...
    public void updateHeartbeat(Long deviceId) {
        ...
        AccessGen2ConnectionInfo info = connectionInfoMap.get(deviceId);
        if (info != null) {
            info.setLastHeartbeatTime(System.currentTimeMillis());
     ...
     */
    public AccessGen2ConnectionInfo getConnectionInfo(Long deviceId) {
        if (deviceId == null) {
            return null;
        }
     ...
     */
    public void updateSupportsFace(Long deviceId, boolean supportsFace) {
        AccessGen2ConnectionInfo info = connectionInfoMap.get(deviceId);
        if (info != null) {
            info.setSupportsFace(supportsFace);
        }
    }
     ...
     * @return 是否支持人脸识别
     */
    public boolean isSupportsFace(Long deviceId) {
        AccessGen2ConnectionInfo info = connectionInfoMap.get(deviceId);
        return info != null && info.isSupportsFace();
    }
     ...
     */
    public void updateSupportsFingerprint(Long deviceId, boolean supportsFingerprint) {
        AccessGen2ConnectionInfo info = connectionInfoMap.get(deviceId);
        if (info != null) {
            info.setSupportsFingerprint(supportsFingerprint);
        }
    }
     ...
     * @return 是否支持指纹识别
     */
    public boolean isSupportsFingerprint(Long deviceId) {
        AccessGen2ConnectionInfo info = connectionInfoMap.get(deviceId);
        return info != null && info.isSupportsFingerprint();
    }
     ...
     */
    @Data
    public static class AccessGen2ConnectionInfo {
        /**
         * 设备ID
         */
