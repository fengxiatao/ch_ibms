...
public class NvrSdkWrapper {
    ...
    private static final int NET_PTZ_POINT_MOVE_CONTROL = 18;  // 转到预设点
    private static final int NET_PTZ_POINT_SET_CONTROL = 19;   // 设置预设点
    private static final int NET_PTZ_POINT_DEL_CONTROL = 20;   // 删除预设点
    private static final int NET_PTZ_TOUR_START = 26;          // 巡航开始
    private static final int NET_PTZ_TOUR_STOP = 27;           // 巡航停止
    private static final int NET_EXTPTZ_EXACTGOTO = 0x70;      // 精确定位（3D定位）
    ...
    public NvrOperationResult queryChannels(long loginHandle) {
        ...
        try {
            ...
            // 获取通道状态（关键：返回的 length 就是实际通道数）
            byte[] channelStatus = queryChannelStatus(loginHandle, maxChannels);
            
            // ✅ 使用 queryChannelStatus 返回的实际长度作为有效通道数
            int actualChannelCount = (channelStatus != null) ? channelStatus.length : 0;
            log.info("{} 实际通道数量: {}", LOG_PREFIX, actualChannelCount);
            
            // 获取通道名称（只查询实际存在的通道）
            String[] channelNames = queryChannelNames(loginHandle, actualChannelCount);
            
            // 获取录像状态
            byte[] recordStatus = queryRecordStatus(loginHandle, actualChannelCount);
            ...
            log.info("{} ✅ 查询通道完成: handle={}, count={} (已过滤未配置通道)", LOG_PREFIX, loginHandle, channelList.size());
     ...
     * 查询通道名称
     * 
     * <p>通过 ToolKits.GetDevConfig 配合 CFG_CMD_CHANNELTITLE 获取 NVR 配置的通道名称。</p>
     * 
     * @param loginHandle 登录句柄
     * @param channelCount 实际通道数量
     * @return 通道名称数组
     */
    private String[] queryChannelNames(long loginHandle, int channelCount) {
        if (channelCount <= 0) {
            return new String[0];
        }
        ...
        try {
            log.debug("{} 开始查询通道名称: handle={}, channelCount={}", LOG_PREFIX, loginHandle, channelCount);
            
            // 方式1：使用 ToolKits.GetDevConfig + CFG_CMD_CHANNELTITLE 获取通道名称
            boolean anySuccess = false;
            for (int channelNo = 0; channelNo < channelCount; channelNo++) {
                try {
                    NetSDKLib.AV_CFG_ChannelName cfg = new NetSDKLib.AV_CFG_ChannelName();
                    boolean ok = ToolKits.GetDevConfig(
                            new LLong(loginHandle), 
                            channelNo, 
                            NetSDKLib.CFG_CMD_CHANNELTITLE, 
                            cfg
                    );
                    ...
                    if (ok) {
                        ...
                        if (StringUtils.hasText(channelName)) {
                            names[channelNo] = channelName;
                            anySuccess = true;
                            log.debug("{} 通道 {} 名称(CFG_CMD_CHANNELTITLE): {}", LOG_PREFIX, channelNo + 1, channelName);
                        }
                    } else {
                        if (channelNo == 0) {
                            log.debug("{} CFG_CMD_CHANNELTITLE 方式获取通道名称失败: {}", LOG_PREFIX, ToolKits.getErrorCodePrint());
                        }
                    }
                } catch (Exception e) {
                    if (channelNo == 0) {
                        log.debug("{} CFG_CMD_CHANNELTITLE 方式查询通道 {} 名称异常: {}", LOG_PREFIX, channelNo + 1, e.getMessage());
                    }
                }
            }
            
            // 方式2：如果方式1失败，尝试使用 CFG_CMD_ENCODE 获取编码配置中的通道名称
            if (!anySuccess) {
                log.debug("{} 尝试使用 CFG_CMD_ENCODE 方式获取通道名称", LOG_PREFIX);
                for (int channelNo = 0; channelNo < channelCount; channelNo++) {
                    try {
                        NetSDKLib.CFG_ENCODE_INFO cfg = new NetSDKLib.CFG_ENCODE_INFO();
                        boolean ok = ToolKits.GetDevConfig(
                                new LLong(loginHandle), 
                                channelNo, 
                                NetSDKLib.CFG_CMD_ENCODE, 
                                cfg
                        );
                        ...
                        if (ok && cfg.szChnName != null) {
                            ...
                            if (StringUtils.hasText(channelName)) {
                                names[channelNo] = channelName;
                                anySuccess = true;
                                log.debug("{} 通道 {} 名称(CFG_CMD_ENCODE): {}", LOG_PREFIX, channelNo + 1, channelName);
                            }
                        }
                    } catch (Exception e) {
                        if (channelNo == 0) {
                            log.debug("{} CFG_CMD_ENCODE 方式查询通道名称异常: {}", LOG_PREFIX, e.getMessage());
                        }
                    }
                }
            }
            ...
            if (anySuccess) {
                log.info("{} ✅ 通道名称查询完成", LOG_PREFIX);
            } else {
                log.warn("{} ⚠ 通道名称查询失败，使用默认名称", LOG_PREFIX);
            }
     ...
     * 查询已配置 IPC 的通道集合
     * 
     * <p>通过 CLIENT_QueryDevState 配合 NET_DEVSTATE_DEV_CHN_COUNT (91) 查询设备实际通道数量，
     * 用于区分已配置和未配置的通道。</p>
     * 
     * <p>判断逻辑：</p>
     * <ul>
     *     <li>方式1：查询 NVR 配置的数字通道数量，只返回实际配置的通道号</li>
     *     <li>方式2：尝试对每个通道获取码流 URL，成功则表示已配置</li>
     *     <li>方式3：检查通道名称是否为非默认名称（如果有自定义名称，说明可能已配置）</li>
     * </ul>
     * 
     * @param loginHandle 登录句柄
     * @param maxChannels 最大通道数
     * @return 已配置 IPC 的通道号集合（通道号从 1 开始），如果查询失败返回空集合
     */
    private Set<Integer> queryConfiguredChannels(long loginHandle, int maxChannels) {
        Set<Integer> configuredChannels = new HashSet<>();
        
        try {
            log.debug("{} 开始查询已配置通道: handle={}", LOG_PREFIX, loginHandle);
            
            // 方式1：使用 NET_DEVSTATE_DEV_CHN_COUNT (91) 查询设备通道数量信息
            // 该查询返回 NET_DEV_CHN_COUNT_INFO 结构，包含已使用的通道数
            int bufSize = 1024;
            com.sun.jna.Memory infoBuf = new com.sun.jna.Memory(bufSize);
            com.sun.jna.ptr.IntByReference returnLen = new com.sun.jna.ptr.IntByReference(0);
            
            boolean result = getNetSdk().CLIENT_QueryDevState(
                    new LLong(loginHandle),
                    91, // NET_DEVSTATE_DEV_CHN_COUNT - 查询设备通道数量
                    infoBuf,
                    bufSize,
                    returnLen,
                    TIMEOUT_MS
            ...
            if (result && returnLen.getValue() > 0) {
                // NET_DEV_CHN_COUNT_INFO 结构解析
                // dwSize (4 bytes) + nMaxVideo (4 bytes) + nRetVideo (4 bytes) + ...
                int maxVideo = infoBuf.getInt(4);   // 最大视频通道数
                int retVideo = infoBuf.getInt(8);   // 实际视频通道数
                int maxAlarm = infoBuf.getInt(12);  // 最大报警输入
                int retAlarm = infoBuf.getInt(16);  // 实际报警输入
                
                log.info("{} 通道数量查询成功: maxVideo={}, retVideo={}, maxAlarm={}, retAlarm={}", 
                        LOG_PREFIX, maxVideo, retVideo, maxAlarm, retAlarm);
                
                // 如果实际视频通道数 > 0，则返回 1 到 retVideo 的通道号
                if (retVideo > 0) {
                    for (int i = 1; i <= retVideo && i <= maxChannels; i++) {
                        configuredChannels.add(i);
                    }
                    log.info("{} ✅ 根据设备返回的实际通道数确定已配置通道: count={}", LOG_PREFIX, retVideo);
                    return configuredChannels;
                }
            } else {
                log.debug("{} NET_DEVSTATE_DEV_CHN_COUNT 查询失败，尝试其他方式: error={}", 
                        LOG_PREFIX, ToolKits.getErrorCodePrint());
            }
            
            // 方式2：通过 RTSP URL 测试来判断通道是否配置
            // 使用 CLIENT_MakeKeyFrame 或类似的轻量级操作来测试通道是否可用
            log.debug("{} 尝试通过 I 帧请求测试通道配置状态...", LOG_PREFIX);
            for (int i = 0; i < maxChannels; i++) {
                int channelNo = i; // SDK 通道号从 0 开始
                try {
                    // 尝试请求 I 帧，如果通道未配置会返回失败
                    boolean keyFrameResult = getNetSdk().CLIENT_MakeKeyFrame(
                            new LLong(loginHandle), 
                            channelNo, 
                            0  // 主码流
                    );
                    ...
                    if (keyFrameResult) {
                        configuredChannels.add(i + 1); // 返回的通道号从 1 开始
                        log.debug("{} 通道 {} I帧请求成功，已配置", LOG_PREFIX, i + 1);
                    } else {
                        // 检查错误码，某些错误码表示通道存在但暂时不可用
                        int errorCode = getNetSdk().CLIENT_GetLastError();
                        // 0x80000000 | 10 = -2147483638 表示用户无此权限
                        // 0x80000000 | 11 = -2147483637 表示登录设备超时
                        // 0x80000000 | 45 = -2147483603 表示通道无法使用（未配置）
                        if (errorCode != -2147483603 && errorCode != 0) {
                            // 非"通道无法使用"的错误，可能是其他原因（权限等），仍认为通道已配置
                            configuredChannels.add(i + 1);
                            log.debug("{} 通道 {} I帧请求失败但错误码非45(通道不可用)，认为已配置: errorCode=0x{}", 
                                    LOG_PREFIX, i + 1, Integer.toHexString(errorCode));
                        } else {
                            log.debug("{} 通道 {} 未配置 (errorCode=0x{})", LOG_PREFIX, i + 1, Integer.toHexString(errorCode));
                        }
                    }
                } catch (Exception e) {
                    log.debug("{} 通道 {} I帧测试异常: {}", LOG_PREFIX, i + 1, e.getMessage());
                }
            }
            
            if (!configuredChannels.isEmpty()) {
                log.info("{} ✅ 通过 I 帧测试确定已配置通道: {}", LOG_PREFIX, configuredChannels);
                return configuredChannels;
            }
            
            // 方式3：如果以上方式都失败，返回空集合，让调用方回退到保留所有状态正常的通道
            log.warn("{} 无法确定已配置通道，将返回所有状态正常的通道", LOG_PREFIX);
            
        } catch (Exception e) {
            log.warn("{} 查询已配置通道异常: {}", LOG_PREFIX, e.getMessage());
        }
     ...
     * 通过 NVR 进行预设点控制
     * 
     * @param loginHandle NVR 登录句柄
     * @param channelNo   通道号
     * @param presetNo    预设点号
     * @param presetAction 动作: SET-设置预设点, GOTO-转到预设点, CLEAR-清除预设点
     * @return 操作结果
     */
    public NvrOperationResult presetControl(Long loginHandle, Integer channelNo, Integer presetNo, String presetAction) {
        if (loginHandle == null || loginHandle <= 0) {
            return NvrOperationResult.failure("无效的登录句柄");
        }
        
        try {
            log.info("{} 预设点控制: handle={}, channel={}, presetNo={}, action={}", 
                    LOG_PREFIX, loginHandle, channelNo, presetNo, presetAction);
            
            int sdkChannelNo = channelNo != null && channelNo > 0 ? channelNo - 1 : 0;
            int ptzCommand;
            
            switch (presetAction != null ? presetAction.toUpperCase() : "") {
                case "GOTO":
                    ptzCommand = NET_PTZ_POINT_MOVE_CONTROL;
                    break;
                case "SET":
                    ptzCommand = NET_PTZ_POINT_SET_CONTROL;
                    break;
                case "CLEAR":
                    ptzCommand = NET_PTZ_POINT_DEL_CONTROL;
                    break;
                default:
                    return NvrOperationResult.failure("不支持的预设点操作: " + presetAction);
            }
            ...
            if (result) {
                log.info("{} 预设点控制成功: channel={}, presetNo={}, action={}", 
                        LOG_PREFIX, channelNo, presetNo, presetAction);
                return NvrOperationResult.success("预设点控制成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 预设点控制失败: channel={}, presetNo={}, action={}, error={}", 
                        LOG_PREFIX, channelNo, presetNo, presetAction, errorMsg);
                return NvrOperationResult.failure("预设点控制失败: " + errorMsg);
            }
        } catch (Exception e) {
            log.error("{} 预设点控制异常: channel={}, error={}", LOG_PREFIX, channelNo, e.getMessage(), e);
    ...
    public NvrOperationResult presetControlDirect(String ip, String username, String password, 
                                                   Integer channelNo, Integer presetNo, 
                                                   String presetAction, String presetName) {
        if (!sdkInitialized) {
            return NvrOperationResult.failure("SDK 未初始化");
        }
        
        try {
            log.info("{} 直连IPC预设点控制: ip={}, channel={}, presetNo={}, action={}, name={}", 
                    LOG_PREFIX, ip, channelNo, presetNo, presetAction, presetName);
            ...
            if (result) {
                log.info("{} 直连IPC预设点控制成功: ip={}, presetNo={}, action={}", 
                        LOG_PREFIX, ip, presetNo, presetAction);
                ipcLoginTimeCache.put(ip, System.currentTimeMillis());
                return NvrOperationResult.success("预设点控制成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 直连IPC预设点控制失败: ip={}, error={}", LOG_PREFIX, ip, errorMsg);
                clearIpcSession(ip);
                return NvrOperationResult.failure("预设点控制失败: " + errorMsg);
            }
        } catch (Exception e) {
    ...
    public NvrOperationResult ptzAreaZoomDirect(String ip, String username, String password, 
                                                 Integer channelNo, Integer startX, Integer startY,
        ...
        try {
            log.info("{} 直连IPC区域放大: ip={}, channel={}, area=({},{}) -> ({},{})", 
                    LOG_PREFIX, ip, channelNo, startX, startY, endX, endY);
            ...
            // 注意：区域放大需要使用 3D 定位功能，将起止坐标转换为中心点和缩放级别
            // 计算中心点
            int centerX = (startX + endX) / 2;
            int centerY = (startY + endY) / 2;
            // 计算缩放级别（基于区域大小，区域越小，缩放越大）
            int areaWidth = Math.abs(endX - startX);
            int areaHeight = Math.abs(endY - startY);
            int zoom = Math.max(1, 8 - Math.min(areaWidth, areaHeight) / 100);
            ...
            if (result) {
                log.info("{} 直连IPC区域放大成功: ip={}", LOG_PREFIX, ip);
                ipcLoginTimeCache.put(ip, System.currentTimeMillis());
                return NvrOperationResult.success("区域放大成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 直连IPC区域放大失败: ip={}, error={}", LOG_PREFIX, ip, errorMsg);
                return NvrOperationResult.failure("区域放大失败: " + errorMsg);
            }
        } catch (Exception e) {
            log.error("{} 直连IPC区域放大异常: ip={}, error={}", LOG_PREFIX, ip, e.getMessage(), e);
            return NvrOperationResult.failure("区域放大异常: " + e.getMessage());
        }
    }
    ...
    public NvrOperationResult ptz3DPositionDirect(String ip, String username, String password, 
                                                   Integer channelNo, Integer posX, Integer posY, int zoom) {
        if (!sdkInitialized) {
            return NvrOperationResult.failure("SDK 未初始化");
        }
        
        try {
            log.info("{} 直连IPC 3D定位: ip={}, channel={}, x={}, y={}, zoom={}", 
                    LOG_PREFIX, ip, channelNo, posX, posY, zoom);
            ...
            boolean result = getNetSdk().CLIENT_DHPTZControlEx(
                    ...
                    NET_EXTPTZ_EXACTGOTO,  // 精确定位
                    0,
                    posX != null ? posX * 10 : 0,  // 水平角度 * 10
                    posY != null ? posY * 10 : 0,  // 垂直角度 * 10
                    zoom
            );
            
            if (result) {
                log.info("{} 直连IPC 3D定位成功: ip={}", LOG_PREFIX, ip);
                ipcLoginTimeCache.put(ip, System.currentTimeMillis());
                return NvrOperationResult.success("3D定位成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 直连IPC 3D定位失败: ip={}, error={}", LOG_PREFIX, ip, errorMsg);
                return NvrOperationResult.failure("3D定位失败: " + errorMsg);
            }
        } catch (Exception e) {
            log.error("{} 直连IPC 3D定位异常: ip={}, error={}", LOG_PREFIX, ip, e.getMessage(), e);
            return NvrOperationResult.failure("3D定位异常: " + e.getMessage());
        }
    }
    ...
    public NvrOperationResult syncTourToDeviceDirect(String ip, String username, String password,
                                                      Integer channelNo, Integer tourNo, String tourName,
                                                      java.util.List<Integer> presetNos, 
                                                      java.util.List<Integer> dwellTimes) {
        if (!sdkInitialized) {
            return NvrOperationResult.failure("SDK 未初始化");
        }
        
        try {
            log.info("{} 直连IPC同步巡航: ip={}, channel={}, tourNo={}, name={}, presets={}", 
                    LOG_PREFIX, ip, channelNo, tourNo, tourName, presetNos);
            
            // 巡航同步功能暂不支持，需要使用大华 SDK 的配置 API
            log.warn("{} 巡航同步功能暂不支持，请使用设备 Web 界面配置", LOG_PREFIX);
            return NvrOperationResult.failure("巡航同步功能暂不支持，请使用设备 Web 界面配置");
            
        } catch (Exception e) {
            log.error("{} 直连IPC同步巡航异常: ip={}, error={}", LOG_PREFIX, ip, e.getMessage(), e);
            return NvrOperationResult.failure("同步巡航异常: " + e.getMessage());
        }
    }
    ...
    public NvrOperationResult startTourDirect(String ip, String username, String password,
                                               Integer channelNo, Integer tourNo) {
        if (!sdkInitialized) {
            return NvrOperationResult.failure("SDK 未初始化");
        }
        
        try {
            log.info("{} 直连IPC启动巡航: ip={}, channel={}, tourNo={}", LOG_PREFIX, ip, channelNo, tourNo);
            ...
            boolean result = getNetSdk().CLIENT_DHPTZControlEx(
                    loginHandle,
                    sdkChannelNo,
                    NET_PTZ_TOUR_START,  // 巡航开始
                    0,
                    tourNo != null ? tourNo : 1,
                    0,
                    0
            );
            
            if (result) {
                log.info("{} 直连IPC启动巡航成功: ip={}, tourNo={}", LOG_PREFIX, ip, tourNo);
                ipcLoginTimeCache.put(ip, System.currentTimeMillis());
                return NvrOperationResult.success("启动巡航成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 直连IPC启动巡航失败: ip={}, error={}", LOG_PREFIX, ip, errorMsg);
                return NvrOperationResult.failure("启动巡航失败: " + errorMsg);
            }
        } catch (Exception e) {
            log.error("{} 直连IPC启动巡航异常: ip={}, error={}", LOG_PREFIX, ip, e.getMessage(), e);
    ...
    public NvrOperationResult stopTourDirect(String ip, String username, String password,
        ...
        try {
            log.info("{} 直连IPC停止巡航: ip={}, channel={}, tourNo={}", LOG_PREFIX, ip, channelNo, tourNo);
            ...
            if (result) {
                log.info("{} 直连IPC停止巡航成功: ip={}, tourNo={}", LOG_PREFIX, ip, tourNo);
                ipcLoginTimeCache.put(ip, System.currentTimeMillis());
                return NvrOperationResult.success("停止巡航成功");
            } else {
                String errorMsg = ToolKits.getErrorCodePrint();
                log.warn("{} 直连IPC停止巡航失败: ip={}, error={}", LOG_PREFIX, ip, errorMsg);
                return NvrOperationResult.failure("停止巡航失败: " + errorMsg);
            }
        } catch (Exception e) {
            log.error("{} 直连IPC停止巡航异常: ip={}, error={}", LOG_PREFIX, ip, e.getMessage(), e);
