...
public class GatewayStartupProperties {
    ...
    /**
     * 冷启动时若本地 Profile 缓存为空，是否经 MQ 请求 Biz 对本租户批量重推（需配置 {@link #tenantId}）。
     * <p>
     * 未配置 tenantId（多租户兼容模式）时不会发送预热请求。
     */
    private boolean profileWarmupOnColdStart = true;

    /**
     * 发出预热请求后，最多等待多少秒以便 {@code DEVICE_PROFILE_CHANGED} 灌入缓存（超时仍继续启动）。
     */
    private int profileWarmupWaitSeconds = 25;

    /**
     * 等待预热期间轮询缓存间隔（毫秒）
     */
    private int profileWarmupPollIntervalMs = 400;
    ...
    public String toSummary() {
        return String.format(
            ...
            "whitelist=%s, blacklist=%s, syncStateToDb=%s, profileWarmupOnColdStart=%s, profileWarmupWaitSeconds=%d}",
            ...
            syncStateToDb,
            profileWarmupOnColdStart,
            profileWarmupWaitSeconds
        );
    }
}
