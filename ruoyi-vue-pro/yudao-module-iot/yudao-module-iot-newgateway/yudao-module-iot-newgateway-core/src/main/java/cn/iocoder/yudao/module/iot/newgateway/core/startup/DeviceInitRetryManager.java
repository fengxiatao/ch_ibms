...
public class DeviceInitRetryManager {
    ...
    /**
     * 获取重试队列大小（待重试 + 已耗尽仍保留在 map 中的条目）
     *
     * @return 当前 {@link #retryQueue} 条目数；Micrometer Gauge {@code gateway.startup.retry.queue.size} 引用此方法
     */
    ...
    /**
     * 待重试条目数（未标记 {@link RetryContext#isMaxRetriesReached()}）
     */
    public int getPendingRetryQueueCount() {
        int n = 0;
        for (RetryContext ctx : retryQueue.values()) {
            if (!ctx.isMaxRetriesReached()) {
                n++;
            }
        }
        return n;
    ...
    /**
     * 已耗尽重试仍保留在 map 中的条目数（{@link RetryContext#isMaxRetriesReached()} 为 true）
     */
    public int getExhaustedRetryQueueCount() {
        int n = 0;
        for (RetryContext ctx : retryQueue.values()) {
            if (ctx.isMaxRetriesReached()) {
                n++;
