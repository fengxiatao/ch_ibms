...
import cn.iocoder.yudao.module.iot.newgateway.device.GatewayProfileWarmupPublisher;
...
public class GatewayStartupInitializer {
    ...
    @Resource
    private GatewayDeviceProfileCache deviceProfileCache;

    @Resource
    private GatewayProfileWarmupPublisher profileWarmupPublisher;
    ...
    /**
     * MQ-only 基线：缓存为空时无法列出设备。单租户场景下向 Biz 发批量预热，并短暂等待 Profile 消息灌入缓存。
     */
    private void requestProfileWarmupIfNeeded() {
        if (!properties.isProfileWarmupOnColdStart()) {
            return;
        }
        Long tenantId = properties.getTenantId();
        if (tenantId == null) {
            log.info("[GatewayStartup] 未配置 tenantId，跳过 MQ 批量预热（多租户兼容模式请依赖管理端重推或各实例配置）");
            return;
        }
        if (deviceProfileCache.size() > 0) {
            log.info("[GatewayStartup] 设备 Profile 缓存已有 {} 条，跳过冷启动预热", deviceProfileCache.size());
            return;
        }
        log.info("[GatewayStartup] Profile 缓存为空，请求 Biz 批量预热 tenantId={}", tenantId);
        try {
            profileWarmupPublisher.publish(tenantId, "gateway-startup");
        } catch (MessagingException ex) {
            // MQ 未就绪/Topic 未创建时，不应阻断网关启动；继续走后续流程（可能拉不到设备列表，但不至于直接启动失败）
            log.warn("[GatewayStartup] 请求 Biz 批量预热失败（将跳过预热等待并继续启动），tenantId={}, err={}",
                    tenantId, ex.getMessage());
            return;
        } catch (Exception ex) {
            log.warn("[GatewayStartup] 请求 Biz 批量预热异常（将跳过预热等待并继续启动），tenantId={}, err={}",
                    tenantId, ex.getMessage(), ex);
            return;
        }
        long deadline = System.currentTimeMillis() + properties.getProfileWarmupWaitSeconds() * 1000L;
        int interval = Math.max(50, properties.getProfileWarmupPollIntervalMs());
        while (System.currentTimeMillis() < deadline) {
            if (deviceProfileCache.size() > 0) {
                log.info("[GatewayStartup] 预热后缓存条数={}", deviceProfileCache.size());
                return;
            }
            try {
                ...
                Thread.currentThread().interrupt();
                log.warn("[GatewayStartup] 等待预热被中断");
                return;
            }
        }
        log.warn("[GatewayStartup] 等待预热超时（{}s），缓存仍为空；将拉不到设备列表，可检查 Topic/Biz 或调大 iot.gateway.startup.profile-warmup-wait-seconds",
                properties.getProfileWarmupWaitSeconds());
    }
    ...
    private List<IotDeviceRespDTO> fetchDeviceList() {
        try {
            ...
            if (tenantId != null) {
                // 单租户模式：只获取指定租户的设备
                log.info("[GatewayStartup] 单租户模式: 从本地设备缓存获取租户 {} 的设备列表（由 Biz MQ 预热）...", tenantId);
                ...
                log.info("[GatewayStartup] 兼容模式: 从本地设备缓存获取全部设备（未指定 tenantId，依赖 Biz MQ 预热）...");
