 ...
 *   <li>被动连接在 {@code onDeviceConnected} 成功时写入 {@code DeviceSessionRegistry}，断开时 {@code remove}（与 Alarm/Changhui 一致）</li>
 * </ul>
 * 
 * <h2>配置示例</h2>
 * <pre>
...
public class TemplatePlugin implements PassiveDeviceHandler {
    ...
    public void onConnect(ChannelHandlerContext ctx, String deviceIdentifier) {
        ...
        try {
            ...
            if (online) {
                registerDeviceSession(deviceId);
            }
    ...
    private void registerDeviceSession(Long deviceId) {
        deviceSessionRegistry.put(SimpleDeviceSession.builder()
                .deviceId(deviceId)
                .pluginId(PluginConstants.PLUGIN_ID_TEMPLATE)
                .vendorKey(VENDOR)
