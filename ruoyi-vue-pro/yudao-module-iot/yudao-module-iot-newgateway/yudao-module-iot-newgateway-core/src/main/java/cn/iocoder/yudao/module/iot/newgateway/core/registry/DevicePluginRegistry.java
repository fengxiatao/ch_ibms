...
public class DevicePluginRegistry {
    ...
    /**
     * 设备类型 -> 插件 ID（用于指标 tag 等，与 {@link #handlers} 同步写入）
     */
    private final Map<String, String> pluginIdByDeviceType = new ConcurrentHashMap<>();
    ...
    private void registerPlugin(String beanName, Object bean) {
        ...
        if (bean instanceof DeviceHandler) {
            ...
            pluginIdByDeviceType.put(deviceType, pluginId);
    ...
    /**
     * 按设备类型解析插件 ID（用于 Micrometer 等）
     */
    public String resolvePluginIdForDeviceType(String deviceType) {
        if (deviceType == null || deviceType.isBlank()) {
            return "unknown";
        }
        return pluginIdByDeviceType.getOrDefault(deviceType, "unknown");
    }

    /**
     * 插件注解上的默认厂商（与单台设备 params 中的 brand 等不同）
     */
    public String resolveAnnotationVendorForDeviceType(String deviceType) {
        if (deviceType == null || deviceType.isBlank()) {
            return "unknown";
        }
        String pluginId = pluginIdByDeviceType.get(deviceType);
        if (pluginId == null) {
            return "unknown";
        }
        PluginDescriptor d = plugins.get(pluginId);
        if (d == null || d.getVendor() == null || d.getVendor().isBlank()) {
            return "unknown";
        }
        return d.getVendor().trim();
    }
}
