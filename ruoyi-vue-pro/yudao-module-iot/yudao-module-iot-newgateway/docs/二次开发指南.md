...
> **与 Biz 分步落地、勾选进度**：以 [`yudao-module-iot/docs/iot-newgateway-biz-mq-landing-plan.md`](../../docs/iot-newgateway-biz-mq-landing-plan.md) 为唯一主文档；本文侧重插件与框架实现细节。
...
代码按 Maven 子模块分布（详见 [`iot-gateway-multimodule-and-thin-shell.md`](../../docs/iot-gateway-multimodule-and-thin-shell.md)）：**启动类与 `application*.yaml`** 在 `yudao-module-iot-newgateway-bootstrap`；**门禁 ACCESS_GEN1/2** 在 `yudao-module-iot-newgateway-domain-access`；**NVR/IPC** 在 `yudao-module-iot-newgateway-domain-video`；其余在 **`yudao-module-iot-newgateway-core`**（瘦核心，无 NetSDK 依赖）。下图为逻辑目录（包名仍为 `cn.iocoder.yudao.module.iot.newgateway`）。

```
# 逻辑包根：…/newgateway/（core 模块为主，bootstrap 仅启动类，domain-video 含 nvr/ipc）
├── IotNewGatewayServerApplication.java    # 启动类（bootstrap 模块）
├── consumer/                               # 消息消费者
...
│   └── template/                          # 模板插件（参考实现）
├── remote/                                # 远程调用
│   └── device/GatewayMqIotDeviceCommonApi.java  # MQ-only 设备快照（缓存 + Request-Reply）
└── util/                                  # 工具类
