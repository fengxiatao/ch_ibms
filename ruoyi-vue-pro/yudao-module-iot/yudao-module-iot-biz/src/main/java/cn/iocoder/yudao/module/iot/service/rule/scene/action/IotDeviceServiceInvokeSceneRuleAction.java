...
public class IotDeviceServiceInvokeSceneRuleAction implements IotSceneRuleAction {
    ...
    @Override
    public void execute(cn.iocoder.yudao.module.iot.core.mq.message.IotDeviceMessage message,
     ...
     * 为产品下的所有设备执行服务调用
     */
    private void executeForAllDevices(cn.iocoder.yudao.module.iot.core.mq.message.IotDeviceMessage message,
                                      IotSceneRuleDO rule, IotSceneRuleDO.Action actionConfig) {
        // 1. 参数校验
     ...
     * 为指定设备执行服务调用
     */
    private void executeServiceInvokeForDevice(IotSceneRuleDO rule, IotSceneRuleDO.Action actionConfig, OtaDeviceView device) {
        // 1. 构建服务调用消息
        ...
        // 2. 发送设备消息
        try {
            cn.iocoder.yudao.module.iot.core.mq.message.IotDeviceMessage result = deviceMessageService.sendDeviceMessage(downstreamMessage, device);
     ...
     * @return 设备消息
     */
    private IotDeviceMessage buildServiceInvokeMessage(IotSceneRuleDO.Action actionConfig, OtaDeviceView device) {
        try {
