...
public class IotProductController {
    ...
    @PostMapping("/init-property-table/{id}")
    @Operation(summary = "初始化产品属性表", description = "手动创建产品的TDengine超级表")
    @Parameter(name = "id", description = "产品编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('iot:product:update')")
    public CommonResult<Boolean> initProductPropertyTable(@PathVariable("id") Long id) {
        try {
            devicePropertyService.defineDevicePropertyData(id);
            return success(true);
        } catch (Exception e) {
