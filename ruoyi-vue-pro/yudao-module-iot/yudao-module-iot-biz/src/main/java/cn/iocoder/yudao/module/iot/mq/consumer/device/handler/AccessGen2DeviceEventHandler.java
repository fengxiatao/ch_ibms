 ...
 * 二代门禁设备事件处理器
 * 
 * <p>处理二代门禁设备（人脸一体机）的事件上报，包括：</p>
 * <ul>
 *   <li>人脸识别事件 - 记录人脸识别日志</li>
 *   <li>刷卡事件 - 记录刷卡日志</li>
 *   <li>指纹识别事件 - 记录指纹识别日志</li>
 *   <li>开门事件 - 记录开门日志</li>
 *   <li>报警事件 - 记录门禁报警</li>
 * </ul>
 * 
...
public class AccessGen2DeviceEventHandler implements DeviceEventHandler {

    private static final String LOG_PREFIX = "[AccessGen2DeviceEventHandler]";

    /**
     * 设备类型标识 - 二代门禁
     */
    private static final String DEVICE_TYPE = AccessDeviceTypeConstants.ACCESS_GEN2;
    ...
    public void handleEvent(IotDeviceMessage message) {
        ...
        try {
            ...
            if (event != null) {
                ...
                log.info("{} 二代门禁事件已交由 AccessEventHandler 处理: deviceId={}, eventType={}", 
                        LOG_PREFIX, deviceId, event.getEventType());
            } else {
                log.warn("{} 二代门禁事件反序列化为空，忽略: deviceId={}", LOG_PREFIX, deviceId);
            }
        } catch (Exception e) {
            log.error("{} 处理二代门禁事件失败: deviceId={}", LOG_PREFIX, deviceId, e);
        }
    }
