 ...
 * <p>执行频率：建议每 5 分钟执行一次
 * <p>优先级：NORMAL（普通优先级）
 * <p>冲突策略：QUEUE（排队执行，确保每次升级任务都能被处理）
 * <p>允许并发：false（不允许并发，避免重复推送）
 *
 * @author 长辉信息科技有限公司
 */
@Component
@Slf4j
public class IotOtaUpgradeJob extends IotBusinessJobHandler {
    ...
    public JobBusinessType getBusinessType() {
        return JobBusinessType.IOT_OTA_UPGRADE;
    }
    ...
    public boolean isConcurrent() {
        return false; // 不允许并发执行，避免重复推送
    }

    @Override
    public String getConflictStrategy() {
        return JobConflictStrategy.QUEUE.name(); // 排队执行，确保每次升级任务都能被处理
    }
    ...
    protected String doExecute(String param) throws Exception {
        ...
        // TODO 长辉开发团队：可以优化成批量获取 原因是：1. N+1 问题；2. offline 的设备无需查询
        // 2. 遍历推送记录
        int successCount = 0;
        int failureCount = 0;
