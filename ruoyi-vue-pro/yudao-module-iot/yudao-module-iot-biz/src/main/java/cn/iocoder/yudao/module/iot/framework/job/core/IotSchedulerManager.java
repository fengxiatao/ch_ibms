...
public class IotSchedulerManager {
    ...
    public void stop() {
        ...
        try {
            // destroy() 会调用 scheduler.shutdown(waitForJobsToCompleteOnShutdown)
            // 确保完全关闭，包括 ClusterManager 线程，避免数据源已关闭仍尝试获取连接
            schedulerFactoryBean.destroy();
        } catch (Exception e) {
            log.warn("[stop][Scheduler 关闭异常]", e);
            try {
                if (scheduler != null && !scheduler.isShutdown()) {
                    scheduler.shutdown(true);
                }
            } catch (SchedulerException ex) {
                log.warn("[stop][调用 scheduler.shutdown 失败]", ex);
            }
        } finally {
            this.scheduler = null;
        }
