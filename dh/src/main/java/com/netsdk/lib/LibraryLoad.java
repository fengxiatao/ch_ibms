...
public class LibraryLoad {
  ...
  /**
   * 静态初始化：在 Windows 平台优先将动态库提取目录设置为项目目录下的 libs/winXX，
   * 以避免默认解压到系统临时目录导致加载到不兼容版本的原生库。
   */
  static {
    try {
      ...
      if (os != null && os.toLowerCase().startsWith("windows")) {
        boolean is64 = arch != null && (arch.toLowerCase().contains("64") || arch.toLowerCase().contains("amd64") || arch.toLowerCase().contains("x86_64"));
        File libsDir = new File(is64 ? "./libs/win64" : "./libs/win32");
        EXTRACT_PATH = libsDir.getAbsolutePath();
        System.out.println("LibraryLoad: 默认动态库提取目录已设置为 " + EXTRACT_PATH);
      }
    } catch (Throwable t) {
      System.err.println("LibraryLoad: 设置默认提取目录失败，回退到临时目录。原因：" + t.getMessage());
    }
  }
