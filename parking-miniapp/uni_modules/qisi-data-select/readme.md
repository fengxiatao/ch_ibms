...
::: tip 组件名：qisi-data-select

> 代码块： `uDataSelect`

[点击下载&安装](https://ext.dcloud.net.cn/plugin?name=qisi-data-select)
:::
...
在 uniCloud 开发中，`DB Schema`中配置了 enum 枚举等类型后，在 web 控制台的[自动生成表单](https://uniapp.dcloud.io/uniCloud/schema?id=autocode)功能中，会自动生成`qisi-data-select`组件并绑定好 data
...
|   `new`valueField	|    String		|   -	| value		|            localdata对应的value字段名称								|
|   `new`textField	|    String		|   -	| text		|                localdata对应的text字段名称							|
|  `new`width		|    Number		|   -	| 310		|                 组件宽度												|
