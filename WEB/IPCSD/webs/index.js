...
const ipcFunc = () => {
    ...
    const init = () => { 
        ...
        _doc.querySelectorAll('.testButton').forEach(item => {
            item.addEventListener('click', function(event) {
                event.stopPropagation();
                let userConfirmed  = confirm(tl('com.IPOnlineTip'));
                if(userConfirmed) {
                    try {
                        let url  = 'http://' + event.target.parentNode.getAttribute('data-ip')+':'+event.target.parentNode.getAttribute('data-port');
    ...
    const afterLogin = () => {
        ...
        RPC.PTZManager('getCurrentProtocolCaps',channel).then(res => {
            ChannelPTZCaps[channel] = res?.caps;
    ...
    const onPlayBack = (url, index) => {
        ...
        player.on('UpdateCanvas', function (e) {
            ...
            let _index = playbackIndex%16;
            if(firstTime === 0){
                firstTime = e.timestamp;//获取录像文件的第一帧的时间戳
            }
            $('#h5_curTime_'+ _index).innerText = e.timestamp - firstTime;
        });
        player.on('GetTotalTime', function (e) {
            let playbackIndex = player.playbackIndex;
            let _index = playbackIndex%16;
            $('#h5_totalTime_'+ _index).innerText = e;
        });
        player.on('MSEResolutionChanged', function (e) {
            console.log(e)
        });
    ...
    const startFastPostion = () => {
        onStopEnlarge();
        if(!positionInstance[WndIndex]) {
            positionInstance[WndIndex] = new PluginCanvasES6();
            positionInstance[WndIndex].init($drawCanvas, function (data) {
                fastPostionEvent(data);
            });
            positionInstance[WndIndex].addChangeShapeEvent();
        }
        positionInstance[WndIndex].setRegionNum('rect', 1);
        positionInstance[WndIndex].drawStart('rect', {...canvasParam});
        const fastPostionEvent = (data) =>  {
            let pos = data.data;
            let left = pos[0][0];
            let top = pos[0][1];
            let right = pos[1][0];
            let bottom = pos[1][1];
            let x = (right + left) / (2 * 8192),
                y = (bottom + top) / (2 * 8192);
            let maxlength = Math.abs(bottom - top) > Math.abs(right - left) ? (bottom - top) : (right - left);
            let zoomtemp = (maxlength / 8192);
            let zoom = (right - left) > 0 ? zoomtemp * zoomtemp : (-1) * zoomtemp * zoomtemp;
            let request = Promise.resolve();
            let useNewApi = ChannelPTZCaps?.[channel]?.MoveDirectly?.Type?.includes('Real');
            if(useNewApi) {
                request = RPC.PTZBase.moveDirectly({
                    channel : channel,
                    startPoint : [left,top],
                    endPoint : [right, bottom]
                });
            }else{
                request = RPC.PTZManager('moveDirectly',channel,{screen:[x, y, zoom]});
            }
            request.then(() => {
                console.log('快读定位成功！')
            });
    ...
    const stopFastPostion = () => {
        if(positionInstance[WndIndex]) {
            positionInstance[WndIndex].removeShapeDrawEvent();
            positionInstance[WndIndex] = null;
        }
    }
    /**
     * @description 登出当前设备
    ...
    const onGetCapture = () => {
        if(playerInstance[WndIndex]) {
            let picData = playerInstance[WndIndex].getCapture('jpg', 1.0); //参数释义：type(jpg/jpeg/png),quality(0-1.0)
            console.log(picData);
        }
    }
    /**
    ...
    const updateTable = () => {
        ...
        _doc.querySelectorAll('[id^=h5_check_]').forEach(function(item) {
            item.addEventListener('click', function(event) {
                ...
                if(event.target.checked) {
                    //渲染裁剪时间
                    let pageToIndex = (curPage - 1 ) * lINENUMBER;
                    let _index = event.target.getAttribute('id').split('_')[2] - 0 + pageToIndex;
...
window.onload = function() {
    if (!['http:', 'https:'].includes(window.location.protocol)) {
        confirm('请参考readme.txt，在https server中打开此页面');
        try {
            window.close();
        } catch (e) {
            alert('请手动关闭此标签页');
