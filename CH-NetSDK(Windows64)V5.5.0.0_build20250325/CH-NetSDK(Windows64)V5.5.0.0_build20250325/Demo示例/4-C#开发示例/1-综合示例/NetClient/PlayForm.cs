...
namespace NetClient
{
    ...
    public partial class PlayForm : BaseForm 
    {
                                                 ...
                                                 Int32 _lpUserData)
        {
            ...
            if (_ulID == m_ulConnID)
            {
	            if (_iFlag == 1)//Process the file header
	            {
                    if (m_iPlayerID == -1)
                    {
                        Control.CheckForIllegalCrossThreadCalls = false;
                         m_iPlayerID = PLAYSDK.TC_CreatePlayerFromVoD(pnlVideoPlay.Handle, _ucData, _iLen);//Create a VOD player
		                if (m_iPlayerID >= 0)
		                {
                             int i = PLAYSDK.TC_Play(m_iPlayerID);//Play
                            if (i < 0)
                            {
        ...
        {
            if (m_ClientInfo.m_iServerID == -1 || (Int32)m_ulConnID == -1)
	        {
		        return;
	        }
        ...
        {
             PLAYSDK.TC_Stop(m_iPlayerID);//Stop playing
            PLAYSDK.TC_StopAudio(m_iPlayerID);
             PLAYSDK.TC_DeletePlayer(m_iPlayerID);//Delete the player instance
        ...
        {
            if (m_ClientInfo.m_iServerID == -1 || (Int32)m_ulConnID == -1)//|| m_iDownloadPause)
            {
                return;
            }
