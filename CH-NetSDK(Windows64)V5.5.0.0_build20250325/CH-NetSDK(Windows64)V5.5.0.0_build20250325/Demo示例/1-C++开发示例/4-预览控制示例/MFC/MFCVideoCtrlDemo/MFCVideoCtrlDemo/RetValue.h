...
#define RET_BUILD_PROTOCOL_FAILED			-24		//build protocol error
#define RET_SEND_PROTOCOL_FAILED			-25		//send protocol error
...
#define RET_LOAD_OSCORE_FAILED				-29		//sdk load OsCore.dll fail
...
#define RET_NOT_FOUND_VALIED_COMPOSEID		-37		//not find valild composeid
#define RET_UNSUPPORT_APICALL				-38     //不支持的接口调用
#define RET_RELEASE_QUERY_FAILED			-39		//释放查询任务资源失败
...
#define RET_HOST_DOMAINNAME_FAILED			-43		//解析域名失败
#define RET_INIT_NETWORK_FAILED				-44		//初始化网络失败
#define RET_CONNECT_NETWORK_FAILED			-45		//连接网络失败
#define RET_SERVICES_REPLY_FAILED			-46		//服务回码结果是失败，对应协议字段值是1
#define RET_SERVICES_REPLY_NOTRESPOND		-47		//服务回码结果是超时(web未响应)，对应协议字段值是2
#define RET_SERVICES_REPLY_UNKNOWN			-48		//服务回码结果是未知初始状态
#define RET_WAIT_SERVICES_REPLY_TIMEOUT		-49		//等待服务回码超时
...
#define RET_SEND_LIST_FULL					-105	//发送链表满了
...
#define RET_RECV_LIST_FULL					-112	//接收链表满，性能不足，数据处理不过来
#define  RET_EXCEED_USABLE_PLAYER_NUM 		-113 //正在使用中的流播放器数超过了最大可用的流播放器数量
#define  RET_MAX_USABLE_PLAYER_NUM_ERR 		-114 //设置的最大可用流播放器数错误
#define RET_RECV_FILE_MAX_COUNT				-115	//下载路数已达最大下载路数
...
#define RET_REMOTEWAKEUP_TIMEOUT			-219	//根据设备出厂ID对休眠设备进行唤醒
#define RET_ACTIVE_QUERYNVSDORMANCY_TIMEOUT	-220	//根据设备出厂ID查询设备休眠状态
#define RET_GDDZ_AUTHENTICATION_FAILED		-221	//GDDZ双向认证失败
...
#define RET_SYNCTALK_ALREADY_CONN			-326	//同步对讲已经建立连接
...
#define RET_DEC_VIDEO_OVER_LEN				-430	//解码器视频解码传入的buff长度不够

#define RET_FM_CPC_LIBRARY					-500	//加密卡驱动加载失败
#define RET_FM_CPC_OPENDEVICE				-501	//加密卡打开失败
#define RET_FM_CPS_SM3INIT					-502	//加密卡初始化失败
#define RET_FM_CPS_SM3UPDATE				-503	//加密卡数据更新失败
#define RET_FM_CPS_SM3FINAL					-504	//加密卡计算MAC失败
