...
extern "C"{
	...
	int __stdcall NetClient_RegisterDrawFunEx(unsigned int _ulConID, pfCBDrawFun _pfDrawFun, void* _pvUserData, void *_pCmd, int _iCmdLen);
...
int __stdcall NetClient_StartPlayEx(unsigned int _ulID, void* _pvBuff, int _iBuffSize);
...
ITS_OPTIM_CMD_SET //set the first GB2312 characters optimized license plate
...
ITS_OPTIM_CMD_SET use SetLicensePlateOptimizeFirstcharacter //set license plate optimization first GB2312 characters
...
ITS_OPTIM_CMD_GET //set license plate optimization first GB2312 characters
...
ITS_OPTIM_CMD_GET use SetLicensePlateOptimizeFirstcharacter //set license plate optimization first GB2312 characters
...
Description  ：	Video file playback control interface, such as play/pause/fast/slow.....You must use private player library
Prameter(in) ：		[IN] _ulConID:Return value NetClient_PlayBack
				    [IN] _iControlCode： 
                   	[IN] _pcInBuffer:
					[IN] _iInLen:
					[IN]    _pcOutBuffer
...
/***************************************************************
Function     ：	NetClient_PlayerControl
Description  ：	Pass through parameters to player library interface: TC_PlayerControl
Prameter(in) ：		[IN] _uiConID:Return value NetClient_StartRecv/NetClient_StartRecvEx/NetClient_StartRecv_V4/NetClient_StartRecv_V5
				    [IN] _iCmd: See the AVPlaySdkTypes.h header file for details,such as CTRL_DRAW_FACE_PARAM......
                   	[IN] _pvCmdBuf:Parameter structure, see development document for details 
					[IN] _iBufLen:Parameter structure size
					[IN]    _pvUsrData
return       ：		0: correct.
others       ：2020-09-24
***************************************************************/
int __stdcall NetClient_PlayerControl(unsigned int _uiRecvID, int _iCmd, void* _pvCmdBuf, int _iBufLen, void* _pvUsrData);
...
/***************************************************************
Function	:	NetClient_XmlSetDevConfig
Description:	Set the configuration information of the device by xml protocol
Prameter	:	[IN] _iLogonID: Login ID
				[IN] _iCmd: Device Configuration Commands, see Configuration Commands
				[IN] _pvInPara: Input data buffer pointer, can not be NULL
				[IN] _iInSize: The length of the input data buffer, in bytes
				[OUT] _pvOutRet: output para data pointer, the variable of struct XmlResponseStatus
				[IN] _iOutSize: The length of the output data buffer, in bytes
Parameter details: when _iCmd == 0(NETXMLCFG_REGISTER_CENTER): set the register center information
					 _pvInParaBuf: input parameter , the variable of struct XmlRegistrationCenter
					 _iInBufLen: the size of struct, sizeof(XmlRegistrationCenter).
					 _pvOutParaBuf: unnecessary parameter, can set be NULL 
					 _iOutBufLen: unnecessary parameter, can set be 0.
others		:	2020.08.17
***************************************************************/
int __stdcall NetClient_XmlSetDevConfig(int _iLogonID, int _iCmd, void* _pvInPara, int _iInSize, void* _pvOutRet, int _iOutSize);

/***************************************************************
Function	:	NetClient_XmlGetDevConfig
Description:	Get the configuration information of the device by xml protocol
Prameter	:	[IN] _iLogonID: Login ID
				[IN] _iCmd: Device Configuration Commands, see Configuration Commands
				[IN] _pvInParaBuf: Input data buffer pointer
				[IN] _iInBufLen: The length of the input data buffer, in bytes
				[OUT] _pvOutParaBuf: output para data pointer, can not be NULL
				[IN] _iOutBufLen: The length of the output data buffer, in bytes
Parameter details: when _iCmd == 0(NETXMLCFG_REGISTER_CENTER): get the register center information
					 _pvInParaBuf: unnecessary parameter, can set be NULL 
					 _iInBufLen: unnecessary parameter, can set be 0.
					 _pvOutParaBuf: output parameter , the variable of struct XmlRegistrationCenter
					 _iOutBufLen: the size of struct, sizeof(XmlRegistrationCenter).
others		:	2020.08.17
***************************************************************/
int __stdcall NetClient_XmlGetDevConfig(int _iLogonID, int _iCmd, void* _pvInPara, int _iInSize, void* _pvOutRet, int _iOutSize);

/***************************************************************
Function	:	NetClient_XmlCmdConfig
Description:	Set the cmd configuration information of the device by xml protocol
Prameter	:	[IN] _iLogonID: Login ID
				[IN] _iCmd: Device Configuration Commands, see Configuration Commands
				[IN] _pvInPara: Input data buffer pointer, can not be NULL
				[IN] _iInSize: The length of the input data buffer, in bytes
				[OUT] _pvOutRet: output para data pointer, the variable of struct result
				[IN] _iOutSize: The length of the output data buffer, in bytes
Parameter details: when _iCmd == 0(NETXMLCMD_SMART_PIC_QUERY): set the register center information
					 _pvInParaBuf: input parameter , the variable of struct XmlSmartPicQueryCondition
					 _iInBufLen: the size of struct, sizeof(XmlSmartPicQueryCondition).
					 _pvOutParaBuf: output parameter , the variable of structXmlSmartPicQueryResultArr
					 _iOutBufLen: the size of struct, sizeof(structXmlSmartPicQueryResultArr).
others		:	2022.06.16
***************************************************************/
int __stdcall NetClient_XmlCmdConfig(int _iLogonID, int _iCmd, void* _pvInPara, int _iInSize, void* _pvOutRet, int _iOutSize);

/***************************************************************
Function	:	NetClient_CommonFuntion
Description:	the common funtion that not related with logonid
				like: NETCLIENT_COMMONFUNC_CMD_REPAIR_MP4FILE Repaire the Mp4 File.the mp4 file is not have the moov box when the program exit abnormal 
				when the mp4 file is success ,the new file is _pcFileName,  the old file is rename by add .bak
Prameter	:	[IN] _iCmd: Command to common function ,NETCLIENT_COMMONFUNC_CMD_REPAIR_MP4FILE is to repair mp4 file
				[IN] _pvBuf: buffer to set param 
				[IN] _iBufSize : buffer size

others		:	2022.08.15
***************************************************************/
int __stdcall NetClient_CommonFuntion(int _iCmd, void *_pvBuf, int _iBufSize);

/***************************************************************
Function	:	NetClient_SetDzTransparentPara
Description:	DZ transparent parameter configuration and settings interfaces
Prameter	:	[IN] _iLogonID: Login ID, login handle
				[IN] _pvParaBuf: Input data buffer pointer, input DzTransparentPara structure pointer
				[IN] _iBufSize: Input the size of the input data buffer, in bytes sizeof(DzTransparentPara)
				[IN] _blSync: Synchronization flag: 0(false)-asynchronous non blocking(parameter change message:PARA_DZ_TRANSPARENT), 1(true)-synchronous blocking
				[IN] _blTranscoding: 0(false)-transparent data; 1(true)-sdk transcoding characters,with NetClient_ SetSDKInitConfig:INIT_CONFIG_CLIENT_CHARSET use
others		:	2023.11.27
***************************************************************/
int __stdcall NetClient_SetDzTransparentPara(int _iLogonID, void* _pvParaBuf, int _iBufSize, bool _blSync, bool _blTranscoding);

/***************************************************************
Function	:	NetClient_GetDzTransparentPara
Description:	DZ transparent parameter query and acquisition interface
Prameter	:	[IN] _iLogonID: Login ID, login handle
				[IN] _pvParaBuf: Output data buffer pointer, input DzTransparentPara structure pointer
				[IN] _iBufSize: Input the size of the output data buffer, in bytes sizeof(DzTransparentPara)
				[IN] _blSync: Synchronization flag: 0(false)-asynchronous non blocking(main message:WCM_DZ_TRANSPARENT), 1(true)-synchronous blocking
				[IN] _blTranscoding: 0(false)-transparent data; 1(true)-sdk transcoding characters,with NetClient_ SetSDKInitConfig:INIT_CONFIG_CLIENT_CHARSET use
others		:	2023.11.27
***************************************************************/
int __stdcall NetClient_GetDzTransparentPara(int _iLogonID, void* _pvParaBuf, int _iBufSize, bool _blSync, bool _blTranscoding);
