...
CLS_DeviceManager::~CLS_DeviceManager(void)
{
	...
	ItProtocolMap itProtocolMap = m_mapProtocol.begin();
	while(m_mapProtocol.end() != itProtocolMap)
	{
		while(!itProtocolMap->second.empty())
		{
			char* pcTemp = itProtocolMap->second.front();
			itProtocolMap->second.pop();
			free(pcTemp);
		}
		++itProtocolMap;
	}
...
BOOL CLS_DeviceManager::IsDeviceEqual(PDEVICE_INFO _pDevice,char* _pcIP, int _iPort, char* _pcProxy,char* _pcID)
{
	...
	if (0 == strlen(_pcID)*strcmp(_pcID,_pDevice->cID) 
				...
				/*&& 0 == strlen(_pcProxy)*strcmp(_pcProxy,_pDevice->cProxy)*/)//Direct connection and proxy cannot be used at the same time
	{
		return TRUE;
	}
...
PDEVICE_INFO CLS_DeviceManager::AddDevice( int _iLogonID )
{
	...
	if (m_mapDevice.end() == it)
	{
		...
		queue<char *> queNew;
		m_mapProtocol[_iLogonID] = queNew;
	}
...
PDEVICE_INFO CLS_DeviceManager::RemoveDevice(int _iLogonID)
{
	...
	ItProtocolMap itProtocolMap = m_mapProtocol.find(_iLogonID);
	if(m_mapProtocol.end() != itProtocolMap)
	{
		while(!itProtocolMap->second.empty())
		{
			char* pcTemp = itProtocolMap->second.front();
		...
		m_mapProtocol.erase(itProtocolMap);
	}
...
PDEVICE_INFO CLS_DeviceManager::RemoveDevice( char* _pcIP, int _iPort,char* _pcProxy,char* _pcID,int* _piLogonID )
{
	...
	ItProtocolMap itProtocolMap = m_mapProtocol.find(iLogonID);
	if(m_mapProtocol.end() != itProtocolMap)
	{
		while(!itProtocolMap->second.empty())
		{
...
queue<char *>* CLS_DeviceManager::GetProtocolQueue(int _iLogonID)
{
	ItProtocolMap itProtocolMap = m_mapProtocol.find(_iLogonID);
	if(m_mapProtocol.end() == itProtocolMap)
	{
		queue<char *> queNew;
		m_mapProtocol[_iLogonID] = queNew;
		return &m_mapProtocol[_iLogonID];
	}
	return &itProtocolMap->second;
}
