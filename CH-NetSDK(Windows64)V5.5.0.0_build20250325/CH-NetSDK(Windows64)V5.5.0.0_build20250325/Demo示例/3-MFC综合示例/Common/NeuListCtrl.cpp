...
void CNeuListCtrl::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pNMListView != NULL)
	{
		...
		//Cancel the selected state of the last row each time you click --add by litao
		...
		case DATECTRL:
			{
				...
				((CDateTimeCtrl*)pSubItemInfo->m_pWnd)->SetFormat("HH:mm");//Switch to timed cruise, if necessary, need to expand
				break;
			}
		default: 
			break;
...
{
	int iCount = this->GetItemCount();//If there is no content in the list ctrl, you don't need to get the value
	if (iCount < 1)
	{
		return ;
	}
					...
					case COMBOBOX:
						{
							...
							{
								//The following code is used to initialize and read the existing combox content, it can be displayed normally, so when adding combox content, be sure to setitemdata
...
HBRUSH CNeuListCtrl::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	// TODO: If the default one is not the desired brush, return another brush
	return hbr;
}
...
void CNeuListCtrl::OnNMCustomdraw(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	if (lplvdr != NULL)
	{
		...
		switch(lplvdr->nmcd.dwDrawStage)//Judgment status
		{
		case CDDS_PREPAINT:
			{
		...
		case CDDS_ITEMPREPAINT://If the color is changed before drawing the ITEM
			{
				COLORREF ItemColor;
				if(MapItemColor.Lookup((unsigned long &)nmcd.dwItemSpec, ItemColor))
					//Search in the template according to the //ITEM number and COLORREF set in SetItemColor(DWORD iItem, COLORREF color), and then assign the color.
				{
...
{
	MapItemColor.SetAt(iItem, color);//Set the color of a row.
	this->RedrawItems(iItem, iItem);//Re-dye
	this->SetFocus();//Set the focus
	UpdateWindow();
}

void CNeuListCtrl::OnCbnSelchangeCombo()//Get Combox Content
{
	if (m_pControlPtrArray)
	{
		CString strTemp ;
