...
void CLS_FilePlayReviewPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SLIDER_PBK_VOLUME, m_sldPbkVolume);
	DDX_Control(pDX, IDC_CHECK_SHOW_BITRATE, m_chkShowBitRate);
}
...
BEGIN_MESSAGE_MAP(CLS_FilePlayReviewPage, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_PLAYBACK_AUDIO, &CLS_FilePlayReviewPage::OnBnClickedCheckPlaybackAudio)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_PBK_VOLUME, &CLS_FilePlayReviewPage::OnNMCustomdrawSliderPbkVolume)
	ON_BN_CLICKED(IDC_CHECK_SHOW_BITRATE, &CLS_FilePlayReviewPage::OnBnClickedCheckShowBitrate)
END_MESSAGE_MAP()
...
BOOL CLS_FilePlayReviewPage::OnInitDialog()
{
    ...
    SetDlgItemInt(IDC_EDIT_PLAY_SPEED, 2);
	m_sldPbkVolume.SetRange(0, 65535, TRUE);
	m_sldPbkVolume.SetPos(0);
	m_sldPbkVolume.SetLineSize(655);
...
void CLS_FilePlayReviewPage::SetDownloadParam( int _iLogonID, CString _szRemoteFileName, int _iFileAttr)
{
	m_iLogonID = _iLogonID;
	m_szFileName = _szRemoteFileName;
	m_iDownloadType = DownloadByFile;
	m_iFileAttr = _iFileAttr;
}

void CLS_FilePlayReviewPage::SetDownloadParamEx( int _iLogonID, CString _szRemoteFileName, int _iFileAttr)
{
	m_iLogonID = _iLogonID;
	m_szFileName = _szRemoteFileName;
	...
	if (_ulID == pThis->m_ulConnID)
	{
		...
		else
		{
			if (pThis->m_iPlayerID >= 0)
			{
				...
				{
					if (_iLen < ((int)sizeof(pThis->m_cDataBuffer) - pThis->m_iDataLen))
					{
...
void CLS_FilePlayReviewPage::StopPlay()
{
	...
	//关闭显示帧率
	NetClient_ShowBitrateOnVideo(m_ulConnID, 0, 0, 0);
...
void CLS_FilePlayReviewPage::OnMainNotify( int _ulLogonID,int _iWparam, void* _iLParam, void* _iUser )
{
	...
	case WCM_START_PLAY:
		{
			if (m_ulConnID == (unsigned long)_iLParam && m_iIframe) {
				SetDlgItemInt(IDC_EDIT_PLAY_SPEED, 3);
				OnBnClickedBtnFastForward();
			}
		}
		break;
	default:
...
void CLS_FilePlayReviewPage::UI_UpdateProgressByTime()
{
	...
	if (m_iPlayerID != INVALID_ID)
	{
		...
		unsigned int iAbsoluteFrameTime = 0;
...
{
	m_iPlayerID = NetClient_GetPlayerIndex((int)m_ulConnID);
	if (m_iPlayerID >= 0)
	{
		CString szDecryptKey;
		GetDlgItemText(IDC_EDIT_PLAYBACK_DECRYPT, szDecryptKey);
		int iRet = TC_SetVideoDecryptKey(m_iPlayerID, szDecryptKey.GetBuffer(), szDecryptKey.GetLength());
		if (RET_SUCCESS == iRet)
		{
			OnBnClickedBtnPlay();
		}
		...
		{
			AddLog(LOG_TYPE_FAIL, "", "[CLS_FilePlayReviewPage::OnBnClickedButtonDecrypt] TC_SetVideoDecryptKey failed m_iPlayerID %d, iRet %d\n", m_iPlayerID, iRet);
		}
	}
}
...
void CLS_FilePlayReviewPage::UI_UpdateText()
{
	...
	SetDlgItemText(IDC_CHECK_SHOW_BITRATE, GetTextByLan(_T("显示帧率"), _T("ShowBitRate")));
}
...
void CLS_FilePlayReviewPage::OnBnClickedBtnOsd()
{
	...
	memcpy(tTmpPlayBackOsg.cOsdText, "No serenity is far away", sizeof("No serenity is far away")); 
...
void CLS_FilePlayReviewPage::OnBnClickedBtnFastForward()
{
	...
	int iOutLen = 0;
	int iSpeed = GetDlgItemInt(IDC_EDIT_PLAY_SPEED);
...
void CLS_FilePlayReviewPage::OnBnClickedBtnSlowforward()
{
	int iSpeed = GetDlgItemInt(IDC_EDIT_PLAY_SPEED);
	if (iSpeed >= 5) {
		MessageBox(GetTextByLan("慢放不支持32倍速！", "Slow playback does not support 32x speed!"), GetTextEx(IDS_CONFIG_PROMPT),MB_OK);
		return;
	}
...
void CLS_FilePlayReviewPage::StartSuperPlayBackByFile()
{
	...
	stParam.iFileAttr = m_iFileAttr;
    stParam.iPlaybackDirection = m_iDirection;
	stParam.m_iBitRateFlag = m_iBitRateFlag;
	stParam.iIFrame = m_iIframe;
	stParam.m_iDiskGroup = m_iDiskGroup;
...
void CLS_FilePlayReviewPage::OnBnClickedCheckPlaybackAudio()
{
	if (INVALID_ID == m_ulConnID) {
		return;
	}
	...
	BOOL bChecked = IsDlgButtonChecked(IDC_CHECK_PLAYBACK_AUDIO);
	if(bChecked) {
		int iOpenAudio = 1;
		iRet = NetClient_PlayBackControl(m_ulConnID, PLAY_CONTROL_START_AUDIO, &iOpenAudio, sizeof(int), NULL, NULL);
		if(RET_SUCCESS == iRet) {
			bIsAudioPlaying = TRUE;
			AddLog(LOG_TYPE_SUCC, "", "NetClient_PlayBackControl-PLAY_CONTROL_START_AUDIO(%u)", m_ulConnID);

			tPlaybackVolume tVolume = {0};
			tVolume.iSize = sizeof(tPlaybackVolume);
			int iRet = NetClient_PlayBackControl(m_ulConnID, PLAY_CONTROL_GET_VOLUME, &tVolume, sizeof(tVolume), NULL, NULL);
			if (RET_SUCCESS == iRet) {
				AddLog(LOG_TYPE_SUCC, "", "NetClient_PlayBackControl-PLAY_CONTROL_GET_VOLUME(%u, %d)", m_ulConnID, tVolume.usVolume);
				m_sldPbkVolume.SetPos(tVolume.usVolume);
			} else {
				AddLog(LOG_TYPE_FAIL, "", "NetClient_PlayBackControl-PLAY_CONTROL_GET_VOLUME(%u, &d)", m_ulConnID, tVolume.usVolume);
			}

		} else {
			AddLog(LOG_TYPE_FAIL, "", "NetClient_PlayBackControl-PLAY_CONTROL_START_AUDIO(%u)", m_ulConnID);
		}
	} else {
		int iOpenAudio = 0;
		iRet = NetClient_PlayBackControl(m_ulConnID, PLAY_CONTROL_STOP_AUDIO, &iOpenAudio, sizeof(int), NULL, NULL);
		if (RET_SUCCESS == iRet) {
			AddLog(LOG_TYPE_SUCC, "", "NetClient_PlayBackControl-PLAY_CONTROL_STOP_AUDIO(%u)", m_ulConnID);
		} else {
			AddLog(LOG_TYPE_FAIL, "", "NetClient_PlayBackControl-PLAY_CONTROL_STOP_AUDIO(%u)", m_ulConnID);
		}
	}
	CheckDlgButton(IDC_CHECK_PLAYBACK_AUDIO, bIsAudioPlaying);
}

void CLS_FilePlayReviewPage::OnNMCustomdrawSliderPbkVolume(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);

	int iVolume = m_sldPbkVolume.GetPos();
	tPlaybackVolume tVolume = {0};
	tVolume.iSize = sizeof(tPlaybackVolume);
	tVolume.usVolume = (unsigned short)iVolume;
	int iRet = NetClient_PlayBackControl(m_ulConnID, PLAY_CONTROL_SET_VOLUME, &tVolume, sizeof(tVolume), NULL, NULL);
	if (RET_SUCCESS == iRet) {
		SetDlgItemInt(IDC_STATIC_PBK_VOLUME_VALUE, iVolume*100/65535);
		AddLog(LOG_TYPE_SUCC, "", "NetClient_PlayBackControl-PLAY_CONTROL_SET_VOLUME(%u, %d)", m_ulConnID, iVolume);
	} else {
		AddLog(LOG_TYPE_FAIL, "", "NetClient_PlayBackControl-PLAY_CONTROL_SET_VOLUME(%u, &d)", m_ulConnID, iVolume);
	}

	*pResult = 0;
}

void CLS_FilePlayReviewPage::OnBnClickedCheckShowBitrate()
{
	int iEnable = m_chkShowBitRate.GetCheck();
	int iRet = NetClient_ShowBitrateOnVideo(m_ulConnID, 0, 0, iEnable);
	if (RET_SUCCESS == iRet) {
		AddLog(LOG_TYPE_SUCC,"","NetClient_ShowBitrateOnVideo(%d,0,0,%d)", m_ulConnID, iEnable);
	} else {
		AddLog(LOG_TYPE_FAIL,"","NetClient_ShowBitrateOnVideo(%d,0,0,%d)", m_ulConnID, iEnable);
	}
}
