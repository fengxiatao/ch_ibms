...
void CLS_RemoteATMFilePage::Update_UI_Text()
{
	...
	int iColumnIndex = 0;
	InsertColumn( m_ListQueryFile, iColumnIndex++, "ID", LVCFMT_LEFT, 50 );//insert column
...
void CLS_RemoteATMFilePage::OnBnClickedButtonDownload()
{
	...
	if (iRet >= 0)
	{
		int iRet = NetClient_NetFileDownloadFile(&iConnID, m_iLogonID, (char *)(LPCSTR)m_szFileName, (char *)(LPCSTR)(g_szDownloadPath + m_szFileName), 1, -1, 32);//Set the download speed to 32x
