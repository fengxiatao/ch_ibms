...
#define MAX_PLAY_PAGE_NUM				16		//Play up to max 16 channels at the same time
#define DEFAULT_PLAY_PAGE_NUM			5		//Play up to default 5 channels at the same time
...
public:
	void SetDownloadParam(int _iLogonID, CString _szRemoteFileName, int _iFileAttr);
	void SetDownloadParamEx(int _iLogonID, CString _szRemoteFileName, int _iFileAttr);
	...
	CString GetFilename();
	int		GetLogonID();
    void SetPlaybackDirection(int iDirection) {
        m_iDirection = iDirection;
    }
	void SetBitRateFlag(int _iBitRateFlag){m_iBitRateFlag = _iBitRateFlag;}
	void SetIframeFlag(int _iIframeFlag){m_iIframe = _iIframeFlag;}
	void SetDiskGroupFlag(int _iDiskGroup){m_iDiskGroup = _iDiskGroup;}
private:
    int m_iDirection;
	int m_iBitRateFlag;
	int m_iIframe;
...
public:
	...
	afx_msg void OnBnClickedCheckPlaybackAudio();
	afx_msg void OnNMCustomdrawSliderPbkVolume(NMHDR *pNMHDR, LRESULT *pResult);
protected:
	virtual void OnCancel();	
private:
	CButton m_chkShowBitRate;
public:
	afx_msg void OnBnClickedCheckShowBitrate();
};
