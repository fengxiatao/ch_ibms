...
void CLS_DSMPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_USE_NSLOOK, m_chkUseNslook);
	DDX_Control(pDX, IDC_CHECK_DSM_IPV6, m_chkIpV6);
}
...
BEGIN_MESSAGE_MAP(CLS_DSMPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_CHECK_USE_NSLOOK, &CLS_DSMPage::OnBnClickedCheckUseNslook)
	ON_BN_CLICKED(IDC_CHECK_DSM_IPV6, &CLS_DSMPage::OnBnClickedCheckDsmIpv6)
END_MESSAGE_MAP()
...
void CLS_DSMPage::OnBnClickedBtnServerLogon()
{
	...
	if (BST_CHECKED == m_chkUseNslook.GetCheck()) {
		NslookLogonRegisterServer();
	} else {
		NvssdkLogonRegisterServer();
	}
}

void CLS_DSMPage::NslookLogonRegisterServer()
{
	if (NULL != NSLook_LogoffServer && NULL != NSLook_LogonServer) {
		if (m_iRegID >= 0) {
		...
		if (m_iRegID >= 0) {
			...
			AddLog(LOG_TYPE_SUCC,"","(%d)NSLook_LogonServer(%s,%d,true)",m_iRegID, m_cRegIp, m_iRegPort);
		} else {
			EnableDSMWindow(FALSE);
			AddLog(LOG_TYPE_FAIL,"","(%d)NSLook_LogonServer(%s,%d,true)",m_iRegID, m_cRegIp, m_iRegPort);
		}
	}
}

void CLS_DSMPage::NvssdkLogonRegisterServer()
{
	ActiveDirectoryInfo tDirectory = {0};
	tDirectory.iSize = sizeof(ActiveNetWanInfo);
	int iLogonType = SERVER_NORMAL;
	if (BST_CHECKED == m_chkIpV6.GetCheck()) {
		tDirectory.iIpVer = IP_VERSION_6;
		strcpy_s(tDirectory.cDsmIpV6, sizeof(tDirectory.cDsmIpV6), m_cRegIp);
	} else {
		tDirectory.iIpVer = IP_VERSION_4;
		strcpy_s(tDirectory.cDsmIP, sizeof(tDirectory.cDsmIP), m_cRegIp);
	}
	tDirectory.iDsmPort = m_iRegPort;
	strcpy_s(tDirectory.cAccontName, sizeof(tDirectory.cAccontName), m_cRegUser);
	strcpy_s(tDirectory.cAccontPwd, sizeof(tDirectory.cAccontPwd), m_cRegPwd);
	int iRet = NetClient_SetDsmConfig(DSM_CMD_SET_DIRECTORY_INFO, &tDirectory, sizeof(ActiveDirectoryInfo));
	if (RET_SUCCESS != iRet) {
		EnableDSMWindow(FALSE);
		AddLog(LOG_TYPE_FAIL, "", "NvssdkLogonRegisterServer:NetClient_SetDsmConfig::DSM_CMD_SET_DIRECTORY_INFO");
	} else {
		EnableDSMWindow(TRUE);
		AddLog(LOG_TYPE_SUCC, "", "NvssdkLogonRegisterServer:NetClient_SetDsmConfig::DSM_CMD_SET_DIRECTORY_INFO");
	}
}

void CLS_DSMPage::OnBnClickedBtnServerVersion()
{
	if (NULL == NSLook_GetServerVersion) {
		return;
	}
...
void CLS_DSMPage::OnBnClickedCheckUseNslook()
{
	UpdateRegisterInfo();
}

void CLS_DSMPage::UpdateRegisterInfo()
{
	GetDlgItemText(IDC_EDIT_SERVER_IP, m_cRegIp, sizeof(m_cRegIp));
	GetDlgItemText(IDC_EDIT_USERNAME, m_cRegUser, sizeof(m_cRegUser));
	GetDlgItemText(IDC_EDIT_PASSWORD, m_cRegPwd, sizeof(m_cRegPwd));
	m_iRegPort = GetDlgItemInt(IDC_EDIT_SERVER_PORT);
	RegisterInfo tInfo = {0};
	if (BST_CHECKED == m_chkUseNslook.GetCheck()) {
		tInfo.blUseNslook = TRUE;
	} else {
		tInfo.blUseNslook = FALSE;
	}
	if (BST_CHECKED == m_chkIpV6.GetCheck()) {
		tInfo.blUseIpV6 = TRUE;
	} else {
		tInfo.blUseIpV6 = FALSE;
	}
	if(NULL != m_pCurrent) {
		strcpy_s(tInfo.cRegUser, sizeof(tInfo.cRegUser), m_cRegUser);
		strcpy_s(tInfo.cRegPwd, sizeof(tInfo.cRegPwd), m_cRegPwd);
		strcpy_s(tInfo.cRegIP, sizeof(tInfo.cRegIP), m_cRegIp);
		tInfo.iRegPort = m_iRegPort;
		m_pCurrent->SetRegisterInfo(&tInfo);
	}
}

void CLS_DSMPage::OnBnClickedCheckDsmIpv6()
{
	UpdateRegisterInfo();
}
