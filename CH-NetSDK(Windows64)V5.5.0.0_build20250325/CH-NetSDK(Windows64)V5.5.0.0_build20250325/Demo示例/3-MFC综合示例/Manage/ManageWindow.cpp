...
BOOL CLS_ManageWindow::OnInitDialog()
{
	...
	else if (MANAGE_DDNS == m_iActiveShowType)
	{
		m_tabManage.InsertItem(MANAGE_DDNS, _T("DDNS"));
	}
	else
	{
		m_tabManage.InsertItem(MANAGE_ACTIVE,_T("Active"));
...
int CLS_ManageWindow::ShowBasePage( int _iIndex )
{
	...
	if (NULL == m_pPage[_iIndex])
	{
		...
		case MANAGE_DDNS:
			{
				m_pPage[_iIndex] = new CLS_ActiveDdnsPage(this);
				m_pPage[_iIndex]->Create(IDD_DLG_MNG_DDNS, this);
			}
			break;
		}
