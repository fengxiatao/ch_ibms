...
{
#define INSERT_PRODUCTMODEL(id)				m_mapProductModel.insert(pair<int,char*>(id, (char*)#id));
#define INSERT_PRODUCTMODEL_M7(id, idEx)     Make_M7_PM(id, #id, (char*)#idEx)
	...
	INSERT_PRODUCTMODEL(TC_FS0013ERS_HD);         // Modify the HD player
	INSERT_PRODUCTMODEL(TC_NC9100S3E_MP_IR30);    //
	INSERT_PRODUCTMODEL(TC_NC9000S3_2MP_E);       // 200W camera Zhu builds the project
	INSERT_PRODUCTMODEL(TC_NC9200S3_MP_E_IR15);   // Previously reused 3 million productmodel 0x0059, Xu Mengmeng project
	INSERT_PRODUCTMODEL(TC_NC9100S1_MP_IR);       // 0xC0046, Liu Zilong project
	INSERT_PRODUCTMODEL(TC_NS921S3_HD_V1);        // 0xC0056, the TC_NS921_N1_2MP device of the Ma Zunfeng project was revoked and changed to this one
	INSERT_PRODUCTMODEL(TC_NC9000S3_MP_E);        // Qi Xueqing project
	...
	INSERT_PRODUCTMODEL(TC_H908NC_HD_2);
...
#define INSERT_DEVICETYPE(id)\
	m_mapProductModel.insert(pair<int,char*>(TYPE_##id, (char*)#id));
...
//#endif
	...
	pParent->GetTransProtocolCallbackFun(&tLogonPara.pvTransUdata);
