...
public:
	...
	afx_msg LRESULT OnAlarmNotify_V5(WPARAM wParam, LPARAM lParam);
#ifdef SDK_X64
	...
	static void AlarmNotify_V5(int _iLogonID, int _iAlarmType, void* _pInfo, int _iSize, void* _pUser);
	...
	static void TransProtocolData(int _iLogonID, int _iCmd, void* _pvData, int _iLen, void* _pvUsr);
	CTreeCtrl* GetDeviceTreeCtrl();
	void OnLogoffDevice(int _iLogonID);
	void GetTransProtocolCallbackFun(void** _pvThis);
...
private:
	...
	void OnLogon();
	void OnLogoff();
	int GetNetModeByMenuID(UINT _uiID);
	...
	int GetLogonFaildReason(int _iLogonID, int _iStatus);
	...
	void AddAlarmInfo(int _iLogonID, int _iChannel, int _iAlarmType, int _iAlarmState, void* _pvAlmPara, int _iParaSize);
	...
	void SetSdkWorMode();
	void SetExternDevLogonMode();
	void UI_DeleteDevice(HTREEITEM _hItem, int _iLogonID);
	void LoadSdkVideoMode();
	void ProcessDZVideoCover(int _iLogonID, unsigned int _uiConID, int _iChanNo, int _iStreamNo);
...
public:
	...
	int GetDemoUseMode();
	static BOOL m_blProcStats;		//Whether to enable the flag of protocol statistics
};
