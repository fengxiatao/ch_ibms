...
class CLS_QueueStand : public CLS_BasePage
{
	DECLARE_DYNAMIC(CLS_QueueStand)

public:
	CLS_QueueStand(CWnd* pParent = NULL);
	virtual ~CLS_QueueStand();
	virtual BOOL OnInitDialog();
	virtual void OnChannelChanged(int _iLogonID, int _iChannelNo, int _iStreamNo);
	virtual void OnLanguageChanged(int _iLanguage);
	void UI_UpdateUIText();
	void GetQueueStandInfo();				//Get the Education Cohort detection parameters
	...
	enum { IDD = IDD_DIALOG_CFG_XML_QUEUESTAND };
...
public:
	...
	ValidRgQueueStand m_tValidRegion[MAX_REGION_COUNT_CLOTH];	//Valid area coordinates
