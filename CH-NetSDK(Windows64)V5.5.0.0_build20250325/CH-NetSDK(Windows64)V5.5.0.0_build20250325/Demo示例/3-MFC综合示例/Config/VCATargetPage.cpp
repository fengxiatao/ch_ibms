...
#define LEN_2		2		//define length

// CLS_VCATargetPage dialog

//	VIDEOSIZE The corresponding resolution value in the list
...
BOOL CLS_VCATargetPage::OnInitDialog()
{
	...
	m_cboAlarmColor.SetCurSel(0);	//target's alarm color
...
void CLS_VCATargetPage::UpdatePageUI()
{
	...
	if(iRet >= 0)
	{
		//An error occurs when m_iChannelNO = -1
...
{
	int iChannelType = 0;
	//Get channel type: 1, local analog channel; 2, number channel; 3, synthesis channel
