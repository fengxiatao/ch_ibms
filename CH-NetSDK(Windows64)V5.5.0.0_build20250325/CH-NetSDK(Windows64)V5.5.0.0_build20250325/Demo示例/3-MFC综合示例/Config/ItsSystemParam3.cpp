// ItsSystemParam3.cpp : Implementation file
//
...
void CLS_ItsSystemParam3::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK0, m_chkDevtype0);
	DDX_Control(pDX, IDC_CHECK1, m_cboDevType1);
	DDX_Control(pDX, IDC_CHECK2, m_chkDevType2);
	DDX_Control(pDX, IDC_CHECK3, m_chkDevType3);
	DDX_Control(pDX, IDC_CHECK4, m_chkDevType4);
	DDX_Control(pDX, IDC_CHECK5, m_chkDevType5);
...
BEGIN_MESSAGE_MAP(CLS_ItsSystemParam3, CLS_BasePage)
	...
	ON_CBN_SELCHANGE(IDC_CBO_DEVTYPE, &CLS_ItsSystemParam3::OnCbnSelchangeCboDevtype)
END_MESSAGE_MAP()


// CLS_ItsSystemParam3 message handler
...
void CLS_ItsSystemParam3::UI_UpdateData1()
{
	...
	if (0 == iRet1)
	{
		...
		GetDlgItem(IDC_EDIT_ROAD_NAME)->SetWindowText((LPSTR)(tSetDeviceType.tITS_CamLocation.cRoadName));
		m_chkDevtype0.SetCheck(tSetDeviceType.iDeviceType&1);
		m_cboDevType1.SetCheck(tSetDeviceType.iDeviceType&(1<<1));
		m_chkDevType2.SetCheck(tSetDeviceType.iDeviceType&(1<<2));
		m_chkDevType3.SetCheck(tSetDeviceType.iDeviceType&(1<<3));
		m_chkDevType4.SetCheck(tSetDeviceType.iDeviceType&(1<<4));
		m_chkDevType5.SetCheck(tSetDeviceType.iDeviceType&(1<<5));
...
void CLS_ItsSystemParam3::UI_UpdateData3()
{
	...
	tRecieveCapture.iType = 0;
	tRecieveCapture.iDevType = m_cboDevType.GetCurSel();
	tRecieveCapture.iMode = 1;
...
void CLS_ItsSystemParam3::OnBnClickedBtnSetCapture()
{
	if(m_iLogonID >= 0)
	{
		...
		tSetCapture.iDevType = m_cboDevType.GetCurSel();
		tSetCapture.iMode = 1;
...
void CLS_ItsSystemParam3::OnBnClickedBtnDeviceNumber()
{
	...
	{
		ITS_CamLocationEx tSetDeviceType = {0};
		tSetDeviceType.iSize = sizeof(ITS_CamLocationEx);
		tSetDeviceType.iDeviceType |= (m_chkDevtype0.GetCheck())|( m_cboDevType1.GetCheck() << 1)|( m_chkDevType2.GetCheck() << 2)|( m_chkDevType3.GetCheck()<<3)|( m_chkDevType4.GetCheck()<< 4)|( m_chkDevType5.GetCheck()<<5);
		GetDlgItemText(IDC_EDIT_ROAD_NAME,tSetDeviceType.tITS_CamLocation.cRoadName, LEN_64);
...
void CLS_ItsSystemParam3::OnCbnSelchangeCboDevtype()
{
	UI_UpdateData3();
}
