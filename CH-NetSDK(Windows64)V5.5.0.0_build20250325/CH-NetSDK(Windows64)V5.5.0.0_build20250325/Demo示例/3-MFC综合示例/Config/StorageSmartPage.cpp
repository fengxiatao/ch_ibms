...
void CLS_StorageSmartPage::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	{
		UI_UpdateSurface();
		OnCbnSelchangeCboSmartDisk();//Displays SMART information for the first disk by default
	}
}
...
void CLS_StorageSmartPage::UI_UpdateSmartInfo(DiskSmartMsg* _pstSmartMsg)
{
	...
	SetDlgItemTextEx(IDC_EDT_SMART_EVALUATE_WHOLE, _pstSmartMsg->iAllHealth?IDS_SMART_HEALTH:IDS_SMART_UNHEALTH);
...
void CLS_StorageSmartPage::OnCbnSelchangeCboSmartDisk()
{
	...
	int iRetAses = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_SMART_CHECK, m_iChannelNo, &stDiskSmartAssess, sizeof(SmartCheck), &iReturn);
	...
	if (RET_FAILED == iRetAses)
	{
		...
		AddLog(LOG_TYPE_FAIL, "", "NetClient_GetDevConfig(%d,%d,%d)",m_iLogonID, NET_CLIENT_SMART_CHECK, m_iChannelNo);
	}
...
void CLS_StorageSmartPage::OnBnClickedChkSmartDiskAssess()
{
	...
	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_SMART_CHECK, m_iChannelNo, &stDiskSmartAssess, sizeof(SmartCheck));
	if (RET_SUCCESS != iRet)
	{
		AddLog(LOG_TYPE_FAIL, "", "NetClient_SetDevConfig(%d,%d,%d)",m_iLogonID, NET_CLIENT_SMART_CHECK, m_iChannelNo);
	}
}
