...
void CLS_AppendOSDPage::OnParamChangeNotify(int _iLogonID, int _iChannelNo, int _iParaType,void* _pPara,int _iUserData)
{
	...
	case PARA_APPEND_OSD:
		{
			if (OSD_IRRIGATION_USED == (m_cboOSDArea.GetCurSel() + 1))
			{
				m_edtOSDContent.EnableWindow(FALSE);
			}
			else
			{
				m_edtOSDContent.EnableWindow(TRUE);
			}
			UpdatePageUI();
		}
		break;
...
void CLS_AppendOSDPage::UI_UpdateDialog()
{
	...
	stFuncAbilityLevel.iMainFuncType = MAIN_FUNC_TYPE_AUDIO_VIDEO;
	stFuncAbilityLevel.iSubFuncType = 100;
	int iReturnByte = -1;
	m_iChannelNo = m_iChannelNo < 0 ? 0 : m_iChannelNo;

	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_GET_FUNC_ABILITY, m_iChannelNo, &stFuncAbilityLevel, sizeof(FuncAbilityLevel), &iReturnByte);
	if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "[CLS_AppendOSDPage::NetClient_GetDevConfig] Get fail,error = %d", GetLastError());
	}
	int iOsdRec = atoi(stFuncAbilityLevel.cParam);
	CString AreaName;
	if (iOsdRec <= 0)
	{
	...
	else
	{
		...
		for (int i = 0;i < iOsdRec;++i)
		{
			AreaName.Format(_T("Area%d"),i+1);
			m_cboOSDArea.AddString(AreaName);
		}
	}
...
#define MAX_TEXT_LEN 2048
	...
	for(int i = 0;i < m_edtOSDContent.GetLineCount(); ++i)
	{
		...
		cTemp[len] = '\0';//To manually add terminators
