...
enum LanguageType
{
	LAN_ENGLISH = 0,			//	English
	LAN_CHINESE = 1,			//	Simplified Chinese
	LAN_TRADITIONAL = 2,			//	traditional Chinese
	LAN_KOREAN = 3,		//	Hangul (Korean)
	LAN_SPANISH = 4,	//	Spanish
	LAN_ITALIAN = 5,	//	Italian
	LAN_RUSSIAN = 6,	//	Russian
	LAN_TURKISH = 7,	//	Turkish
	LAN_THAI = 8,		//	Thai
	LAN_POLISH = 9,		//	Polish
	LAN_HEBREW = 10,	//	Hebrew
	LAN_FRENCH = 11,	//	French
	LAN_GERMAN = 12,	//	German
};
...
BEGIN_MESSAGE_MAP(CLS_AutoTestPage, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_SETALI, &CLS_AutoTestPage::OnBnClickedButtonSetali)
END_MESSAGE_MAP()
...
void CLS_AutoTestPage::OnMainNotify( int _ulLogonID,int _iWparam, void* _iLParam, void* _iUser )
{
	...
	int iMsgeTypeEx = HIWORD(_iWparam);
	switch (iMsgType)
	{
	...
	switch(iMsgeTypeEx)
	{
	case WCM_ALI_SETSECRET:
		{
			AliSecretResult tInfo = {0};
			memcpy(&tInfo, _iLParam, (int)sizeof(AliSecretResult));
			if (RET_SUCCESS == tInfo.iResult)
			{
				CString strChannel = "";
				strChannel.Format("channel:%d", iMsgType);
				MessageBox(GetTextByLan(_T("³É¹¦"), _T("Success")) + strChannel);
			}
			else
			{
				CString strChannel = "";
				strChannel.Format("channel:%d", iMsgType);
				MessageBox(GetTextByLan(_T("Ê§°Ü"), _T("failed")) + strChannel);
			}
		}
		break;
	case WCM_ALI_GETSECRET:
		{
			AliSecret tInfo = {0};
			memcpy(&tInfo, _iLParam, (int)sizeof(AliSecret));
			GetDlgItem(IDC_EDIT_PK)->SetWindowText(tInfo.pcPK);
		    GetDlgItem(IDC_EDIT_PS)->SetWindowText(tInfo.pcPS);
		    GetDlgItem(IDC_EDIT_DN)->SetWindowText(tInfo.pcDN);
		    GetDlgItem(IDC_EDIT_DS)->SetWindowText(tInfo.pcDS);
		}
		break;
	}
}
...
void CLS_AutoTestPage::OnBnClickedButtonSetali()
{
	// TODO: Add your control notification handler code here
	int iRet = RET_FAILED;
	AliSecret tInfo = {0};
	tInfo.iSize = (int)sizeof(AliSecret);

	CString strPk,strPs,strDn,strDs;
	GetDlgItem(IDC_EDIT_PK)->GetWindowText(strPk);
	memcpy_s(tInfo.pcPK, (int)sizeof(tInfo.pcPK), strPk, LEN_64);

	GetDlgItem(IDC_EDIT_PS)->GetWindowText(strPs);
	memcpy_s(tInfo.pcPS, (int)sizeof(tInfo.pcPS), strPs, LEN_64);

	GetDlgItem(IDC_EDIT_DN)->GetWindowText(strDn);
	memcpy_s(tInfo.pcDN, (int)sizeof(tInfo.pcDN), strDn, LEN_64);

	GetDlgItem(IDC_EDIT_DS)->GetWindowText(strDs);
	memcpy_s(tInfo.pcDS, (int)sizeof(tInfo.pcDS), strDs, LEN_64);

	iRet = NetClient_SendCommand(m_iLogonID, COMMAND_ID_SETSECRE, 0x7FFFFFFF, &tInfo, (int)sizeof(AliSecret));

	if (RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","CLS_AutoTestPage::NetClient_SendCommand[COMMAND_ID_SETSECRE] (%d, %d)", m_iLogonID, m_iChannelNO);
	}
	else
		AddLog(LOG_TYPE_FAIL, "", "[CLS_AutoTestPage::OnBnClickedButtonSetali]NetClient_SendCommand fail,error = %d", GetLastError());
}
