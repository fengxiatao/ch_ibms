...
#define MAX_EXTRA_SCHEDULE_SEGMENT_NUM			16
...
public:
	...
	void UI_UpdateAlarmScheduleExtra();
	void UI_UpdateAlarmEnable();
	void UI_UpdateText();
	void SetAlarmSchedule(int iWeek);
	void SetAlarmScheduleExtra(int iWeek);
	void UI_Clear();
	void UpdateEventRuleID();
	void CheckPTZTime();
public:
	...
	BOOL m_chkEnbale[4];
	//Additional Arming Template Information
	CDateTimeCtrl m_dtExtraBeginTime[MAX_EXTRA_SCHEDULE_SEGMENT_NUM];
	CDateTimeCtrl m_dtExtraEndTime[MAX_EXTRA_SCHEDULE_SEGMENT_NUM];
	CButton	m_chkExtraSegmentEnable[MAX_EXTRA_SCHEDULE_SEGMENT_NUM];
	...
	void UI_UpdataAlarmScheduleButton(int _iEnable);
	CComboBox m_cboVcaType;
	afx_msg void OnCbnSelchangeComboScheduleScene2();
};
