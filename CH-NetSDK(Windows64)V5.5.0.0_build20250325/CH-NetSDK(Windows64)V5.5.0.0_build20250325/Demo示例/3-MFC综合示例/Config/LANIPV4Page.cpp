...
void CLS_LANIPV4Page::OnBnClickedButtonIpv4()
{
	...
	int iRet = NetClient_SetLanParam(m_iLogonID, LAN_CMD_SET_IPV4, &m_lIPV4);
	if (0 == iRet)
	{			
...
void CLS_LANIPV4Page::DHCPEnable(BOOL bEnable)
{
	...
	GetDlgItem(IDC_BUTTON_IPV4)->EnableWindow(!bEnable);
}

void CLS_LANIPV4Page::OnBnClickedCheckDhcp()
{
	//设备在多址模式下，发DHCP与设备IP连续两条的时候，DHCP会造成设备网卡up/down，所以后面的收不到了
	//DHCP与IP协议分开发
	...
	iRet = NetClient_SetLanParam(m_iLogonID, LAN_CMD_SET_IPV4, &m_lIPV4);
	if (0 == iRet)
	{			
		AddLog(LOG_TYPE_SUCC,"","NetClient_SetLanParam(IPV4 %d)",m_iLogonID);
	}
...
void CLS_LANIPV4Page::OnParamChangeNotify(int _iLogonID, int _iChannelNo, int _iParaType,void* _pPara,int _iUserData)
{
