...
void CLS_CmosPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_CMOS_SMTPSVR_N, m_edtSmtpSvr_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_PORT_N, m_edtPort_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_ACCOUNT_N, m_edtAccount_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_PASSWORD_N, m_edtPassword_n);
	DDX_Control(pDX, IDC_EDIT_USERNAME_N, m_edtUsername);
	DDX_Control(pDX, IDC_EDIT_CMOS_SUJECT_N, m_edtMailSubject_n);
	DDX_Control(pDX, IDC_COMBO_CMOS_AUTHTYPE_N, m_cmbAuthtype_n);
	DDX_Control(pDX, IDC_CBO_SCRITY_TYPE_N, m_cboScrityType_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_MAILBOX_N, m_edtMailBox1_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_MAILBOX1_N, m_edtMailBox2_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_MAILBOX2_N, m_edtMailBox3_n);
	DDX_Control(pDX, IDC_EDIT_CMOS_MAILBOX3_N, m_edtMailBox4_n);
...
ON_BN_CLICKED(IDC_BUTTON_CMOS_SMTPSET_N, &CLS_CmosPage::OnBnClickedButtonCmosSmtpsetN)
END_MESSAGE_MAP()
...
BOOL CLS_CmosPage::OnInitDialog()
{
	...
	m_cmbAuthtype_n.AddString("off");
	m_cmbAuthtype_n.AddString("plain");
	m_cmbAuthtype_n.AddString("cram-md5");
	m_cmbAuthtype_n.AddString("digest-md5");
	m_cmbAuthtype_n.AddString("gssapi");
	m_cmbAuthtype_n.AddString("external");
	m_cmbAuthtype_n.AddString("login");
	m_cmbAuthtype_n.AddString("ntlm");
	m_cmbAuthtype_n.SetCurSel(0);
...
void CLS_CmosPage::UI_UpdateSmtpInfo()
{
	...
	SMTPInfoEx pNewSmtp = {0};
	int iBytesReturned = 0;
	iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_SMTPINFO_EX, m_iChannelNo, &pNewSmtp, sizeof(pNewSmtp), &iBytesReturned);
	if(0 == iRet)
	{
		SetDlgItemText(IDC_EDIT_CMOS_SMTPSVR_N, pNewSmtp.cSmtpIp);
		SetDlgItemInt(IDC_EDIT_CMOS_PORT_N, pNewSmtp.iSmtpPort);
		SetDlgItemText(IDC_EDIT_CMOS_ACCOUNT_N, pNewSmtp.cAccount);
		SetDlgItemText(IDC_EDIT_CMOS_PASSWORD_N, pNewSmtp.cPassword);
		m_cmbAuthtype_n.SetCurSel(pNewSmtp.iAuthtype);
		m_cboScrityType_n.SetCurSel(pNewSmtp.iScrityType-1);
		SetDlgItemText(IDC_EDIT_CMOS_SUJECT_N, pNewSmtp.cEmailSubject);
		SetDlgItemText(IDC_EDIT_USERNAME_N, pNewSmtp.cUsrName);
		SetDlgItemText(IDC_EDIT_CMOS_MAILBOX_N, pNewSmtp.cEmailDstAddr);
		SetDlgItemText(IDC_EDIT_CMOS_MAILBOX1_N, pNewSmtp.cEmailAddr[0]);
		SetDlgItemText(IDC_EDIT_CMOS_MAILBOX2_N, pNewSmtp.cEmailAddr[1]);
		SetDlgItemText(IDC_EDIT_CMOS_MAILBOX3_N, pNewSmtp.cEmailAddr[2]);
		AddLog(LOG_TYPE_SUCC, "", "NetClient_GetDevConfig(NET_CLIENT_SMTPINFO_EX) ID(%d)", m_iLogonID);
	}
	else if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "NetClient_GetDevConfig(NET_CLIENT_SMTPINFO_EX) ID(%d)", m_iLogonID);
	}
...
void CLS_CmosPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_STATIC_NEW, IDS_CONFIG_CMOS_MILEALARMINFO_NEW);
	...
	SetDlgItemTextEx(IDC_STATIC_CMOS_SMTPSVR_N, IDS_CONFIG_CMOS_SMTPSVR);
	SetDlgItemTextEx(IDC_STATIC_CMOS_PORT_N, IDS_CONFIG_CMOS_PORT);
	SetDlgItemTextEx(IDC_STATIC_CMOS_ACCOUNT_N, IDS_CONFIG_CMOS_ACCOUNT);
	SetDlgItemTextEx(IDC_STATIC_CMOS_PASSWORD_N, IDS_CONFIG_CMOS_PASSWORD);
	SetDlgItemTextEx(IDC_STATIC_USERNAME_N, IDS_CFG_USER_NAME);
	SetDlgItemTextEx(IDC_STATIC_CMOS_SUBJECT_NEW, IDS_CONFIG_CMOS_SUBJECT);
	SetDlgItemTextEx(IDC_STATIC_CMOS_AUTHTYPE_N, IDS_CONFIG_CMOS_AUTHTYPE);
	SetDlgItemTextEx(IDC_STC_SCRITY_TYPE_N, IDS_COMS_SCRITY_TYPE);
	SetDlgItemTextEx(IDC_STATIC_CMOS_MAILBOX, IDS_CONFIG_CMOS_MAILBOX);
	SetDlgItemTextEx(IDC_BUTTON_CMOS_SMTPSET_N, IDS_CONFIG_CMOS_SMTPSET);
	...
	InsertString(m_cboScrityType_n,0,IDS_CFG_COM_PARITY_NONE);
	InsertString(m_cboScrityType_n,1,"SSL");
	InsertString(m_cboScrityType_n,2,"TLS");
...
void CLS_CmosPage::OnBnClickedButtonCmosSmtpsetN()
{
	if (m_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG, "", "Invalid Logon id(%d)", m_iLogonID);
	...
	SMTPInfoEx pSmtp = {0};
	int iSmtpPort = GetDlgItemInt(IDC_EDIT_CMOS_PORT_N);
	if(iSmtpPort < 0 || iSmtpPort > 65535)
	{
		return;
	}
	pSmtp.iSmtpPort = iSmtpPort;
	pSmtp.iAuthtype = m_cmbAuthtype_n.GetCurSel();
	pSmtp.iScrityType = m_cboScrityType_n.GetCurSel() + 1;
	GetDlgItemText(IDC_EDIT_CMOS_SMTPSVR_N, pSmtp.cSmtpIp, 128);
	GetDlgItemText(IDC_EDIT_CMOS_ACCOUNT_N, pSmtp.cAccount, 128);
	GetDlgItemText(IDC_EDIT_CMOS_PASSWORD_N, pSmtp.cPassword, 128);
	GetDlgItemText(IDC_EDIT_CMOS_SUJECT_N, pSmtp.cEmailSubject, 128);
	GetDlgItemText(IDC_EDIT_CMOS_MAILBOX_N, pSmtp.cEmailDstAddr, 128);
	GetDlgItemText(IDC_EDIT_CMOS_MAILBOX1_N, pSmtp.cEmailAddr[0], 128);
	GetDlgItemText(IDC_EDIT_CMOS_MAILBOX2_N, pSmtp.cEmailAddr[1], 128);
	GetDlgItemText(IDC_EDIT_CMOS_MAILBOX3_N, pSmtp.cEmailAddr[2], 128);
	GetDlgItemText(IDC_EDIT_USERNAME_N, pSmtp.cUsrName, 64);

	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_SMTPINFO_EX, m_iChannelNO, &pSmtp, sizeof(pSmtp));
	if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "[NetClient_SetDevConfig]NET_CLIENT_SMTPINFO_EX fail!");
	}
	else
	{
		AddLog(LOG_TYPE_SUCC, "", "[NetClient_SetDevConfig]NET_CLIENT_SMTPINFO_EX success!");
	}
}
