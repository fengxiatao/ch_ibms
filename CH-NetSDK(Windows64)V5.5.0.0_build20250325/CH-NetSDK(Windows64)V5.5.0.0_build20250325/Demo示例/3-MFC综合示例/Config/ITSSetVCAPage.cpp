...
bool blExactFix = false; //Whether to manually operate precise positioning
int iFixPreset = -1; // Preset for precise positioning
int iFixArea = -1; // area for precise positioning
int iFixRect = -1; // quadrilateral in precisely positioned area
...
BOOL CLS_ITSSetVCAPage::UI_UpdateGraph()
{
	...
	//Clear all graphics on the video
	POINT pt[2] = {0};
	...
	for(int i = 0; i < MAX_AREA_COUNT; i++)
	{
		...
		//Roughly determine whether the quadrilateral is drawn
		...
		//has this quadrilateral
		else
		{
			AddOrDelPolygon(true); //Add a graphic
			...
			{
				//Roughly judge whether the rectangle is drawn
				if(0 == tITSIllegalParkParm.arrPTParam[j][0].y && 0 == tITSIllegalParkParm.arrPTParam[j][1].y)
				{
					//The rectangle is not drawn, continue to judge
					//trace("%d no rectangle%d\n",i,j);
				}
				else
				{
					AddOrDelPolygon(true); //Add a graphic
	...
	AddOrDelPolygon(true); //Protect the graphic from the next change
	return TRUE;
}
...
void CLS_ITSSetVCAPage::GetCoorEditShow( POINT *_ptPoint, int _iN0, int flag )
{
	...
	else if(1 == flag)
	{
		...
		{
		case 1:
			{    //Get the coordinates of two points of rectangular symmetry from the interface, after conversion, save the coordinates of four points
...
void CLS_ITSSetVCAPage::OnCbnSelchangeComboArea()
{
	...
	UI_UpdateGraph(); //Update the video graph display, remove the graphs that have not been set successfully
}
...
void CLS_ITSSetVCAPage::OnBnClickedButtonSet()
{
	...
	//-------20120604
	//Get the start and end time, and judge whether it is legal (the end time is greater than or equal to the start time)
	int iTimeRange = 0;
	...
	//Do not empty the content, because other time period data in the same area should be retained
	//memset(&tITSIllegalPark, 0, sizeof(ITS_IllegalPark));
	tITSIllegalPark.iAreaNo = m_cboArea.GetCurSel();
	tITSIllegalPark.iPresetNo = m_cboPreset.GetCurSel();
	//Get the original first, and then set the currently selected time period
	int iRet = NetClient_GetITSExtraInfo( m_iLogonID, ITS_ILLEGALPARK_CMD_GET,
		NULL, &tITSIllegalPark, sizeof(ITS_IllegalPark));

	tITSIllegalPark.iIllegalParkTime = GetDlgItemInt(IDC_EDIT_ILLEGALPARKTIME);
	tITSIllegalPark.iTimeRange[iCurSegment] = ((iRangeTimeStartH<<24) & 0xff000000) | //The start hour is placed in the 24~31 bits of the int type
		((iRangeTimeStartM<<16) & 0x00ff0000) | //The start minute is placed in the 16~23 bits of the int type
		((iRangeTimeEndH<<8) & 0x0000ff00) | //The end hour is placed in the 8~15 bits of the int type
		(iRangeTimeEndM&0x000000ff); //The end minute is placed in int type 0~7 bits
	GetCoorEditShow(tITSIllegalPark.arrPTArea, 5, 0); //Write the coordinate data on the interface to the tITSIllegalPark.arrPTArea array
...
{
	//Determine whether all data are selected Send positioning data
	if(iFixRect == -1)
	{
	...
	iFixPreset = -1; // Preset for precise positioning
	iFixArea = -1; // finely positioned area
	iFixRect = -1; // quadrilateral in precisely positioned area
...
void CLS_ITSSetVCAPage::OnLButtonUp( UINT nFlags, CPoint point )
{
	if (IsInsideVideoStatic(point))
	{
		...
		if(2 == m_iCurDrawGrh) //Draw the inner rectangle of the quadrilateral
		{
			m_iCurDrawGrh = 0;
			m_ptSet[1] = m_ptCur;
			m_bMouseDown = false;
			...
			{
				AddOrDelPolygon(false); //Delete the current graphic
			}
			AddOrDelPolygon(true); //Add a graphic for the next graphic drawing to protect the current graphic from being modified
		}
	}
...
void CLS_ITSSetVCAPage::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	if (IsInsideVideoStatic(point))
	{
		...
		m_bMouseDown = true;
		int iPos = 0;
		iPos = WhichAreaOfCurPt(); //Get where the current point is in the video
		...
		if(0 == iPos && 0 == GetDlgItemInt(IDC_EDIT_POINT4_Y))
		{
			...
			if(4 == m_iPointCount)
			{
				ChangePoint(); //Exchange the coordinates of the four points, so that the four-point connection becomes a quadrilateral without intersecting lines
				if(false == IfPolygonValidAndDraw(m_ptSet,4))//If the quadrilateral intersects with other quadrilaterals, the quadrilateral is not drawn successfully
				{
					POINT pt[4] = {0};
					SetCoorEditShow(pt, 5, 0);
					AddOrDelPolygon(false); //Delete the current graphic
				}
				//If this quadrilateral meets the requirements
				AddOrDelPolygon(true); //Add a graphic for the next graphic drawing to protect the current graphic from being modified
				m_iPointCount = 0;
				m_iCurDrawGrh = 0;
			}
		...
		{
			iFixPreset = m_cboPreset.GetCurSel(); // Preset for precise positioning
			iFixArea = iPos/10 -1; // pinpoint area
			iFixRect = iPos%10 -1; // quadrilateral in precisely positioned area
...
void CLS_ITSSetVCAPage::OnMouseMove( UINT nFlags, CPoint point )
{
	if (m_bMouseDown)
	{
		if (IsInsideVideoStatic(point))
		{
			m_ptCur = (POINT)point;
			PolygonOnViedo(&m_ptCur, 1);
			if(1 == m_iCurDrawGrh) //Draw the quadrilateral
			{
				m_ptSet[m_iPointCount] = m_ptCur;
				int iRet = NetClient_DrawPolyOnLocalVideo(m_uiConnID, m_ptSet, m_iPointCount + 1, 4);
			}
			else if(2 == m_iCurDrawGrh) //Draw the rectangle
			{
				m_ptSet[1] = m_ptCur;
				DrawRect(m_ptSet);
			}
...
//Determine whether the current rectangle intersects with other rectangles, and whether it is within the current quadrilateral, if required, draw this rectangle
bool CLS_ITSSetVCAPage::IfRectValidAndDraw( POINT *_ptRect )
{
	//Determine whether it is in the current quadrilateral, if not, return false, you don't need to draw a rectangle
	ITS_IllegalPark tITSIllegalPark;
	memset(&tITSIllegalPark, 0, sizeof(ITS_IllegalPark));
	...
	NetClient_GetITSExtraInfo( m_iLogonID, ITS_ILLEGALPARK_CMD_GET,
		NULL, &tITSIllegalPark, sizeof(ITS_IllegalPark));
	//1, roughly determine whether there is this quadrilateral
	if(0 == tITSIllegalPark.arrPTArea[3].y && 0 == tITSIllegalPark.arrPTArea[1].y)
	{
		//trace("%d has no such quadrilateral\n",i);
		return false;
	}
	//2, with this quadrilateral, determine whether the point is inside the quadrilateral
	else if(!IfPointInPolygon(_ptRect[0], tITSIllegalPark.arrPTArea, 4))
	{
		return false;
	}

	//Determine whether this rectangle intersects with the quadrilateral and other rectangles within the quadrilateral
	POINT ptCur[4] = {0};
	ptCur[0] = _ptRect[0];
	...
	//1, determine whether it intersects with the quadrilateral
	if(IfPolygonIntersect(ptCur, 4, tITSIllegalPark.arrPTArea, 4))
	{
		return false;
	}
	//2, determine whether it intersects with other rectangles
	POINT ptCmp[4] = {0};
	for (int i = 0; i < 4; i++)
	{
...
int CLS_ITSSetVCAPage::WhichAreaOfCurPt( void )
{
	...
	for(int i = 0; i < MAX_AREA_COUNT; i++)
	{
		...
		//Have this quadrilateral, determine whether the point is inside this quadrilateral
		else if(IfPointInPolygon(m_ptCur, tITSIllegalPark.arrPTArea))
		{
			//Inside this quadrilateral, determine whether it is in which rectangle
			ITS_IllegalParkParm tITSIllegalParkParm;
			memset(&tITSIllegalParkParm, 0, sizeof(ITS_IllegalParkParm));
			tITSIllegalParkParm.iAreaNo = i;
			tITSIllegalParkParm.iPresetNo = iPreset;
...
//Determine whether the current quadrilateral intersects with other existing shapes, none of them want to intersect, and the quadrilateral is drawn if it meets the requirements
bool CLS_ITSSetVCAPage::IfPolygonValidAndDraw( POINT *_ptPolygon, int _iPoly )
{
	...
	for(int i = 0; i < MAX_AREA_COUNT; i++)
	{
		...
		//Have this quadrilateral, determine whether the current quadrilateral intersects this quadrilateral
...
void CLS_ITSSetVCAPage::OnClose()
{
	...
	// clear the line drawing on the video
...
//// Judgment method Make a line from the current point and the (0, 0) point, and then judge whether there is an intersection with each edge of the polygon, and count the total number of intersections, even numbers are outside, odd numbers are inside
...
//}
//
////Algorithm to determine whether two polygons intersect, this algorithm is mainly to determine two quadrilaterals
