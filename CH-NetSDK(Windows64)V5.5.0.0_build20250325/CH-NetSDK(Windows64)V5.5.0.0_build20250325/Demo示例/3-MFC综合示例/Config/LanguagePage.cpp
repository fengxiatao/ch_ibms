...
void CLS_LanguagePage::OnParamChangeNotify(int _iLogonID, int _iChannelNo, int _iParaType, void* _pvPara, int _iUser)
{
	STR_Para* ptTmpPara;
	ptTmpPara = (STR_Para*) _pvPara;

	switch(_iParaType)
	{
	case PARA_DEV_UTCTIME_PARA:
		{
			if (NULL != ptTmpPara) {
				SetDlgItemInt(IDC_EDIT_BC_TIMEZONE, (int)ptTmpPara->m_iPara[0]);
				SetDlgItemInt(IDC_EDIT_BC_UTCTIME, (int)ptTmpPara->m_iPara[1]);
				SetDlgItemInt(IDC_EDIT_BC_TIMEMS, (int)ptTmpPara->m_iPara[2]);
			}
		}
		break;
	default:
...
void CLS_LanguagePage::UI_UpdateDialog()
{
	...
	SetDlgItemText(IDC_STATIC_BC_TIMEZONE, GetTextByLan("上报时区", "TimeZone"));
	SetDlgItemText(IDC_STATIC_BC_UTCTIME, GetTextByLan("UTC时间", "UtcTime"));
	SetDlgItemText(IDC_STATIC_BC_TIMEMS, GetTextByLan("时间毫秒", "Milliseconds"));
}
