...
BOOL CLS_AuthorityChannelPage::UI_UpdateAutOptionList()
{
	...
	m_cboRightOption.ResetContent();
	int iCurUser = m_cboUserChoise.GetCurSel();
	//What kind of user to get 1. Ordinary user 2 privileged user 3 super user 4 administrator
	char cUserName[128] = {0};
	char cPassword[128]	= {0};
...
void CLS_AuthorityChannelPage::OnBnClickedButtonSave()
{
	// TODO: Add your control notification handler code here
	int iCurOption = m_cboRightOption.GetCurSel();
	if (iCurOption < 0  || iCurOption >= CHANNEL_OPTION_NUM)
	{
		return;
	}
	for (int i=0; i<32; i++)
...
void CLS_AuthorityChannelPage::OnBnClickedButtonSet()
{
	...
	//Set the permissions of this permission group to the device regardless of whether they are selected or not, 
	//If the number of permissions that can be set exceeds 10, the TDModifyUserAuthority interface will be called multiple times according to the specific number of times
	int iUserAutSize = sizeof(USER_AUTHORITY);
	...
	if (iSendRound>1)
	{
		//If the number of permissions that can be set is more, select an integer multiple of 10, so that the size of the structure is the same each time it is set to the device.
		//And the set content is copied from strUserAut.strAutInfo to strTmpUA by a multiple of 10 and shifted by iSendRound.
		for (int i=0; i<iSendRound-1; i++)
		{
			strTmpUA.iNeedSize = sizeof(AUTHORITY_INFO)*10+4;
			memcpy(strTmpUA.strAutInfo, &strUserAut.strAutInfo[i*10], sizeof(AUTHORITY_INFO)*10);		//According to iSendRound, the starting position of each shift is 10*iSendRound
			int iRet = NetClient_ModifyUserAuthority(m_iLogonID, (LPSTR)(LPCSTR)strName, &strTmpUA, iUserAutSize);
		}
		//Finally, fill in the content of less than 10 permissions and set it to the device
		strTmpUA.iNeedSize = sizeof(AUTHORITY_INFO)*(iAuthorityIndex%10)+4;
		memcpy(strTmpUA.strAutInfo, &strUserAut.strAutInfo[(iSendRound-1)*10], sizeof(AUTHORITY_INFO)*(iAuthorityIndex%10));		//Less than 10 permissions last time, The starting position of the shift is iAuthorityIndex-10*iSendRound
		int iRet = NetClient_ModifyUserAuthority(m_iLogonID, (LPSTR)(LPCSTR)strName, &strTmpUA, iUserAutSize);
	}
	else if (iSendRound == 1)
	{
		//The number of permissions that can be set does not exceed 10 
