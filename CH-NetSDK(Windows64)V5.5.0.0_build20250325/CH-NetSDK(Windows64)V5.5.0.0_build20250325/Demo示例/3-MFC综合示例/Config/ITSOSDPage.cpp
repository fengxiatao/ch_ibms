...
#define MAX_ZOOM_SCALE		500 //Electronic amplification maximum scale 500
#define  FONTSIZE_DEFINE  6		//define character size
...
void CLS_ITSOSDPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_MERGE_PIC, m_cboMergePic);
	DDX_Control(pDX, IDC_CHECK_TWO_THREE, m_chkMergePicTT);
	DDX_Control(pDX, IDC_CHECK_THREE_FOUR, m_chkMergePicTF);
}
...
BEGIN_MESSAGE_MAP(CLS_ITSOSDPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BTN_FONTSIZESET, &CLS_ITSOSDPage::OnBnClickedBtnFontsizeset)
	ON_CBN_SELCHANGE(IDC_CBO_FONTSIZE, &CLS_ITSOSDPage::OnCbnSelchangeCboFontsize)
	ON_BN_CLICKED(IDC_BTN_MERGE_SET, &CLS_ITSOSDPage::OnBnClickedBtnMergeSet)
END_MESSAGE_MAP()
...
void CLS_ITSOSDPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_BTN_FONTSIZESET, IDS_ITS_FONTSET);
	SetDlgItemTextEx(IDC_STATIC_FONTSIZE_W, IDS_CONFIG_OSD_WIDTH);
	SetDlgItemTextEx(IDC_STATIC_FONTSIZE_H, IDS_CONFIG_OSD_HIGHT);
	SetDlgItemText(IDC_STATIC_PICTUREOVERLAY_ILLE, GetTextByLan("µþ¼ÓµÚ¼¸ÕÅµÄÎ¥·¨×¥ÅÄÊ±¼ä", "Superimpose the illegal capture time of the first few photos"));
	SetDlgItemText(IDC_BTN_MERGE_SET, GetTextByLan("ÉèÖÃ", "Set"));
	SetDlgItemText(IDC_CHECK_TWO_THREE, GetTextByLan("¶þÈýÕÅÖÐ¼äÊÇ·ñµþ¼Ó·ÀÎ±Âë", "Two or three sheets of security code superimposed in the middle"));
	SetDlgItemText(IDC_CHECK_THREE_FOUR, GetTextByLan("ÈýËÄÕÅÖÐ¼äÊÇ·ñµþ¼Ó·ÀÎ±Âë", "Three or four sheets of security code superimposed in the middle"));
	SetDlgItemText(IDC_STATIC_MERGE_PIC, GetTextByLan("ºÏ³ÉÍ¼Æ¬", "Merge Picture"));
	...
	//
	InsertString(m_cboFontSize,0,IDS_CONFIG_OSD_FONTSIZEADAPT);
	InsertString(m_cboFontSize,1,_T("1"));
	InsertString(m_cboFontSize,2,_T("2"));
	InsertString(m_cboFontSize,3,_T("3"));
	InsertString(m_cboFontSize,4,_T("4"));
	InsertString(m_cboFontSize,5,_T("5"));
	InsertString(m_cboFontSize,6,IDS_CONFIG_OSD_DEFINEFONTSIZE);
	m_cboFontSize.SetCurSel(1);

	InsertString(m_cboMergePic,0,_T("1"));
	InsertString(m_cboMergePic,1,_T("2"));
	InsertString(m_cboMergePic,2,_T("3"));
	m_cboMergePic.SetCurSel(0);
...
void CLS_ITSOSDPage::OnBnClickedBtnFontsizeset()
{
	if (m_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
	...
	if (6 == iSize)
	{
		...
		{
			AddLog(LOG_TYPE_FAIL, "","OnBnClickedBtnFontsizeset,Font size is error");
			return;
		}
		iSize = (iFonWid << 16) | iFonHig;
	}
	int iRet = NetClient_SetOSDTypeFontSize(m_iLogonID, m_iChannel, iOSDType, iSize);
	if(0 == iRet)
	{
...
void CLS_ITSOSDPage::OnCbnSelchangeCboFontsize()
{
...
BOOL  CLS_ITSOSDPage::UI_UpdateFontSize()
{
	...
	int iOSDType = OSDTYPE_ITS;
	int iSize = -1;
	int iRet = NetClient_GetOSDTypeFontSize(m_iLogonID, m_iChannel, iOSDType, &iSize);
	if (0 == iRet)
	{
		if(iSize <= 5)
		{
		...
		OnCbnSelchangeCboFontsize();

		AddLog(LOG_TYPE_SUCC,"","NetClient_GetOSDTypeFontSize(%d,%d,%d,%d)", m_iLogonID, m_iChannel, iOSDType, iSize);				
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetOSDTypeFontSize(%d,%d,%d,%d)", m_iLogonID, m_iChannel, iOSDType, iSize);				
	}
...
void CLS_ITSOSDPage::OnBnClickedCheckKeeptype()
{
 ...
 void CLS_ITSOSDPage::OnBnClickedBtnMergeSet()
 {
	 if (m_iLogonID < 0)
	 {
		 AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
	 ...
	 {
		 tItsPicMergeOverLay.iMergePicIllegalTime = m_cboMergePic.GetCurSel();
		 tItsPicMergeOverLay.iSecurityCodeLink = m_chkMergePicTT.GetCheck();
		 tItsPicMergeOverLay.iThreeMergeSeCodeEn = m_chkMergePicTF.GetCheck();
		 int iRet = NetClient_SetITSExtraInfo(m_iLogonID, ITS_EXTRAINFO_CMD_PICMERGEOVERLAY, m_iChannel, &tItsPicMergeOverLay, sizeof(tItsPicMergeOverLay));
		 if(iRet == 0)
		 {
			 AddLog(LOG_TYPE_SUCC,"","NetClient_SetITSExtraInfo[ITS_EXTRAINFO_CMD_PICMERGEOVERLAY](%d,%d)",m_iLogonID, m_iChannel);
 ...
 void CLS_ITSOSDPage::UI_UpdateMergePic()
 {
	 if (m_iLogonID < 0)
	 {
		 AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
	 ...
	 {
		 m_cboMergePic.SetCurSel(tItsPicMergeOverLay.iMergePicIllegalTime);
		 m_chkMergePicTT.SetCheck(tItsPicMergeOverLay.iSecurityCodeLink);
		 m_chkMergePicTF.SetCheck(tItsPicMergeOverLay.iThreeMergeSeCodeEn);
	 }
 }
