...
void CLS_UserPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_DUALUSERS_AUTHENTICATION, m_chkDualUsersAuthentication);
}
...
BEGIN_MESSAGE_MAP(CLS_UserPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BNT_LOGONFAIL_LOCKTIMESET, &CLS_UserPage::OnBnClickedBntLogonfailLocktimeset)
	ON_BN_CLICKED(IDC_CHECK_DUALUSERS_AUTHENTICATION, &CLS_UserPage::OnBnClickedCheckDualusersAuthentication)
END_MESSAGE_MAP()
...
void CLS_UserPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_STATIC_LOGONFAIL_LOCKTIME, IDS_CONFIG_USER_LOCKTIME);
	SetDlgItemTextEx(IDC_BNT_LOGONFAIL_LOCKTIMESET, IDS_SET);
	SetDlgItemTextEx(IDC_CHECK_DUALUSERS_AUTHENTICATION, IDS_DUALUSERS_AUTHENTICATION);
}
...
void CLS_UserPage::OnBnClickedBntLogonfailLocktimeset()
{
	int iLogonLockTime = GetDlgItemInt(IDC_EDIT_LOGONFAIL_LOCKTIME);

	UserLogonLockPara tParam = {0};
	tParam.iSize = sizeof(tParam);
	tParam.iType = USERLOGONLOCK_TYPE_MIN;
	tParam.iPara = iLogonLockTime*60;
	tParam.iEnable = TRUE;
	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_LOGONLOCK_PARA, m_iChannelNO,&tParam,sizeof(tParam));
	if (RET_SUCCESS != iRet)
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_SetDevConfig  NET_CLIENT_LOGONLOCK_PARA(%d,%d)",m_iLogonID,m_iChannelNO);
	}
}

void CLS_UserPage::OnBnClickedCheckDualusersAuthentication()
{
	int iEnable = m_chkDualUsersAuthentication.GetCheck();
	int iRet = NetClient_SetCommonEnable(m_iLogonID, CI_COMMON_ID_DUAL_USERS_AUTHENTICATION, m_iChannelNO, iEnable);
	if (RET_SUCCESS != iRet)
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_SetCommonEnable  CI_COMMON_ID_DUAL_USERS_AUTHENTICATION(m_iLogonID = %d, m_iChannelNO = %d, iEnable = %d)", m_iLogonID, m_iChannelNO, iEnable);
		return;
	}
	AddLog(LOG_TYPE_SUCC, "", "NetClient_SetCommonEnable cmd: CI_COMMON_ID_DUAL_USERS_AUTHENTICATION, m_iLogonID = %d, m_iChannelNO = %d, success", m_iLogonID, m_iChannelNO);
}

void CLS_UserPage::UI_UpdateDualUsersAuthEn()
{
	FuncAbilityLevel tFuncAbility = {0};
	tFuncAbility.iMainFuncType = 0x40;
	tFuncAbility.iSubFuncType = 151;//双用户认证能力集
	int iEnable = 0;
	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_GET_FUNC_ABILITY, m_iChannelNO, &tFuncAbility, sizeof(tFuncAbility), &iEnable);
	if (RET_SUCCESS == iRet)
	{
		if (RET_SUCCESS == iRet && 0 == strcmp(tFuncAbility.cParam, "1"))
		{
			m_chkDualUsersAuthentication.EnableWindow(TRUE);
		}
		else
		{
			m_chkDualUsersAuthentication.EnableWindow(FALSE);
		}
		AddLog(LOG_TYPE_SUCC, "", "NetClient_GetDevConfig cmd: NET_CLIENT_GET_FUNC_ABILITY, m_iLogonID = %d, iMainFuncType = %d, iSubFuncType = %d, success", m_iLogonID, tFuncAbility.iMainFuncType, tFuncAbility.iSubFuncType);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetDevConfig  NET_CLIENT_GET_FUNC_ABILITY(m_iLogonID = %d, m_iChannelNO = %d, iRet = %d.)", m_iLogonID, m_iChannelNO, iRet);
	}
	
	iRet = NetClient_GetCommonEnable(m_iLogonID, CI_COMMON_ID_DUAL_USERS_AUTHENTICATION, m_iChannelNO, &iEnable);
	if (RET_SUCCESS == iRet)
	{
		if (1 == iEnable)
		{
			m_chkDualUsersAuthentication.SetCheck(TRUE);
		}
		else
		{
			m_chkDualUsersAuthentication.SetCheck(FALSE);
		}
		AddLog(LOG_TYPE_SUCC, "", "NetClient_GetCommonEnable cmd: CI_COMMON_ID_DUAL_USERS_AUTHENTICATION, m_iLogonID = %d, success", m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetCommonEnable  CI_COMMON_ID_DUAL_USERS_AUTHENTICATION(m_iLogonID = %d, m_iChannelNO = %d, iRet = %d).", m_iLogonID, m_iChannelNO, iRet);
	}
}
