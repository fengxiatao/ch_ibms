...
#define SCAN_SPEED	30	//maximum scan speed
#define MAX_SCREW_STEP	15 //Helical scan, set the maximum value of the step 0-15
#define SCAN_LIST_PRESET_NUM	32	//The maximum number of preset positions in the cruise list

//Preset operation: 0-set, 1-delete, 2-call
#define PRESET_FOCUS_MODE_SET	0
#define PRESET_FOCUS_MODE_DEL	1
#define PRESET_FOCUS_MODE_CALL	2

//Scan Type 0-Left and Right Scan, 1-Auto Scan, 2-Frame Scan, 3-Random Scan, 4-Vertical Scan, 5-Panorama Scan
...
#define SCAN_TYPE_SCREW_SPEED	6 //Helical Sweep - Set Speed
#define SCAN_TYPE_SCREW_STEP	7//Helical scan - set step

//Scan operation: 0-left boundary (upper boundary), 1-right boundary (lower boundary), 2-call
...
void CLS_DomeRun1::UI_UpdateDialogText()
{
	...
	for (int i=0; i<500; i++)
	{
		//Special preset positions 65~99 are not added
		if (i>=64 && i<=98)
		{
			continue;
		}
...
{
	//Get the set scan type related options, and then initialize the control
	TDomeParam tparam = {0};
	...
	else
	{
		...
		//Type is helical scan speed setting
		if (tparam.iParam2 == SCAN_TYPE_SCREW_SPEED || tparam.iParam2 == SCAN_TYPE_SCREW_STEP)
		{
			...
			//iparam3 high 16-bit scan speed, low 16-bit vertical step speed
...
void CLS_DomeRun1::DevControl( int _iType, int _iPresetNo )
{
	...
	//First get the original protocol type
	int iComPort = 0;
	int iDevAddress = 0;
...
{
	//Start the Nth group cruise Call_80 + Call_N + Call_98
	//Stop the Nth group cruise Call_96
...
void CLS_DomeRun1::OnBnClickedButtonScanBrodrLeft()
{
	...
	SetDomeMenu(tParam);
	if (SCAN_TYPE_SCREW_SPEED == m_cboScanType.GetCurSel())
	{
		tParam.iParam2 = SCAN_TYPE_SCREW_STEP;
		tParam.iParam3 = (int)m_cboVerStep.GetItemData(m_cboVerStep.GetCurSel());
