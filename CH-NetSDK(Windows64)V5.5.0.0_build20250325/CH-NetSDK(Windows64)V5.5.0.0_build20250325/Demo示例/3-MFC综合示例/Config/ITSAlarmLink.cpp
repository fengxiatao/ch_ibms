...
void CLS_ITSAlarmLinkPage::UI_InitCboSubAlarmType()
{
	...
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextEx(IDS_ITS_MIX_CAP_SPEED)), 15);
	...
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextEx(IDS_ITS_RIDE_CAPTURE)), 18);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextEx(IDS_ITS_MIXED_ENABLE)), 19);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextEx(IDS_ITS_FASTENSB)), 20);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextEx(IDS_ITS_CALLING)), 21);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextByLan("外地车禁行","Out of town car cant passing")), 33);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextByLan("未佩戴头盔","No helmet")), 34);
	m_cboSubAlarmType.SetItemData(m_cboSubAlarmType.AddString(GetTextByLan("压导流线","Pressure diversion line")), 35);
...
void CLS_ITSAlarmLinkPage::OnBnClickedBtnItsIllegalDetectLinkSet()
{
	...
	//Set linkage recording Now only supports linkage to the current channel
	...
	//Set the linkage single screen Now only supports linkage to the current channel
	memset(iCommonSet, 0, MAX_BITSET_COUNT);
