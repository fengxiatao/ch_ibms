...
void CLS_DomeInfrared::UI_UpdateInfo()
{
	...
	//Get the current parameters of the photosensitive threshold
	...
	//Get the current parameter of the delay time
	...
	if (0 == iRet)
	{
		...
		if (CONTROL_MODE_TIME_OPEN == iIndex)
		{
			...
			CTime ctStartTime(stStartTime);        //Define a CTime object and assign to it
...
//Set the photosensitive threshold
