...
BOOL CLS_DonghuanSet::OnInitDialog()
{
	...
	//Initialization list information
...
void CLS_DonghuanSet::UI_UpdateDialogText()
{
	...
	SetDlgItemTextEx(IDC_STATIC_PE_INTERVAL, IDS_PE_INTERVAL);//Offline storage interval
	SetDlgItemTextEx(IDC_BUTTON_INTERVAL_TIME_SET, IDS_ITS_LIGHTSET);//Set
    SetDlgItemTextEx(IDC_STATIC_INTERVAL_TIME, IDS_PE_INTERVAL_TIME);//Offline storage interval (minutes)
	SetDlgItemTextEx(IDC_STC_ALARM_RECEIVE, IDS_ALARM_RECEIVE_SERVER);//Alarm receiving server
	SetDlgItemTextEx(IDC_STC_ALARM_RECEIVE_IP, IDS_CONFIG_LAN_IP);//IP
	SetDlgItemTextEx(IDC_STC_ALARM_RECEIVE_PORT, IDS_CONFIG_IP_REGISTER_REGPORT1);//Port number
	SetDlgItemTextEx(IDC_BTN_ALARM_RECEIVE_SAVE, IDS_LOG_SAVE);//Save
...
{
	//Update interface data information
	if (NetClient_GetAlarmPortNum)
	{
		...
		//Update Offline Storage Interval
...
void CLS_DonghuanSet::UpdateListInfo(int _iAlarmINo)
{
	if (NetClient_GetAlarmConfig)
	{
		...
		case ALARM_INTERFACE_TYPE_SWITCH:
			{
				//Update the analog device list node, there is no addition
				UpdateItemInfo(_iAlarmINo, &pAlarmInfo, iEnable);
				break;
			}
		case ALARM_INTERFACE_TYPE_ANALOG:
			{
				//Update serial device list information
				UpdateItemInfo(_iAlarmINo, &pAlarmInfo, iEnable);
				break;
			}
		case ALARM_INTERFACE_TYPE_COM:
			{
				if (iEnable)
				{
					//Update the serial device list node, there is no addition
					UpdateItemInfo(_iAlarmINo, &pAlarmInfo, iEnable);
				...
				{
					//delete serial device list node
...
void CLS_DonghuanSet::UpdateItemInfo(int _iChlNo, AlarmInConfig* _pAlarmInfo, int _iEnable)
{
	...
	for (int i=0; i<iSize; i++)
	{
		...
		{
			//Find the entry in the list to update
			blFind = true;
			...
			m_lstPowerDevInfo.SetItemText(i,1,cstrDevName);//device name
			GetDevUpdateInfo(_pAlarmInfo, _iEnable, strDevName, strSchedule, strEnable);
			m_lstPowerDevInfo.SetItemText(i,2,strDevName.c_str());//interface name
			if (_pAlarmInfo->iInterfaceType == ALARM_INTERFACE_TYPE_SWITCH)
			{
				m_lstPowerDevInfo.SetItemText(iSize,3,"--");		//Equipment type
				cstrOperInfo.LoadString(IDS_POWER_DEV_LST_EDIT);	//edit
			}
			else if( _pAlarmInfo->iInterfaceType == ALARM_INTERFACE_TYPE_ANALOG)
			{
				m_lstPowerDevInfo.SetItemText(iSize,3,"--");//Equipment type
				if (_iEnable == 1)
				{
					cstrOperInfo.LoadString(IDS_POWER_DEV_LST_FORBID);	//disabled
				}
				else
				{
					cstrOperInfo.LoadString(IDS_POWER_DEV_LST_ENABLE);	//enable
				}
			}
			else
			{
				...
				m_lstPowerDevInfo.SetItemText(i,3,strComName.c_str());//Equipment type

				cstrOperInfo.LoadString(IDS_POWER_DEV_LST_DELETE);
			}	
			m_lstPowerDevInfo.SetItemText(i,4,strSchedule.c_str());	//Arming Template	
			m_lstPowerDevInfo.SetItemText(i,5,GetText(IDS_POWER_DEV_LST_EDIT));
			m_lstPowerDevInfo.SetItemText(i,6,cstrOperInfo);		//operate	
			break;
		}
	}
	...
	if (!blFind)
	{
		...
		m_lstPowerDevInfo.InsertItem(iSize,cstrNo);//aisle
		CString cstrDevName = _pAlarmInfo->cName;
		if (strcmp(_pAlarmInfo->cName, "") == 0)
		{
			if (_pAlarmInfo->iInterfaceType == ALARM_INTERFACE_TYPE_SWITCH)
		...
		m_lstPowerDevInfo.SetItemText(iSize,1,cstrDevName);//device name
		GetDevUpdateInfo(_pAlarmInfo, _iEnable, strDevName, strSchedule, strEnable);
		m_lstPowerDevInfo.SetItemText(iSize,2,strDevName.c_str());//interface name
		if (_pAlarmInfo->iInterfaceType == ALARM_INTERFACE_TYPE_SWITCH)
		{
			m_lstPowerDevInfo.SetItemText(iSize,3,"--");		//Equipment type
			cstrOperInfo.LoadString(IDS_POWER_DEV_LST_EDIT);	//edit
		...
		else if( _pAlarmInfo->iInterfaceType == ALARM_INTERFACE_TYPE_ANALOG)
		{
			...
			m_lstPowerDevInfo.SetItemText(iSize,6,cstrOperInfo);		//operate	
		}
		else
		{
			GetComTypeLastName(_pAlarmInfo->iInterfaceNo,strComName);
			...
			m_lstPowerDevInfo.SetItemText(iSize,3,strComName.c_str());	//Equipment type

			cstrOperInfo.LoadString(IDS_POWER_DEV_LST_DELETE);
		}
		m_lstPowerDevInfo.SetItemText(iSize,4,strSchedule.c_str());		//Arming Template
...
void CLS_DonghuanSet::DeleteComItem(int _iChlNo, AlarmInConfig* _pAlarmInfo)
{
	...
	for (int i=0; i<iSize; i++)
	{
		...
		if (_iChlNo + 1 == atoi(cstrNo))
		{
			//Find the entry in the list to delete
			m_lstPowerDevInfo.DeleteItem(i);

			//Remove the entry number from the container
...
{
	// TODO: Add your control notification handler code here
	//Set the offline storage interval
...
void CLS_DonghuanSet::OnNMClickListDonghuanList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if(nItem == -1) //Return if there is no data in this row
	{
		return;
	}
	...
	if (5 == nSub)
	{
		m_lstPowerDevInfo.GetItemText(nItem,2,(LPTSTR)cDevName,3);//interface name
		if (strcmp(cDevName,"Ä£") == 0)
		{
...
void CLS_DonghuanSet::OperateInfo(int _iPos)
{
	...
	else if (iItemNo >= 16 && iItemNo < 32) //analog
	{
		if (_iPos == 0)
		{
			//edit
	...
	else if (iItemNo > 32)//Temperature and humidity
	{
		if (_iPos == 0)
		{
			//edit
...
END:
	...
	{
		iEnable = 0;
		cstrEnable.LoadString(IDS_POWER_DEV_LST_ENABLE);	//enable
	}
	else
	{
		iEnable = 1;
		cstrEnable.LoadString(IDS_POWER_DEV_LST_FORBID);	//disable
	}
...
void CLS_DonghuanSet::OnBnClickedBtnAlarmReceiveSave()
{
	...
	srcAddAlarmHost.iSize = sizeof(AddAlarmHost);
	srcAddAlarmHost.iHostIndex = HOSTINDEX;
	srcAddAlarmHost.iEnable = 1;   //enable
	CString cstrIPAddr;
