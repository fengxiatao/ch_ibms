// Cls_ItsPictureCommon2.cpp : Implementation file
//
...
// Cls_ItsPictureCommon2 message handler
...
void Cls_ItsPictureCommon2::CheckEnable()
{
	...
	// Camera version 7.0 (excluding starlight camera)
	...
	GetDlgItem(IDC_STA_LIGHT_EXPECT)->EnableWindow(blState); //Snapshot parameters--brightness expectation
	GetDlgItem(IDC_SLD_LIGHT_EXPECT)->EnableWindow(blState);
	GetDlgItem(IDC_STA_LIGHT_EXPECT_VALUE)->EnableWindow(blState);
	GetDlgItem(IDC_STA_EXPO_UPPER_LIMIT)->EnableWindow(blState); //Snapshot parameter--exposure upper limit
	GetDlgItem(IDC_SLD_EXPO_UPPER_LIMIT)->EnableWindow(blState);
	GetDlgItem(IDC_STA_EXPO_UPPER_LIMIT_VALUE)->EnableWindow(blState);
	GetDlgItem(IDC_STA_GAIN_UPPER_LIMIT)->EnableWindow(blState); //Snapshot parameter--Gain upper limit
...
void Cls_ItsPictureCommon2::OnNMCustomdrawSldLightExpect(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	int iTimeRangeIndex = GetDlgItemInt(IDC_COMBO_TIMERANGEINDEX2);//Get the currently selected time period
	...
	//strctTimeRangeParam.iParam1[12] = ((CScrollBar*)GetDlgItem(IDC_SLD_LIGHT_EXPECT))->GetScrollPos();//Snapshot parameters--brightness expectations
...
void Cls_ItsPictureCommon2::OnNMCustomdrawSldExpoUpperLimit(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	//strctTimeRangeParam.iParam2[3] = ((CScrollBar*)GetDlgItem(IDC_SLD_EXPO_UPPER_LIMIT))->GetScrollPos();//Snapshot parameters--exposure upper limit
...
void Cls_ItsPictureCommon2::OnNMCustomdrawSldGainUpperLimit(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	//strctTimeRangeParam.iParam2[5] = ((CScrollBar*)GetDlgItem(IDC_SLD_GAIN_UPPER_LIMIT))->GetScrollPos();//Snapshot parameter--Gain upper limit
...
void Cls_ItsPictureCommon2::UI_UpdateCap()
{
	...
	//((CScrollBar*)GetDlgItem(IDC_SLD_LIGHT_EXPECT))->SetScrollPos(strctTimeRangeParam.iParam1[12]);//Snapshot parameters--brightness expectation
	...
	GetDlgItem(IDC_STA_LIGHT_EXPECT_VALUE)->SetWindowText(cstrCaptureLightExpect);//Brightness expectation


	//((CScrollBar*)GetDlgItem(IDC_SLD_EXPO_UPPER_LIMIT))->SetScrollPos(strctTimeRangeParam.iParam2[3]);//Snapshot parameters--exposure upper limit
	...
	GetDlgItem(IDC_STA_EXPO_UPPER_LIMIT_VALUE)->SetWindowText(cstrCaptureExposalUpLimit);//Exposure upper limit

	//((CScrollBar*)GetDlgItem(IDC_SLD_GAIN_UPPER_LIMIT))->SetScrollPos(strctTimeRangeParam.iParam2[5]);//Snapshot parameter--Gain upper limit
	...
	GetDlgItem(IDC_STA_GAIN_UPPER_LIMIT_VALUE)->SetWindowText(cstrCaptureGainUpLimit);//Gain upper limit
}
