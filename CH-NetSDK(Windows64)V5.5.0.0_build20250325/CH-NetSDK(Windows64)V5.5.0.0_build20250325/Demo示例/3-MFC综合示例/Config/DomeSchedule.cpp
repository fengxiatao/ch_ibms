...
static int g_iEditID2[] = {IDC_EDIT_WORK_VALUE0, IDC_EDIT_WORK_VALUE1, IDC_EDIT_WORK_VALUE2
, IDC_EDIT_WORK_VALUE3, IDC_EDIT_WORK_VALUE4, IDC_EDIT_WORK_VALUE5, IDC_EDIT_WORK_VALUE6
, IDC_EDIT_WORK_VALUE7};
...
void CLS_DomeSchedule::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE0, m_edtWorkValue2[0]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE1, m_edtWorkValue2[1]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE2, m_edtWorkValue2[2]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE3, m_edtWorkValue2[3]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE4, m_edtWorkValue2[4]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE5, m_edtWorkValue2[5]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE6, m_edtWorkValue2[6]);
	DDX_Control(pDX, IDC_EDIT_WORK_VALUE7, m_edtWorkValue2[7]);
}
...
void CLS_DomeSchedule::UpdatePageUI()
{
	...
	DomeWorkSchedule.iCurParaIndex = 1;
	...
	if (DomeWorkSchedule.iTotalParaCnt < 2) {
		return;
	}

	iWeekDay = m_cboWeek.GetCurSel();
	TDomeWorkSchedule domeWorkSchedule = {0};
	iBytesReturned = 0;
	domeWorkSchedule.iWeekDay = iWeekDay;
	domeWorkSchedule.iCurParaIndex = 2;
	iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_DOME_WORK_SCHEDULE, m_iChannelNo, &domeWorkSchedule, sizeof(TDomeWorkSchedule), &iBytesReturned);
	if (iRet < 0 )
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetDevConfig[NET_CLIENT_DOME_WORK_SCHEDULE] (%d, %d), error(%d)", m_iLogonID, m_iChannelNo, GetLastError());
		return;
	...
	for (int i = 0; i < MAX_SCHEDULE; i++)
	{
		...
		m_chkEnbale[i].SetCheck(domeWorkSchedule.tWork[i].iEnable);
		m_cboSegType[i].SetCurSel(domeWorkSchedule.tWork[i].iWorkType);
		SetDlgItemInt(g_iEditID2[i], domeWorkSchedule.tWork[i].iWorkValue);
	}
}
...
void CLS_DomeSchedule::UI_UpdateDialog()
{
	...
	for (int i = 0; i < MAX_SCHEDULE; i++)
	{
		...
		InsertString(m_cboSegType[i], 5, GetTextByLan(_T("¸¨ÖúÊä³ö"), _T("Auxiliary output")));
		InsertString(m_cboSegType[i], 6, GetTextByLan(_T("µÍ¹¦ºÄ"), _T("Low power consumption")));
	}
...
void CLS_DomeSchedule::OnBnClickedButtonDomeScheduleSet()
{
	...
	DomeWorkSchedule.iTotalParaCnt = 2;
	DomeWorkSchedule.iCurParaIndex = 1;
	...
	if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "1-NetClient_SetDevConfig[NET_CLIENT_DOME_WORK_SCHEDULE] (%d, %d), error(%d)", m_iLogonID, m_iChannelNo, GetLastError());
	}
	else
	{
		AddLog(LOG_TYPE_SUCC, "", "1-NetClient_SetDevConfig[NET_CLIENT_DOME_WORK_SCHEDULE] (%d, %d)", m_iLogonID, m_iChannelNo);
	}
	...
	for (int i= 0; i < MAX_SCHEDULE; i++)
	{
		...
		domeWorkSchedule.tWork[i].iEnable = m_chkEnbale[i].GetCheck();
		domeWorkSchedule.tWork[i].iWorkType = m_cboSegType[i].GetCurSel();
		domeWorkSchedule.tWork[i].iWorkValue = GetDlgItemInt(g_iEditID2[i]); 

	}
	domeWorkSchedule.iTotalParaCnt = 2;
	domeWorkSchedule.iCurParaIndex = 2;
	iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_DOME_WORK_SCHEDULE, m_iChannelNo, &domeWorkSchedule, sizeof(TDomeWorkSchedule));
	if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "2-NetClient_SetDevConfig[NET_CLIENT_DOME_WORK_SCHEDULE] (%d, %d), error(%d)", m_iLogonID, m_iChannelNo, GetLastError());
	}
	else
	{
		AddLog(LOG_TYPE_SUCC, "", "2-NetClient_SetDevConfig[NET_CLIENT_DOME_WORK_SCHEDULE] (%d, %d)", m_iLogonID, m_iChannelNo);
	}
