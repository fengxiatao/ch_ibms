// DNVRChannelInfo.cpp : Implementation file
//
...
void CLS_DNVRChannelInfo::UI_UpdateConnectState(int _iChannel)
{
	...
	ChnOnlineState tOnlineStatus = {0};
	int iRetBytes = -1;

	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_ASYN_CHNONLINESTATE, _iChannel, &tOnlineStatus, sizeof(ChnOnlineState), &iRetBytes);
	if( RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","NetClient_GetDevConfig[CHN_CONNECT_STATE] (%d,%d,%d)", m_iLogonID, _iChannel, tOnlineStatus.iState);
		if (-1 == tOnlineStatus.iState)
		{
			strpcChannelState = GetTextEx(IDS_UNKNOW_CONNECT);
		}
		else if (1 == tOnlineStatus.iState)
		{
