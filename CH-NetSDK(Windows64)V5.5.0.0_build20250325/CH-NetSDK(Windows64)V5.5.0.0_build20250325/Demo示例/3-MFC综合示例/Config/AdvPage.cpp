...
BEGIN_MESSAGE_MAP(CLS_AdvPage, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_GETDEVREALTIMEPARAM, &CLS_AdvPage::OnBnClickedButtonGetdevrealtimeparam)
END_MESSAGE_MAP()
...
void CLS_AdvPage::UI_UpdateLanguage()
{
	...
	SetDlgItemText(IDC_STATIC_GVOLTAGE, GetTextByLan("设备实时参数","Dev Real Time Param"));
	SetDlgItemText(IDC_STATIC_VOLTAGE, GetTextByLan("电压","Voltage"));
	SetDlgItemText(IDC_BUTTON_GETDEVREALTIMEPARAM, GetTextByLan("获取","GET"));
...
void CLS_AdvPage::OnBnClickedButtonGetdevrealtimeparam()
{
	// TODO: Add your control notification handler code here
	int iBytesReturned = 0;
	DevRealTimeParam tDevRealTimeParam = {0};

	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_DEVREALTIMEPARAM, m_iChannelNo, &tDevRealTimeParam, sizeof(tDevRealTimeParam), &iBytesReturned);
	if (iRet < 0)
	{
		AddLog(LOG_TYPE_FAIL, "", "NetClient_GetDevConfig faied! Logon id(%d)", m_iLogonID);
	}
	...
	{
		CString str;
		str.Format("%.2f",tDevRealTimeParam.iVoltageValue/(float)10);
