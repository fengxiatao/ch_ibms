...
void CLS_LinkBSF::InitDlgData()
{
	...
	//operate when the alarm is cleared
...
void CLS_LinkBSF::SetDlgInfo(char* _cParam,int _iSize)
{
	...
	{
		int iRow = 0;
		m_lstLinkDevice.SetItemText(iItem, iRow++, "Number of numbers");
		m_lstLinkDevice.SetItemText(iItem, iRow++, "Link device name");
...
//	CLS_DlgLinkBase::SetData(_pAlarmInLink);
	...
	for (int i = 0; i < m_cboAlarmClear.GetCount(); i++)
	{
		...
		{
			m_cboAlarmClear.SetCurSel(i); //Recovery type
		}
	}
...
void CLS_LinkBSF::SetAlarmTypeFlag(bool _bAlarmTypeFlag)
{
	...
	int iItemComb = m_cboAlarmClear.AddString(GetText(IDS_RECOVER_TYPE_NONE));//No recovery
	m_cboAlarmClear.SetItemData(iItemComb, DH_ALARM_DISAPPEAR_TYPE_NOT);
	iItemComb = m_cboAlarmClear.AddString(GetText(IDS_RECOVER_TYPE_DELAY));//Delayed recovery
	m_cboAlarmClear.SetItemData(iItemComb, DH_ALARM_DISAPPEAR_TYPE_DELAY);
	if (!_bAlarmTypeFlag)
	{
		iItemComb = m_cboAlarmClear.AddString(GetText(IDS_RECOVER_TYPE_DISALARM));//Alarm recovery
		m_cboAlarmClear.SetItemData(iItemComb, DH_ALARM_DISAPPEAR_TYPE_DIS_RECOVERY);
		iItemComb = m_cboAlarmClear.AddString(GetText(IDS_RECOVER_TYPE_DISALARM_DELAY));//Alarm delay recovery
