...
void CLS_3GAdvPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_3GADV_APN, m_edtAPN);
	DDX_Control(pDX, IDC_COMBO_ADV_AUTHTYPE, m_cboAuthiType);
}
...
void CLS_3GAdvPage::OnBnClickedButton3GADVVpdnSet()
{
	...
	if(NetClient_Set3GConfig)
	{
		...
		TDVR3GVPDN t3GVPN = {0};
		t3GVPN.iSize = sizeof(TDVR3GVPDN);
		...
		m_edtAPN.GetWindowText(strAPN);
		strncpy_s(t3GVPN.cDialNumber,strDialNumber.GetBuffer(0),strDialNumber.GetLength());
		strncpy_s(t3GVPN.cAccount,strAccounnter.GetBuffer(0),strAccounnter.GetLength());
		strncpy_s(t3GVPN.cPassword,strPassword.GetBuffer(0),strPassword.GetLength());
		strncpy_s(t3GVPN.cAccessNumber,strAPN.GetBuffer(0),strAPN.GetLength());
		
		
		t3GVPN.i3GAuthenticationType = m_cboAuthiType.GetCurSel();

		int iRet = NetClient_Set3GConfig(m_iLogonID,DVR3G_CMD_SET_VPDN,-1,&t3GVPN,t3GVPN.iSize);
		if (iRet < 0)
		{
			AddLog(LOG_TYPE_FAIL,"","NetClient_Set3GConfig  DVR3G_CMD_GET_VPDN Error!LogonID(%d), Error(0x%08x)",m_iLogonID,::GetLastError());
		} 
		else
		{
			AddLog(LOG_TYPE_SUCC,"","NetClient_Set3GConfig  DVR3G_CMD_GET_VPDN Success!LogonID(%d)",m_iLogonID);
		}
...
BOOL CLS_3GAdvPage::OnInitDialog()
{
	...
	m_cboAuthiType.Clear();
	m_cboAuthiType.AddString(_T("Auto"));
	m_cboAuthiType.AddString(_T("CHAP"));
	m_cboAuthiType.AddString(_T("PAP"));
...
void CLS_3GAdvPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_STATIC_3GADV_APN,IDS_CFG_3G_ADV_APN );
	SetDlgItemTextEx(IDC_STATIC_3GADV_AuthenticationType, IDS_CFG_3G_ADV_AUTHTYPE);
}
...
void CLS_3GAdvPage::UI_Update3GADVInfo()
{
	...
	TDVR3GVPDN t3GVPN = {0};
	t3GVPN.iSize = sizeof(t3GVPN);
	if(NetClient_Get3GConfig && NetClient_Get3GConfig(m_iLogonID,DVR3G_CMD_GET_VPDN, -1, &t3GVPN, sizeof(t3GVPN)) == 0)
	{
		m_edtDialNumber.SetWindowText(t3GVPN.cDialNumber);
		m_edtAccounter.SetWindowText(t3GVPN.cAccount);
		m_edtPassword.SetWindowText(t3GVPN.cPassword);
		m_edtAPN.SetWindowText(t3GVPN.cAccessNumber);
		m_cboAuthiType.SetCurSel(t3GVPN.i3GAuthenticationType);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_Get3GConfig Error!LogonID(%d), Error(0x%08x)",m_iLogonID,::GetLastError());
	}
