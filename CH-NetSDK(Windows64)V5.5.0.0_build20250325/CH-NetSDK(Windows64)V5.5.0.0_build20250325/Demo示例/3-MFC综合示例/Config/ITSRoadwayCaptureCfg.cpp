...
void CLS_DLG_ITSRoadwayCaptureCfg::DoDataExchange(CDataExchange* pDX)
{
	...
	for (int i = 0; i < MAX_ILLEGAL_TYPE_V2; ++i)
	{
		DDX_Control(pDX, (IDC_CHK_ILLEGAL_TYPE0_V2 + i), m_chkIllegalTypeV2[i]);
	}
...
void CLS_DLG_ITSRoadwayCaptureCfg::UI_UpdateDialog()
{
	...
	SetDlgItemText(IDC_CHK_ILLEGAL_TYPE0_V2,GetTextByLan("外地车禁行","Out of town car cant passing"));
	SetDlgItemText(IDC_CHK_ILLEGAL_TYPE1_V2,GetTextByLan("未佩戴头盔","No helmet"));
	SetDlgItemText(IDC_CHK_ILLEGAL_TYPE2_V2,GetTextByLan("压导流线","Pressure diversion line"));
...
int CLS_DLG_ITSRoadwayCaptureCfg::UI_UpdateRelateTimeRangePara()
{
	...
	UI_UpdateCapTypeV2Widget(tITSChnlCapSet.iCapTypeV2);
	iRet = RET_SUCCESS;
...
int CLS_DLG_ITSRoadwayCaptureCfg::SetCapRelateTimeRange()
{
	...
	tITSChnlCapSet.iCapTypeV2 = GetCapTypeV2WidgetValue();
...
int CLS_DLG_ITSRoadwayCaptureCfg::UI_UpdateRelateCarTypePara()
{
	...
	UI_UpdateCapTypeV2Widget(tITSChnlCarCapType.iCapTypeV2);
	iRet = RET_SUCCESS;
...
int CLS_DLG_ITSRoadwayCaptureCfg::SetCapRelateCarType()
{
	...
	tITSChnlCarCapType.iCapTypeV2 = GetCapTypeV2WidgetValue();
...
int CLS_DLG_ITSRoadwayCaptureCfg::GetTimeWidgetValue()
{
	...
	int iTime = ((iStartHour << BIT_TIME_STARTHOUR) & VALUE_TIME_STARTHOUR) |   	//The starting hour is placed in the 24~31 bits of the int type
	((iStartMin << BIT_TIME_STARTMIN) & VALUE_TIME_STARTMIN) |    					//The starting minute is placed in the 16~23 bits of the int type
	((iEndHour << BIT_TIME_ENDHOUR) & VALUE_TIME_ENDHOUR) |      					//The end hour is placed in the 8~15 digits of the int type
...
void CLS_DLG_ITSRoadwayCaptureCfg::UI_UpdateCapTypeV2Widget(int _iIllegalType)
{
	for (int i = 0; i < MAX_ILLEGAL_TYPE_V2; ++i)
	{
		m_chkIllegalTypeV2[i].SetCheck(FALSE);
		BOOL blChk = _iIllegalType & (1 << i);
		m_chkIllegalTypeV2[i].SetCheck(blChk);
	}
}
...
int CLS_DLG_ITSRoadwayCaptureCfg::GetCapTypeV2WidgetValue()
{
	int iIllegalType = 0;

	for (int i = 0; i < MAX_ILLEGAL_TYPE_V2; ++i)
	{
		iIllegalType += ((int)(m_chkIllegalTypeV2[i].GetCheck())) << i;
	}
