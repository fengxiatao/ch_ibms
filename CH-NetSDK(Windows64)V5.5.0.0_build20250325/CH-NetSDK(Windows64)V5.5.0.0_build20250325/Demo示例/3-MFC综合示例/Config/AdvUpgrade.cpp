...
//Advanced options parameter configuration export	
...
void CLS_AdvUpgrade::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_FILELIST, m_edtFileList);
}
...
void CLS_AdvUpgrade::OnMainNotify( int _iLogonID,int _wParam, void* _iLParam, void* _iUser)
{
	...
	case WCM_ERR_ORDER:
		...
		break;
	case WCM_EXPORT_CONFIG_FINISHED:
		{
			MessageBox(GetTextByLan(_T("导出成功"),_T("Export Success")));
		}
		break;
	}
}
...
void CLS_AdvUpgrade::OnBnClickedButtonAdvUpgradeBin()
{
	...
	if (m_szKernelFile != "")
	{
		int iMBRet = MessageBox("Turn off the intercom", _T(""), MB_OKCANCEL);
		if (iMBRet == IDOK)
		{
			StopTalk();
...
{
	// TODO: Add your control notification handler code here
	CFileDialog fileDlg(TRUE, _T(".bin"), NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, _T("bin file(*.bin)|*.bin|all files(*.*)|*.*||"));
...
{
	// TODO: Add your control notification handler code here
	CFileDialog fileDlg(TRUE, _T(".box"), NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, _T("(*.box)|*.box|All files(*.*)|*.*||"));
	if (IDOK == fileDlg.DoModal())
	{
...
void CLS_AdvUpgrade::OnBnClickedButtonConfigOutput()
{
	...
	strFileName = strFileName + ".box";                                 //File name of exported data
	...
	if (strSystemOutputPath != "")
	{
		...
		CString strFileList="";
		m_edtFileList.GetWindowText(strFileList);
		int iCount = 0;
		if (strFileList == "")
		{
			AddLog(LOG_TYPE_FAIL, "", "FileList Invalid!");
			return;
		}
		int iIndex = 0;
		while(strFileList.GetLength() > 0)
		{
			if (strFileList.Find(",") == -1)
			{
				if (iCount >= FILE_COUNT)
				{
					strcpy_s(stExportConfig.cFileListEx[iCount - FILE_COUNT], strFileList);
				}
				else
				{
					strcpy_s(stExportConfig.cFileList[iCount], strFileList);
				}
				iCount++;
				break;
			}
			...
			{
				CString strFileNameIndex = strFileList.Left(strFileList.Find(","));
				if (iCount >= FILE_COUNT)
				{
					strcpy_s(stExportConfig.cFileListEx[iCount - FILE_COUNT], strFileNameIndex);
				}
				else
				{
					strcpy_s(stExportConfig.cFileList[iCount], strFileNameIndex);
				}
				iCount++;
				strFileList = strFileList.Mid(strFileList.Find(",") + 1);
			}
		}
...
CString CLS_AdvUpgrade::GetSystemOutputPath()
{
	...
	{   
		...
		bi.lpfn      = OutputPathCallbackProc;  //Callback
