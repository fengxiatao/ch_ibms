...
void CLS_DomeHDParam1::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_MINI_SHUTTER, m_cboMiniShutter);
...
BEGIN_MESSAGE_MAP(CLS_DomeHDParam1, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_MINI_SHUTTER, &CLS_DomeHDParam1::OnCbnSelchangeComboMiniShutter)
END_MESSAGE_MAP()
		...
		case PARA_ITS_TIMERANGEPARAM:
			{
				...
				if (iType < 0 || iType >= MAX_TIME_RANGE_PARAM || iIndex < 0 || iIndex > MAX_ITS_TEMPLATE)//Currently supports MAX_TIME_RANGE_PARAM-1 types, MAX_ITS_TEMPLATE templates, index=8 is a temporary value
				{
					return;
				}
				if (MAX_ITS_TEMPLATE == iIndex)
				{
					UpdateDomeHDPara(255, iType);//Update the temporary effect value of the corresponding type
				}
				else
				{
					...
					{
						UpdateDomeHDPara(iIndex, iType);//Update the effect value of the corresponding type of the template
					}	
				}
			}
			break;
...
void CLS_DomeHDParam1::UI_UpdateDialogText()
{
	...
	SetDlgItemTextEx(IDC_STATIC_SHUTTER, IDS_HD_MINI_SHUTTER);
	...
	m_cboMiniShutter.ResetContent();
	for (int i = 0; i < sizeof(g_iShutSpd3516)/sizeof(CString); i++)
	{
		m_cboMiniShutter.InsertString(i, g_szShutSpd3516[i]);
		m_cboMiniShutter.SetItemData(i,g_iShutSpd3516[i]);
	}
	m_cboMiniShutter.SetCurSel(0);

	// aperture size
	//0-Close, 1-F1.6, 2-F2.0, 3-F3.4, 4-F4.4, 5-F6.0, 6-F8.0, 7-F11.0, 8- F2.2, 9-F2.3, 10-F2.4
	...
	case 0:
		{
			...
			SetDlgItemText(IDC_STATIC_MINI_SHUTTER, GetTextEx(IDS_HD_MINI_SHUTTER));
	...
	case 1:
		{
			...
			GetDlgItem(IDC_STATIC_MINI_SHUTTER)->SetWindowText(GetTextEx(IDS_HD_MINI_SHUTTER));
...
void CLS_DomeHDParam1::UpdateDomeHDPara(int _iTemplateID, int _iType)
{
	...
	else
	{
		...
		//mini shutter
		if (bGetTemplatePara || _iType == HD_PARA_MINI_SHUTTER)
		{
			int iType = -1;
			int iIndex = pb.iAutoEnable[HD_PARA_SHUTTER];
			if (iIndex == 1)
			...
			int iCount = m_cboMiniShutter.GetCount();
			for (int x=0; x<iCount; x++)
			{
				if (m_cboMiniShutter.GetItemData(x) == iType)
				{
					m_cboMiniShutter.SetCurSel(x);
					break;
				}
			}
		}

		//0-Aperture adjustment
...
void CLS_DomeHDParam1::OnCbnSelchangeComboGammaset()
{
	...
	{
		GetDlgItem(IDC_SLIDER_GAMMA_MODIFY)->EnableWindow(FALSE);
		//Select automatic, set AutoEnable to 1
		SetDomeHDPara(HD_PARA_GAMMA, 1, 0, 0, 0, 0);
	}
	else
	{
		GetDlgItem(IDC_SLIDER_GAMMA_MODIFY)->EnableWindow(TRUE);
		//Select manual, Param2 gives the selected curve type, Param3 gives the corresponding value
...
void CLS_DomeHDParam1::OnCbnSelchangeComboShutter()
{
	...
	{
		SetDomeHDPara(HD_PARA_SHUTTER, 0, iShutter, 0, 0, 0); //Manual
	} 
	else
	{
		SetDomeHDPara(HD_PARA_SHUTTER, 1, 0, iShutter, 0, 0); //not manual
	}
}
...
void CLS_DomeHDParam1::OnCbnSelchangeComboAperture()
{
	...
	{
		SetDomeHDPara(HD_PARA_APERTURE, 0, iAperture, 0, 0, 0); //Manual
	} 
	else
	{
		SetDomeHDPara(HD_PARA_APERTURE, 1, 0, iAperture, 0, 0); //not manual
	}
}
...
{
	// TODO: Add your control notification handler code here
	int iSel = m_cboAutoGain.GetCurSel();//0 off, 1 on
	if (iSel == 1)//Auto gain on
	{
...
{
	// TODO: Exposure Compensation---Used to control compensation gain
	int iIndex = m_cbo_Expo_OffSet.GetCurSel();
	int iOffSet_Expo = 50;
	if (iIndex == 0)
	{
		//Hide compensation gain adjustment options
...
{
	// TODO: High sensitivity Param1 stands for high sensitivity, 1 is on, 0 is off, and the number of options is the same
...
void CLS_DomeHDParam1::OnBnClickedButtonMaxgainlimitSet()
{
	...
	if (1 == iExpoMode)
	{
		SetDomeHDPara(HD_PARA_GAIN, 0, iPos, 0, 0, 0); //Manual
	}
	else
	{
...
void CLS_DomeHDParam1::OnCbnSelchangeComboMiniShutter()
{
	// TODO: mini Shutter Speed
	int iIndex = m_cboExpoMode.GetCurSel();
	int iMiniShutter = (int)m_cboMiniShutter.GetItemData(m_cboMiniShutter.GetCurSel());
	if (iIndex == 0)
	{
		SetDomeHDPara(HD_PARA_MINI_SHUTTER, 1, 0, iMiniShutter, 0, 0); //not Manual
	} 
}
