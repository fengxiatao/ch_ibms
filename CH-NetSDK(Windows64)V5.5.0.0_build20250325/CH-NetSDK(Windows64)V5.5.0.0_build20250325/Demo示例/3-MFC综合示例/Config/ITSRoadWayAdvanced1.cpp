// ITSRoadWayAdvanced1.cpp : Implementation file
//
...
// CLS_ITSRoadWayAdvanced1 dialog
...
// CLS_ITSRoadWayAdvanced1 message handler
...
void CLS_ITSRoadWayAdvanced1::OnBnClickedBtnSetIllegalpark()
{
	...
	tITSIllegalPark.iTimeRange[iCurSegment] = ((iRangeTimeStartH<<24) & 0xff000000) |   //The start hour is placed in the 24~31 bits of the int type
		...
		((iRangeTimeEndH<<8) & 0x0000ff00) |                  //The end hour is placed in the 8~15 digits of the int type
		(iRangeTimeEndM&0x000000ff);                          //The end minute is placed in int type 0~7 bits
	GetCoorEditShow(tITSIllegalPark.arrPTArea, 5, 0);           //Write the coordinate data on the interface into the tITSIllegalPark.arrPTArea array
...
void CLS_ITSRoadWayAdvanced1::UI_UpdateIllegalpark()
{
	...
	if(iRet == 0)
	{
		...
		SYSTEMTIME stStartTime;        //Define a variable of type system time
		stStartTime.wHour = iStartHour;
		stStartTime.wMinute = iStartMinute;
		//CTime ctStartTime = CTime(stStartTime);   //Define a CTime object and assign to it
		m_dtcFrom.SetTime(&stStartTime);

		SYSTEMTIME stEndTime;        //Define a variable of type system time
		stEndTime.wHour = iEndHour;
		stEndTime.wMinute = iEndMinute;
		//CTime ctEndTime(stEndTime);        //Define a CTime object and assign to it
