...
void CDeviceDiskInfo::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO_SDDEVICENO2, m_cboDiskInfoEx);
    DDX_Control(pDX, IDC_CHECK_FULLFLAG, m_chkDiskFullFlag);
}
...
BEGIN_MESSAGE_MAP(CDeviceDiskInfo, CDialog)
    ...
    ON_CBN_SELCHANGE(IDC_COMBO_SDDEVICENO2, &CDeviceDiskInfo::OnCbnSelchangeComboSddeviceno2)
    ON_BN_CLICKED(IDC_BUTTON4, &CDeviceDiskInfo::OnBnClickedButton4)
END_MESSAGE_MAP()
...
void CDeviceDiskInfo::UI_UpdateSurface()
{
	...
	SetDlgItemText(IDC_STATIC_NFSACCOUNT, GetTextByLan(_T("”√ªß√˚"), _T("Account")));
	SetDlgItemText(IDC_STATIC_NFSPASSWORD, GetTextByLan(_T("√‹¬Î"), _T("Password")));
...
void CDeviceDiskInfo::UpdateDiskInfo()
{
	...
	NVS_NFS_DEVEx nfsDev;
	int iByteReturn = -1;
	memset(&nfsDev, 0, sizeof(NVS_NFS_DEVEx));
	if(0 == NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_LOCALSTORE_MAPDEVICE_IPV6, m_iChannelNo, &nfsDev, sizeof(nfsDev),&iByteReturn))
	{
		SetDlgItemText(IDC_EDIT_NFSPATH, nfsDev.cStorePath);
		SetDlgItemText(IDC_EDIT_NFSACCOUNT, nfsDev.cUsername);
		SetDlgItemText(IDC_EDIT_NFSPASSWORD, nfsDev.cPassword);
		if (IsValidIPv6(nfsDev.cDeviceIPv6) >= 1)
		{
			SetDlgItemText(IDC_EDIT_NFSIP, nfsDev.cDeviceIPv6);
		}
		else
		{
...
int CDeviceDiskInfo::DiskPart(int _iDiskNo, int _iPartNum, int _iFormatNow)
{
	...
	{	//Format after default partition
...
void CDeviceDiskInfo::OnBnClickedButtonNfsMap()
{
	...
	NVS_NFS_DEVEx nfsDev;
	memset(&nfsDev, 0, sizeof(NVS_NFS_DEVEx));
	nfsDev.iSize = (int)sizeof(NVS_NFS_DEVEx);
	...
	GetDlgItemText(IDC_EDIT_NFSACCOUNT,nfsDev.cUsername, sizeof(nfsDev.cUsername));
	GetDlgItemText(IDC_EDIT_NFSPASSWORD,nfsDev.cPassword, sizeof(nfsDev.cPassword));
	GetDlgItemText(IDC_EDIT_NFSIP, nfsDev.cDeviceIPv6, sizeof(nfsDev.cDeviceIPv6));
	if (IsValidIPv6(nfsDev.cDeviceIPv6) >= 1)
	{
		memset(nfsDev.cDeviceIP,0,sizeof(nfsDev.cDeviceIP));
	}
	else
	{
		memset(nfsDev.cDeviceIPv6,0,sizeof(nfsDev.cDeviceIPv6));
	}
	//if(0 == NetClient_NetFileMapStoreDevice(m_iLogonID, &nfsDev))
	if(0 == NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_LOCALSTORE_MAPDEVICE_IPV6, m_iChannelNo, &nfsDev, sizeof(nfsDev)))
	{
		AddLog(LOG_TYPE_SUCC, "", "NetClient_NetFileMapStoreDevice(%d)", m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL, "", "NetClient_NetFileMapStoreDevice(%d),ip:%s, path:%s,ipv6:%s", m_iLogonID, nfsDev.cDeviceIP, nfsDev.cStorePath,nfsDev.cDeviceIPv6);
	}
}

void CDeviceDiskInfo::UpdateDiskInfoEx()
{
    m_cboDiskInfoEx.ResetContent();
    m_chkDiskFullFlag.SetCheck(BST_UNCHECKED);
    
    SetDlgItemText(IDC_STATIC_SDNO2, GetTextByLan(_T("¥≈≈Ã±‡∫≈"), _T("Disk No.")));
    SetDlgItemText(IDC_CHECK_FULLFLAG, GetTextByLan(_T("¥≈≈Ã¬˙≈Ã"), _T("Full Flag")));
    SetDlgItemText(IDC_BUTTON4, GetTextByLan(_T("ªÒ»°"), _T("Get")));
    SetDlgItemText(IDC_STATIC_SDDEVICE2, GetTextByLan(_T("¥≈≈Ã–≈œ¢"), _T("Disk Info")));
   
}

#define DISK_INFO_FULL 1
void CDeviceDiskInfo::OnCbnSelchangeComboSddeviceno2()
{
    NVS_DISKINFOEX diskInfo; 
    diskInfo.iSize = sizeof(diskInfo);
    int iByteReturn = 0;
    int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_GETDISKINFOEX, m_iChannelNo, (void*)&diskInfo, sizeof(diskInfo), &iByteReturn);
    if (RET_SUCCESS == iRet)
    {
        int idx = m_cboDiskInfoEx.GetCurSel();
        if (DISK_INFO_FULL == diskInfo.stDiskInfos[idx].iFullFlag)
        {
            m_chkDiskFullFlag.SetCheck(BST_CHECKED);
        }
        else
            m_chkDiskFullFlag.SetCheck(BST_UNCHECKED);
    }
...
void CDeviceDiskInfo::OnBnClickedButton4()
{
    UpdateDiskInfoEx();
    NVS_DISKINFOEX diskInfo; 
    diskInfo.iSize = sizeof(diskInfo);
    ...
    if (RET_SUCCESS == iRet)
    {
        for (int idx = 0; idx < diskInfo.iDiskNum; idx++)
        {
            m_cboDiskInfoEx.SetItemData(m_cboDiskInfoEx.AddString(IntToCString(diskInfo.stDiskInfos[idx].iDiskNo)), diskInfo.stDiskInfos[idx].iDiskNo);
        }
        m_cboDiskInfoEx.SetCurSel(0);
        OnCbnSelchangeComboSddeviceno2();
    }
