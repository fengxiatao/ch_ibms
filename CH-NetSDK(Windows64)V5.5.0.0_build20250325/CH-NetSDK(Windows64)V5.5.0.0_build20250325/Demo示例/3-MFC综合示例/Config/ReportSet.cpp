...
BOOL CLS_ReportSet::OnInitDialog()
{
	...
	m_edtFtpPath.SetLimitText(31);
	m_edtFtpPath.setFilter(FILTERCH, true, true);
...
void CLS_ReportSet::UnloadReportTime()
{
	GetDlgItem(IDC_GBO_CAPTURE_SET)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STC_CAPTURE_TIME)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STC_CAPTURE_INTERVAL)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BTN_CAPTURE_SET)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STATIC)->ShowWindow(SW_HIDE);
	m_dtCaptureBeg.ShowWindow(SW_HIDE);
	m_dtCaptureEnd.ShowWindow(SW_HIDE);
	m_edtCaptureInterval.ShowWindow(SW_HIDE);
...
{
	m_lstReportTime.DeleteAllItems();
	int iCount = (int)m_vecReptTime.size();
...
void CLS_ReportSet::OnBnClickedBtnReportAdd()
{
	...
	for (int i=0; i<(int)m_vecReptTime.size(); i++)
	{
		ScenetimePoint stTmp = m_vecReptTime[i];
	...
	{
		blFind = false;
		for (int j=0; j<(int)m_vecReptTime.size(); j++)
		{
...
{
	if (m_iSelItem < 0 || m_iSelItem > (int)m_vecReptTime.size())
	{
		return;
	}
...
void CLS_ReportSet::OnNMClickLstReportTime(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (nItem < 0 || nItem > (int)m_vecReptTime.size())
	{
		return;
	}
	m_iSelItem = nItem;
