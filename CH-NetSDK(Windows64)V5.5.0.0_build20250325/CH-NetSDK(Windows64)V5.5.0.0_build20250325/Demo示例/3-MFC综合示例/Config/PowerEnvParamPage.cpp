...
enum n_AlarmType{
	BIT_HEAT_HI_ALARM = 1 << 0,//temperature upper limit alarm
	BIT_HEAT_HI_DEALARM = 1 << 1,//The upper temperature limit alarm
	BIT_HEAT_LO_ALARM = 1 <<2,//temperature lower limit alarm
	BIT_HEAT_LO_DEALARM=1 << 3,//The temperature lower limit fire alarm
	BIT_HUM_HI_ALARM = 1 <<4,//humidity upper limit alarm
	BIT_HUM_HI_DEALARM = 1 <<5,//humidity upper limit fire alarm
	BIT_HUM_LO_ALARM = 1 << 6,//Lower humidity alarm
	BIT_HUM_LO_DEALARM = 1 << 7,//Lower humidity alarm
	BIT_OFFLINE_ALARM = 1 << 31//Device offline alarm
};
...
// CLS_PowerEnvParamPage message handler
...
void CLS_PowerEnvParamPage::OnNMClickLstDh(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if(nItem > iCount) //Return if the row has no data
	{
		return;
	}
	if (nSub == 5)
