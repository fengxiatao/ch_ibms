...
void CLS_DomeHDParam3::UpdateDomeHDPara(int _iTemplateID, int _iType)
{
	...
	else
	{
		...
		//19-Use scene: 0 indoor; 1 outdoor
		...
		if(bGetTemplatePara || _iType == HD_PARA_STRONG_LIGHT)
		{
			...
			{	//Write default value in case of shutdown
		...
		if(bGetTemplatePara || _iType == HD_PARA_DNR)
		{
			...
			int iMode = 0;
			if (iEnable == 1)											// normal mode
			{
				if(iValue == 0)											// means close
				{
...
void CLS_DomeHDParam3::OnCbnSelchangeCboThroughMist()
{
	...
	// TODO: through fog
	if (m_cboThroughMist.GetCurSel() == 1)														//turn on
	{
		GetDlgItem(IDC_STATIC_MIST_STRENGTH)->EnableWindow(TRUE);
		GetDlgItem(IDC_SLIDER_MIST_STRENGTH)->EnableWindow(TRUE);
		GetDlgItem(IDC_STATIC_MIST_STRENGTH_TEXT)->EnableWindow(TRUE);
...
//Strong light suppression setting button
...
//Set the digital noise reduction button
...
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: Temporal noise reduction level
...
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: Airspace noise reduction level
...
{
	// TODO: Auto Contrast Param1 1 on, 0 off
	...
	else
	{
		...
		m_cboThroughMist.SetCurSel(0);//20140410 Auto contrast and fog penetration are mutually exclusive
	}
}
...
{
	// TODO: Add your control notification handler code here
	//After the automatic comparison is turned on, there are three levels of low, medium and high
	SetDomeHDPara(HD_PARA_AUTO_CONTRAST, 1, 1, m_cboSencitivityAutoContrast.GetCurSel(), 0, 0);
}

void CLS_DomeHDParam3::OnCbnSelchangeCboLightless()
{
	// TODO: low light color suppression 0-off, 1-1 level, 2-2 level, 3-3 level
	SetDomeHDPara(HD_PARA_LIGHTLESS, 1, m_cboLightness.GetCurSel(), 0, 0, 0);
}

void CLS_DomeHDParam3::OnCbnSelchangeCboNightStrenth()
{
	// TODO: Night Vision Enhancement 1 means on, 0 means off
...
{
	// TODO: select the current template and get the corresponding parameters
	UpdateDomeHDPara();
}
