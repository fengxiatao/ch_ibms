...
#include "math.h"
#define ITS_ROADTYPE_NUM 8 //Number of lane types
#define ITS_INTERVAL_MAX 1000 // maximum time interval
...
BOOL CLS_ItsSignalCheck::UI_UpdateAreaSet() //Signal light area
{
	if (m_iLogonID < 0)
		return FALSE;
...
void CLS_ItsSignalCheck::OnBnClickedBtnSettime() //Set time parameters
{
	if (m_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
...
BOOL CLS_ItsSignalCheck::UI_UpdateTimeSet() //Refresh time parameter
{
	if (m_iLogonID < 0)
		return FALSE;
	TITSLightInfoEx tLightInfoEx = {0};
