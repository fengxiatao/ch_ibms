...
void CLS_StorageStrategyPage::DoDataExchange(CDataExchange* pDX)
{
	...
	//Because the middle ID of the control is occupied and initialized in two segments
	for(int i = 0; i < 16; ++i)
	{
...
BOOL CLS_StorageStrategyPage::UI_UpdateDiskGroup()
{
	...
	if (iRet == 0)
	{
		for (int i=0; i<MAX_DISK_NUM_EX; i++)
		{
            if (i < DISK_SATA_NUM)
            {
			    ...
			    {
	//disk i is no disk or backup disk
				    m_chkDiskNo[i].EnableWindow(FALSE);
			    }
            }
            else if (i < DISK_SATA_EX_NUM + DISK_SATA_NUM)
            {
                if (storage.m_strctSATA_EX[i - DISK_SATA_NUM].m_u16Status == 0 || storage.m_strctSATA_EX[i - DISK_SATA_NUM].m_u16Usage == 1)
                {
                     //disk i is no disk or backup disk
                    m_chkDiskNo[i].EnableWindow(FALSE);
                }
            ...
            {
                if (storage.m_tSata[i - 16].m_u16Status == 0 || storage.m_tSata[i - 16].m_u16Usage == 1)
                {
                     //disk i is no disk or backup disk
                    m_chkDiskNo[i].EnableWindow(FALSE);
                }         
	...
	//Change to the large number processing method
	//DISK_GROUP strDiskGroup[MAX_DISK_GROUP_NUM] = {0};
	...
	//			if (strDiskGroup[iIndex].uiDiskNO[i/32]>>(i%32) & 1)
	//			{
	// //Gray out the disks used by other disk groups except the currently selected disk group number.
	//				m_chkDiskNo[i].EnableWindow(FALSE);
	//			}
	//		}
	//		iIndex++;
...
void CLS_StorageStrategyPage::OnBnClickedButtonSave()
{
    ...
    m_strDiskGroup[iGroupIndex].iSize = sizeof(m_strDiskGroup[iGroupIndex]);
	for (int i=0; i<MAX_DISK_GROUP_NUM; i++)
	{
	...
	//	}
	//}
    if (0 == m_strDiskGroup[iGroupIndex].uiDiskNO[0]) //No disk default clear channel
    {
        memset(m_iChannelEnable, 0, sizeof(m_iChannelEnable));
        for (int idx = 0; idx < LEN_64; idx++)
        {

            m_chkChannelNo[idx].SetCheck(false);
	...
	int iRet = NetClient_SetDiskGroup(m_iLogonID, &m_strDiskGroup[iGroupIndex], 1);
	if (iRet < 0)
	{
...
{
	int index = 0;
	//The middle value of the control ID is occupied
