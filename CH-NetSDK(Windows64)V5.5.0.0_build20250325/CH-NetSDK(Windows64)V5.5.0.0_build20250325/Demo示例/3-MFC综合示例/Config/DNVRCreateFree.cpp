...
void CLS_DNVRCreateFree::OnBnClickedButtonCreatefreeSetAll()
{
	...
	for (int i = 0; i<tParam.iPicCnt; i++)
	{
		...
		{
			MessageBox("Please select a channel number!","hint",MB_OK|MB_TOPMOST);
		...
		{
			cstrTmp.Format("The sum of the abscissa and width of multi-screen %d is greater than 10000, please reset!", i+1);
			MessageBox(cstrTmp,"hint",MB_OK|MB_TOPMOST);
			m_edtCoordsX[i].SetFocus();
			return;
		}
		if (iWidth < iVWidth)
		{
			cstrTmp.Format("Multi-screen %d width cannot be smaller than %d, please reset!", i+1, iVWidth);
			MessageBox(cstrTmp,"hint",MB_OK|MB_TOPMOST);
		...
		{
			cstrTmp.Format("The sum of the multi-screen %d ordinates and heights is greater than 10000, please reset!", i+1);
			MessageBox(cstrTmp,"hint",MB_OK|MB_TOPMOST);
		...
		if (iHeight < iVHeight)
		{
			cstrTmp.Format("The height of multi-screen %d cannot be less than %d, please reset it!", i+1, iVHeight);
			MessageBox(cstrTmp,"hint",MB_OK|MB_TOPMOST);
		...
		if (0 != i)
		{
			for (int j=i-1; j>=0; j--)
			{
				...
				{
					//do nothing The screen does not overlap
				}
				else
				{
					cstrTmp.Format("Multi-screen %d and multi-screen %d have overlapping areas, please reset!", j+1, i+1);
					MessageBox(cstrTmp,"hint",MB_OK|MB_TOPMOST);
					return;
				}
			}
