...
void CLS_PEDevSwitchEdit::OnBnClickedButtonSave()
{
	...
	{
		MessageBox("Name is not allowed to be empty or contain space!", "", MB_OK|MB_TOPMOST);
		m_edtName.SetFocus();
		return;
	}
	...
	if (cstrDelayTime == _T(""))
	{
		MessageBox("Alarm suppression delay time is not allowed to be empty!", "", MB_OK|MB_TOPMOST);
