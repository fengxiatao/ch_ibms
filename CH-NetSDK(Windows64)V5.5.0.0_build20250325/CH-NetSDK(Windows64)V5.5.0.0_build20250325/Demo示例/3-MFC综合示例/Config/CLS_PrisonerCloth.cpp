...
IMPLEMENT_DYNAMIC(CLS_PrisonerCloth, CDialog)

CLS_PrisonerCloth::CLS_PrisonerCloth(CWnd* pParent /*=NULL*/)
	: CLS_BasePage(CLS_PrisonerCloth::IDD, pParent)
{
	m_iSensitivity = 50;
	m_iMinSize = 2;
	m_iMaxSize = 25;
	memset(m_tValidRegion, 0, sizeof(ValidRgPrisonerCloth) * MAX_REGION_COUNT_CLOTH);
}

CLS_PrisonerCloth::~CLS_PrisonerCloth()
{
	vector<vca_TPolygonEx> temp;
	temp.swap(m_vecMaskRegion);
}

BOOL CLS_PrisonerCloth::OnInitDialog()
{
...
void CLS_PrisonerCloth::OnChannelChanged(int _iLogonID,int _iChannelNo,int _iStreamNo)
{
	m_iLogonID = _iLogonID;
	m_iStreamNO = _iStreamNo;
	m_iChannelNO = _iChannelNo;
	GetPrisonerClothInfo();
}

void CLS_PrisonerCloth::OnLanguageChanged(int _iLanguage)
{
	UI_UpdateUIText();
}

void CLS_PrisonerCloth::UI_UpdateUIText()
{
	SetDlgItemText(IDC_STATIC_RULENUM, GetTextByLan(_T("规则号"), _T("RuleNum")));
	SetDlgItemText(IDC_STATIC_PRISONER_SCENE, GetTextByLan(_T("场景"), _T("Scene")));
...
void CLS_PrisonerCloth::GetPrisonerClothInfo()
{
	XmlPrisonerCloth tInfo;
	memset(&tInfo, 0, sizeof(XmlPrisonerCloth));
	...
	int iRet = NetClient_XmlGetDevConfig(m_iLogonID, NETXMLCFG_PRISONERCLOTH, &tInfo, sizeof(XmlPrisonerCloth), &tInfo, sizeof(XmlPrisonerCloth));
	if(RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC, "","CLS_PrisonerCloth::NetClient_XmlGetDevConfig[NETXMLCFG_PRISONERCLOTH] (%d)", m_iLogonID);
		...
		for(int i = 0; i < tInfo.iValidRegionCount && i < MAX_REGION_COUNT_CLOTH; ++i)
		{
			...
			memcpy(&m_tValidRegion[iCur], &tInfo.tValidRegion[i], sizeof(ValidRgPrisonerCloth));
		}
		m_cboValidRegNum.SetCurSel(0);
	...
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_PrisonerCloth::NetClient_XmlGetDevConfig[NETXMLCFG_PRISONERCLOTH] (%d), error(%d)", m_iLogonID, GetLastError());
	}
}

void CLS_PrisonerCloth::ShowValidList(int _iIndex)
{
...
void CLS_PrisonerCloth::ShowMaskList(int _iIndex)
{
...
BOOL CLS_PrisonerCloth::SetVCAStatus(bool _bStatus)
{
	int iProType = 0;
	int iProMode = 0;
...
void CLS_PrisonerCloth::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_PRISONER_SCENE, m_cboSceneNum);
...
BEGIN_MESSAGE_MAP(CLS_PrisonerCloth, CDialog)
	ON_WM_SHOWWINDOW()
	ON_BN_CLICKED(IDC_BUTTON_DRAW_MASK, &CLS_PrisonerCloth::OnBnClickedButtonDrawMask)
	ON_BN_CLICKED(IDC_BUTTON_DRAW_NEW, &CLS_PrisonerCloth::OnBnClickedButtonDrawNew)
	ON_BN_CLICKED(IDC_BUTTON_DELETE_MASK, &CLS_PrisonerCloth::OnBnClickedButtonDeleteMask)
	ON_BN_CLICKED(IDC_BUTTON_DRAW_VALID, &CLS_PrisonerCloth::OnBnClickedButtonDrawValid)
	ON_BN_CLICKED(IDC_BUTTON_DELETE_VALID, &CLS_PrisonerCloth::OnBnClickedButtonDeleteValid)
	ON_BN_CLICKED(IDC_BUTTON_SET, &CLS_PrisonerCloth::OnBnClickedButtonSet)
	ON_CBN_SELCHANGE(IDC_COMBO_REGION_NUM, &CLS_PrisonerCloth::OnCbnSelchangeComboRegionNum)
	ON_CBN_SELCHANGE(IDC_COMBO_MASK_REGION_NUM, &CLS_PrisonerCloth::OnCbnSelchangeComboMaskRegionNum)
	ON_BN_CLICKED(IDC_CHECK_REGION_ENABLED, &CLS_PrisonerCloth::OnBnClickedCheckRegionEnabled)
	ON_CBN_SELCHANGE(IDC_COMBO_RULENUM, &CLS_PrisonerCloth::OnCbnSelchangeComboRulenum)
	ON_CBN_SELCHANGE(IDC_COMBO_PRISONER_SCENE, &CLS_PrisonerCloth::OnCbnSelchangeComboPrisonerScene)
	ON_CBN_SELCHANGE(IDC_COMBO_MODEL, &CLS_PrisonerCloth::OnCbnSelchangeComboModel)
END_MESSAGE_MAP()

void CLS_PrisonerCloth::OnBnClickedButtonDrawMask()
{
...
void CLS_PrisonerCloth::OnBnClickedButtonDrawNew()
{
...
void CLS_PrisonerCloth::OnBnClickedButtonDeleteMask()
{
...
void CLS_PrisonerCloth::OnBnClickedButtonDrawValid()
{
	...
	if (IDOK == m_pDlgVideoView->DoModal())
	{
		...
		CString cstPolygon = cPointBuf;
		GetPointsFromString(cstPolygon, iPointNum, (POINT*)tPolygon);
		ValidRgPrisonerCloth &tInfo = m_tValidRegion[iCurRegion];
		int iEnable = tInfo.iRegionEnabled;
		memset(&tInfo, 0, sizeof(ValidRgPrisonerCloth));
...
void CLS_PrisonerCloth::OnBnClickedButtonDeleteValid()
{
	...
	memset(&m_tValidRegion[iIndex], 0, sizeof(ValidRgPrisonerCloth));
	OnCbnSelchangeComboRegionNum();
}

void CLS_PrisonerCloth::OnBnClickedButtonSet()
{
	// Set parameters
	UpdateData(TRUE);
	XmlPrisonerCloth tInfo;
	...
	for(int i = 0; i < MAX_REGION_COUNT_CLOTH; ++i)
	{
		...
		memcpy(&tInfo.tValidRegion[iIndex++], &m_tValidRegion[i], sizeof(ValidRgPrisonerCloth));
	}
	...
	int iRet = NetClient_XmlSetDevConfig(m_iLogonID, NETXMLCFG_PRISONERCLOTH, &tInfo, sizeof(XmlPrisonerCloth), NULL, 0);
	if(RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC, "","CLS_PrisonerCloth::NetClient_XmlSetDevConfig[NETXMLCFG_PRISONERCLOTH] (%d)", m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_PrisonerCloth::NetClient_XmlSetDevConfig[NETXMLCFG_PRISONERCLOTH] (%d), error(%d)", m_iLogonID, GetLastError());
	}
}


void CLS_PrisonerCloth::OnCbnSelchangeComboRegionNum()
{
	ShowValidList(m_cboValidRegNum.GetCurSel());
}

void CLS_PrisonerCloth::OnCbnSelchangeComboMaskRegionNum()
{
	ShowMaskList(m_cboMaskRegNum.GetCurSel());
}

void CLS_PrisonerCloth::OnBnClickedCheckRegionEnabled()
{
...
void CLS_PrisonerCloth::OnCbnSelchangeComboRulenum()
{
	// Toggle the rule number
	GetPrisonerClothInfo();
}

void CLS_PrisonerCloth::OnCbnSelchangeComboPrisonerScene()
{
	// Toggle the scene number
	GetPrisonerClothInfo();
}

void CLS_PrisonerCloth::OnCbnSelchangeComboModel()
{
	// Switch modes
	GetPrisonerClothInfo();
}

void CLS_PrisonerCloth::OnShowWindow(BOOL bShow, UINT nStatus)
{
