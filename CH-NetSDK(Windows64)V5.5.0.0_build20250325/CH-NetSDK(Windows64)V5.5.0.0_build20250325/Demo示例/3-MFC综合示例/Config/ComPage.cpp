...
void CLS_ComPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_ENABLE_GAUGED, m_cboGaugedEnable);
	DDX_Control(pDX, IDC_COMBO_GAUGED_RESULT, m_cboGaugedResult);
	DDX_Control(pDX, IDC_COMBO_DATATYPE, m_cboType);
	DDX_Text(pDX, IDC_EDIT_COM_DATA_RECV, m_csComRecvData);
}
...
BEGIN_MESSAGE_MAP(CLS_ComPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BUTTON_NORTH_ANGLE_SET, &CLS_ComPage::OnBnClickedButtonNorthAngleSet)
	ON_BN_CLICKED(IDC_BUTTON_VISUAL_RANGE, &CLS_ComPage::OnBnClickedButtonVisualRange)
	ON_BN_CLICKED(IDC_BUTTON_SEND, &CLS_ComPage::OnBnClickedButtonSend)
	ON_BN_CLICKED(IDC_BUTTON_COM_RECV, &CLS_ComPage::OnBnClickedButtonComRecv)
END_MESSAGE_MAP()
...
BOOL CLS_ComPage::OnInitDialog()
{
	...
	m_cboGaugedEnable.ResetContent();
	m_cboGaugedEnable.InsertString(0, "off");
	m_cboGaugedEnable.InsertString(1, "on");
	m_cboGaugedEnable.SetCurSel(0);

	m_cboGaugedResult.ResetContent();
	m_cboGaugedResult.InsertString(0, "Succ");
	m_cboGaugedResult.InsertString(1, "Failed");
	m_cboGaugedResult.SetCurSel(0);

	m_cboType.InsertString(0, GetTextByLan(_T("指定串口"),_T("to ComNo")));
	m_cboType.InsertString(1, GetTextByLan(_T("指定通道"),_T("to Channel")));
...
void CLS_ComPage::OnBnClickedButtonComFormat()
{
	...
	if (7 == iWorkMode)//A maximum of 1 ATM serial port can be set
	{
...
void CLS_ComPage::UI_UpdateDialog()
{
	...
	InsertString(m_cboWorkMode,7,IDS_CFG_PTZ_WORKMODE_EXDEV);
	...
	SetDlgItemText(IDC_STATIC_NORTH_STATUS,GetTextByLan(_T("校正状态"),_T("iStatus"))  );
	GetDlgItem(IDC_STATIC_DEVICE_NAME)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_COMBO_DEVICE_NAME)->ShowWindow(SW_HIDE);
...
void CLS_ComPage::OnBnClickedBtnDlsend()
{
	...
	//char ucBuf[1024] = {0};
	char cDlSend[256] = {0xF6,0x0D,0x00,0x53,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	int iLen = strData.GetLength()/2;
	if (iLen >= 8)
	{
		iLen = 8;
...
void CLS_ComPage::OnBnClickedButtonNorthAngleSet()
{
	if(m_iLogonID < 0 || m_iChannelNO < 0)
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ComPage::OnBnClickedButtonNorthAngleSet (%d, %d)", m_iLogonID, m_iChannelNO);
	...
	NorthAngle tInPara = {0};
	tInPara.iSize = sizeof(tInPara);

	int iRet = NetClient_CmdConfig(m_iLogonID, CMD_NORTH_ANGLE, m_iChannelNO, &tInPara, tInPara.iSize, &tInfo, sizeof(tInfo));
	if(RET_SUCCESS > iRet)
	{
		AddLog(LOG_TYPE_FAIL, "", "[NetClient_CmdConfig][CMD_NORTH_ANGLE] Set fail,error = %d", GetLastError());
	}
	else
	{
		SetDlgItemInt(IDC_EDIT_NORTH_ANGLE, tInfo.iAngle);
		SetDlgItemInt(IDC_EDIT_NORTH_STATUS, tInfo.iStatus);
		AddLog(LOG_TYPE_SUCC, "", "[NetClient_CmdConfig][CMD_NORTH_ANGLE] Set Success", m_iLogonID);
	}
}


void CLS_ComPage::OnBnClickedButtonVisualRange()
{
	if(m_iLogonID < 0 || m_iChannelNO < 0)
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ComPage::OnBnClickedButtonVisualRange (%d, %d)", m_iLogonID, m_iChannelNO);
	...
	VisualRange tInPara = {0};
	tInPara.iSize = sizeof(tInPara);

	int iRet = NetClient_CmdConfig(m_iLogonID, CMD_VISUAL_RANGE, m_iChannelNO, &tInPara, tInPara.iSize, &tInfo, sizeof(tInfo));
	if(RET_SUCCESS > iRet)
	{
		AddLog(LOG_TYPE_FAIL, "", "[NetClient_CmdConfig][CMD_VISUAL_RANGE] Set fail,error = %d", GetLastError());
	}
	else
	{
		SetDlgItemInt(IDC_EDIT_VISUAL_RANGE, tInfo.iRange);
		SetDlgItemInt(IDC_EDIT_VISUAL_RANGE_HVIEW, tInfo.iHview);
		SetDlgItemInt(IDC_EDIT_VISUAL_RANGE_VVIEW, tInfo.iVview);
		AddLog(LOG_TYPE_SUCC, "", "[NetClient_CmdConfig][CMD_VISUAL_RANGE] Set Success", m_iLogonID);
	}
}


void CLS_ComPage::OnMainNotify(int _ulLogonID,int _iWparam, void* _iLParam, void* _iUser)
{
	...
	switch(iMsgType)
	{
	case WCM_NORTH_ANGLE:
		{
			NorthAngle *tInfo = (NorthAngle*)_iLParam;
			AddLog(LOG_TYPE_SUCC, "", "[NetClient_CmdConfig][CMD_VISUAL_RANGE] Set Success", tInfo->iAngle);
			SetDlgItemInt(IDC_EDIT_NORTH_ANGLE, tInfo->iAngle);
			SetDlgItemInt(IDC_EDIT_NORTH_STATUS, tInfo->iStatus);
		...
		break;
		{
	case WCM_VISUAL_RANGE:
		{
			VisualRange *tInfo = (VisualRange*)_iLParam;
			SetDlgItemInt(IDC_EDIT_VISUAL_RANGE, tInfo->iRange);
			SetDlgItemInt(IDC_EDIT_VISUAL_RANGE_HVIEW, tInfo->iHview);
			SetDlgItemInt(IDC_EDIT_VISUAL_RANGE_VVIEW, tInfo->iVview);
		}
		break;
	case WCM_VIRTUALGAUGEDIS:
		{
			VirGaugedIs *tInfo = (VirGaugedIs*)_iLParam;
			m_cboGaugedResult.SetCurSel(tInfo->iResult);
		}
		break;
	default:
		break;
...
void CLS_ComPage::OnBnClickedButtonSend()
{
	if(m_iLogonID < 0 || m_iChannelNO < 0)
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ComPage::OnBnClickedButtonSend (%d, %d)", m_iLogonID, m_iChannelNO);

	}

	VirGaugedIs  tInfo = {0};
	tInfo.iSize = sizeof(tInfo);
	tInfo.iEnable = m_cboGaugedEnable.GetCurSel();
	int iRet = NetClient_SendCommand(m_iLogonID,  COMMAND_ID_VIRTUALGAUGEDIS,  m_iChannelNO,  &tInfo, (int)sizeof(tInfo));
	if (RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","[CLS_ComPage]NetClient_SendCommand(COMMAND_ID_VIRTUALGAUGEDIS)Success!LogonID(%d)",m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","[CLS_ComPage]NetClient_SendCommand(COMMAND_ID_VIRTUALGAUGEDIS)Failed!LogonID(%d), Error(0x%08x)",m_iLogonID, ::GetLastError());
	}
}

void CLS_ComPage::OnBnClickedButtonComRecv()
{
	if(m_iLogonID < 0 || m_iChannelNO < 0)
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ComPage::OnBnClickedButtonSend (%d, %d)", m_iLogonID, m_iChannelNO);
	...
	ComRecvData  tInfo = {0};
	tInfo.iSize = sizeof(tInfo);
	tInfo.iComNo = (int)m_cboComNo.GetItemData(iComIndex);
	
	int iRet = NetClient_GetDevConfig(m_iLogonID,  NET_CLIENT_COM_RECVDATA,  m_iChannelNO,  &tInfo, (int)sizeof(tInfo),&iBytesReturned);
	if (RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","[CLS_ComPage]NetClient_GetDevConfig(NET_CLIENT_COM_RECVDATA)Success!LogonID(%d)",m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","[CLS_ComPage]NetClient_GetDevConfig(NET_CLIENT_COM_RECVDATA)Failed!LogonID(%d), Error(0x%08x)",m_iLogonID, ::GetLastError());
	}

	m_csComRecvData = tInfo.ucBuffer;
