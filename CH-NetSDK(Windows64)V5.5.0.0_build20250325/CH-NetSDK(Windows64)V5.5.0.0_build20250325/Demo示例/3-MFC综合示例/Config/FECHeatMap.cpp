...
void CLS_FECHeatMap::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_SEQUENCE, m_cboSortType);
}
...
void CLS_FECHeatMap::UI_UpdateDialog()
{
	...
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORT_AGE)), REPORT_QUERY_TYPE_AGE_RANGE);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_GENDER)), REPORT_QUERY_TYPE_SEX);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORT_PEOPLE_NUM)), REPORT_QUERY_TYPE_PEOPLE_NUM);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_NATION)), REPORT_QUERY_TYPE_NATION);
	...
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORTTYPE_PERSON_SEX)), REPORT_QUERY_TYPE_PERSON_SEX);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORTTYPE_PERSON_NUM)), REPORT_QUERY_TYPE_PERSON_NUM);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORTTYPE_PERSON_DIRECTION)), REPORT_QUERY_TYPE_PERSON_DIRECTION);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORTTYPE_VIHICLE_TYPE)), REPORT_QUERY_TYPE_VIHICLE_TYPE);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextEx(IDS_REPORTTYPE_VIHICLE_DIRECTION)), REPORT_QUERY_TYPE_VIHICLE_DIRECTION);
	m_cboReportQueryType.SetItemData(m_cboReportQueryType.AddString(GetTextByLan("人数统计","People Counting")), REPORT_QUERY_TYPE_PEOPLEA_COUNTING);

	
	iTempIndex = (iTempIndex < 0 || iTempIndex >= m_cboReportQueryType.GetCount()) ? 0 : iTempIndex;
	m_cboReportQueryType.SetCurSel(iTempIndex);

	iTempIndex = m_cboSortType.GetCurSel();
	m_cboSortType.ResetContent();
	m_cboSortType.SetItemData(m_cboSortType.AddString(GetTextEx(IDS_POSITION_SEQUENCE)), REPORT_QUERY_POSITIVE);
	m_cboSortType.SetItemData(m_cboSortType.AddString(GetTextEx(IDS_INVERTED_ORDER)), REPORT_QUERY_REVERSE);
	m_cboSortType.SetCurSel((iTempIndex > 0) ? iTempIndex : 0);
...
void CLS_FECHeatMap::OnCbnSelchangeCboReportQuerryType()
{
	...
	case REPORT_QUERY_TYPE_TIME:
		...
		break;
	case REPORT_QUERY_TYPE_TEMPERATURE:
		cstrLogContent = GetTextEx(IDS_POWER_ENV_DEBUG_TEMPERATURE) + _T("(degree)");
		break;
	case REPORT_QUERY_TYPE_HUMIDNESS:
...
void CLS_FECHeatMap::OnBnClickedBtnQueryTimeMap()
{
	...
	tReportQuery.iSort = m_cboSortType.GetItemData(m_cboSortType.GetCurSel());
...
void CLS_FECHeatMap::UpdateReportQueryMap(ReportQueryResult* _ptInfo)
{
	...
	//If you can get the data directly from the callback, you will not call the interface to get the data.
