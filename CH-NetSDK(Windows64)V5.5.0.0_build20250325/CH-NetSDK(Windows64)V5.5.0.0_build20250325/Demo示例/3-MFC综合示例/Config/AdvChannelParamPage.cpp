...
BEGIN_MESSAGE_MAP(CLS_AdvChannelParamPage, CDialog)
	...
	ON_WM_SHOWWINDOW()
	ON_BN_CLICKED(IDC_BUTTON_SYNC_GETDEVICEPARAM, &CLS_AdvChannelParamPage::OnBnClickedButtonSyncGetdeviceparam)
	ON_BN_CLICKED(IDC_BUTTON_SYNC_GETCHANNELPARAM, &CLS_AdvChannelParamPage::OnBnClickedButtonSyncGetchannelparam)
END_MESSAGE_MAP()
...
void CLS_AdvChannelParamPage::OnBnClickedButtonChannelColor()
{
	...
	tColor2GrayParam.m_iTimeRange |= (iGray2Color_hour << 24);//Dawn hour
	tColor2GrayParam.m_iTimeRange |= (iGray2Color_minute << 16);//The minute of dawn
	tColor2GrayParam.m_iTimeRange |= (iColor2Gray_hour << 8);//Dark hour
	tColor2GrayParam.m_iTimeRange |= iColor2Gray_minute;//Dark minutes
...
void CLS_AdvChannelParamPage::UI_UpdateParam()
{
	...
	else
	{
		...
		int iGray2Color_hour = (tTempColor2GrayParam.m_iTimeRange >> 24) & 0x00000FF;//daylight hour
		int iGray2Color_minute = (tTempColor2GrayParam.m_iTimeRange >> 16) & 0x00000FF;//The minute of dawn
		int iColor2Gray_hour = (tTempColor2GrayParam.m_iTimeRange >> 8) & 0x00000FF; // dark hour
		int iColor2Gray_minute = tTempColor2GrayParam.m_iTimeRange & 0x00000FF;//Dark minute
...
void CLS_AdvChannelParamPage::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	if (!bShow)
	{
		KillTimer(BRIGHT_VALUE);
	}
}

void CLS_AdvChannelParamPage::OnBnClickedButtonSyncGetdeviceparam()
{
	int iRet = NetClient_SendCommand(m_iLogonID, COMMAND_ID_SYNC_GETDEVICEPARAM, m_iChannelNO, NULL, 0);
	if (RET_SUCCESS == iRet) {
		AddLog(LOG_TYPE_SUCC, "", "NetClient_SendCommand:COMMAND_ID_SYNC_GETDEVICEPARAM Success! logonID(%d)", m_iLogonID);
	} else {
		AddLog(LOG_TYPE_FAIL, "", "NetClient_SendCommand:COMMAND_ID_SYNC_GETDEVICEPARAM failed! logonID(%d),iRet(%d)" ,m_iLogonID, iRet);
	}
}

void CLS_AdvChannelParamPage::OnBnClickedButtonSyncGetchannelparam()
{
	int iRet = NetClient_SendCommand(m_iLogonID, COMMAND_ID_SYNC_GETCHANNELPARAM, m_iChannelNO, NULL, 0);
	if (RET_SUCCESS == iRet) {
		AddLog(LOG_TYPE_SUCC, "", "NetClient_SendCommand:COMMAND_ID_SYNC_GETCHANNELPARAM Success! logonID(%d),m_iChNo(%d)", m_iLogonID, m_iChannelNO);
	} else {
		AddLog(LOG_TYPE_FAIL, "", "NetClient_SendCommand:COMMAND_ID_SYNC_GETCHANNELPARAM failed! logonID(%d),m_iChNo(%d),iRet(%d)" ,m_iLogonID, m_iChannelNO, iRet);
	}
}
