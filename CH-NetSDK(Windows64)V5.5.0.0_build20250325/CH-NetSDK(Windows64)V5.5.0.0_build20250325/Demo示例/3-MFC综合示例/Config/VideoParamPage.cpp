...
static map<int, CString> g_mapNVSize;	//NTSC system video size
static map<int, CString> g_mapPVSize;	//PAL video size
...
#define     PASSAGE_TO_NORMAL   0	//Corridor mode resolution to normal resolution
#define     NORMAL_TO_PASSAGE   1	//Normal resolution to corridor mode resolution
...
//	ON_CBN_SELCHANGE(IDC_COMBO_FRAMERATE, &CLS_VideoParamPage::OnCbnSelchangeComboFramerate)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_VENCTYPE, &CLS_VideoParamPage::OnCbnSelchangeComboVenctype)		//Modify the video encoding method
...
BOOL CLS_VideoParamPage::OnInitDialog()
{
	...
	m_cboPassageMode.AddString("On");//on
	m_cboPassageMode.AddString("Off");//closure
...
void CLS_VideoParamPage::OnChannelChanged( int _iLogonID,int _iChannelNo,int _iStreamNo )
{
	...
	UI_InitVideoEncodeList();		//Get the current encoding format and refresh to the interface;
	UI_UpdateFrameMode();
	UI_UpdateIFrameRate();
...
void CLS_VideoParamPage::OnCbnSelchangeComboVenctype()
{
	...
	if(4 == m_iStreamNo)
	{
		tInfo.iStreamType = CHANNEL_THREE_STREAM;
	}
	else
	{
		tInfo.iStreamType = m_iStreamNo;
	}
	tInfo.iVencType = iVencType;

	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_VENCTYPE, m_iChannelNo, &tInfo, tInfo.iSize);
	if (0 == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","NetClient_SetDevConfig(%d,%d,%d)",m_iLogonID,m_iChannelNo,iVencType);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_SetDevConfig(%d,%d,%d)",m_iLogonID,m_iChannelNo,iVencType);
	}
}
...
BOOL CLS_VideoParamPage::UI_UpdateVencProfileEncoder()
{
	...
	{//H264 only supports setting profile
		int iLevel = 0;
...
void CLS_VideoParamPage::OnCbnSelchangeComboVideosizePassage()
{
	...
	int iVideoSize = 0; 
	if (m_cboPassageMode.GetCurSel() == 0)//Open
	{			
		iVideoSize = VideoSizeConvert(iCurVideoSize, NORMAL_TO_PASSAGE);
...
void CLS_VideoParamPage::UI_UpdateVideoSize()
{
	...
	//Get the current video format of the device
	...
	//Protection to avoid crashes caused by incorrect device sending
	if(iCount > 100)
	{
		iCount = 100;
	}
	...
	int index = 0;
	for(int i =0;i<iCount && iCount < 100;++i)
	{
		int iVideoSize = piVideoSizelist[i];
		CString csVideoSize;


		//Add fisheye super resolution
		if (0 != (iVideoSize&0x40000000))
		{
			int iWidth = iVideoSize&0x7FFF;
			int iHeight = (iVideoSize>>15)&0x7FFF;
			if (iWidth < iHeight)
			{//Flip resolution corridor mode
				continue;
			}
			csVideoSize.Format(_T("%dx%d"), iWidth, iHeight);
		}
		...
		{
			int iTempVideoSize = VideoSizeConvert(iCurrentVideoSize, PASSAGE_TO_NORMAL);
	...
	bool bNewMode = false; //Is it the resolution of the new mode
	if (0 != (iCurrentVideoSize&0x40000000))
	{
		bNewMode = true;
	}
	...
	if (bNewMode)
	{
		int iWidth = iCurrentVideoSize&0x7FFF;
		int iHeight = (iCurrentVideoSize>>15)&0x7FFF;
		if (iWidth < iHeight)
		{//Flip resolution corridor mode
...
void CLS_VideoParamPage::UI_InitVideoEncodeList()
{
	...
	//Get the current video compression encoding method
	...
	//Get a list of video compression encoding methods
	int iReturnByte = 0;
...
int CLS_VideoParamPage::VideoSizeConvert(int _iVideoSize, int _iCmd)
{
	int iNewVideoSize = 0;
	bool bNewMode = false; //Is it the resolution of the new mode
	if (0 != (_iVideoSize&0x40000000))
	{
		bNewMode = true;
	}
	...
	if (bNewMode)
	{
		int iWidth = _iVideoSize&0x7FFF;
		int iHeight = (_iVideoSize>>15)&0x7FFF;
		int iNewHeight = (iWidth<<15);

		iNewVideoSize = iNewHeight | iHeight;
		iNewVideoSize |= 0x40000000;
	}
	else
	{
		if (PASSAGE_TO_NORMAL == _iCmd)//Corridor mode resolution to normal resolution
		{
			iNewVideoSize = _iVideoSize - 0x100000 ;
		}
		else if (NORMAL_TO_PASSAGE == _iCmd)//normal resolution to corridor mode resolution
		{
			iNewVideoSize = _iVideoSize + 0x100000 ;
		}
	}

	return iNewVideoSize;
}
