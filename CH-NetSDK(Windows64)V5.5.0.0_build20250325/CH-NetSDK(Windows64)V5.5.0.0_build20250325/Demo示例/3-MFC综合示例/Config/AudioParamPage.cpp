...
#define AUDIOTYPE_INDEX_CLOSE   0   // Audio output method Off Index
#define AUDIOTYPE_INDEX_USESELF 1   // Audio output method Speaker Index
#define AUDIOTYPE_INDEX_LINE    2   // Audio output method Pinout Index
#define AUDIOTYPE_INDEX_MAX     3 
#define AUDIO_SAMPLE_NO			100	//Maximum recording number
...
void CLS_AudioParamPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_AUDIOCTRL, m_cboAudioCtrlType);
	DDX_Control(pDX, IDC_COMBO_AUDIONUM, m_cboLaserVioce);
}
...
BOOL CLS_AudioParamPage::OnInitDialog()
{
	...
	m_cboAudioCtrlType.ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STATIC_VOLUMNTYPECTRL)->ShowWindow(SW_HIDE);
...
void CLS_AudioParamPage::UI_UpdateAudioNum()
{
...
void CLS_AudioParamPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_STATIC_VOLUMNTYPECTRL, IDS_VOLUMN_CTRLTYPE);
	...
	GetDlgItem(IDC_EDT_PLAY_AUDIO_NO)->ShowWindow(SW_HIDE);
...
void CLS_AudioParamPage::OnBnClickedButtonAudioEncoder()
{
	...
	if (0 == iRet)
	{
		...
		//Modify the audio encoding method without restarting
		//int iDeviceType = 0;
...
void CLS_AudioParamPage::OnBnClickedBtnPlayAudioPlay()
{
	...
	tPlayAudioSample.iType = 0;						//audio type, reserved
	tPlayAudioSample.iSampleNo = m_cboLaserVioce.GetCurSel();
...
void CLS_AudioParamPage::OnEnChangeEdtPlayAudioNo()
{
	...
	//Limit audio number input to 0-100
...
//Enable or disable real-time volume value upload
