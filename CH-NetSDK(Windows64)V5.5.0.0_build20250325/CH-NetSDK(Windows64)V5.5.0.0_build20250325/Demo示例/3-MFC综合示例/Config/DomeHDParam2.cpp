...
const int CONST_iSemiautomaticWhiteBalanceIndex = 1;//COMBOX value index for semi-automatic white balance
const int CONST_iManualWhiteBalanceIndex = 19;//COMBOX value index for manual white balance
...
void CLS_DomeHDParam2::UI_UpdateDialogText()
{
	...
	//Minimum focal length distance 10cm, 50cm, 1.0m, 1.5m, 3.0m, 6.0m, infinity (100m)
	...
	//white balance
	//Automatically represents different scene modes:
	//0-Default Auto 1-Sunny Mode 2-Fluorescent Mode 3-Warm Light Mode 4-Incandescent Light Mode 5-Natural Light Mode 6-Lock White Balance
	//7-ATW 8-indoor 9-outdoor 10-outdoor automatic 11-sodium lamp automatic 12-sodium lamp 13-sodium lamp outdoor automatic 14-sodium vapor and other modes
	//15- Mercury Vapor Lamp Mode 16- Cloudy 17- Dusk
	//Manual represents different modes: 0-semi-automatic 1-pure manual
	...
	else
	{
		...
		if(bGetTemplatePara || _iType == HD_PARA_BACK_LIGHT)	
		{
			...
			if (iBackLight == 1)
			{
				...
				{
					iBackLightRange--;//LNM3020: When backlight compensation is on, 1-low, 2-medium, 3-high
				}
		...
		//10-Backlight compensation area
		...
		if(bGetTemplatePara || _iType == HD_PARA_WIDE_DYNAMIC)	
		{
			...
			if ((iWideDynamic == WIDE_DY_OPEN) && (iAutoEnable == WIDE_DY_AUTO))					//automatic
			{
			...
			else if ((iWideDynamic == WIDE_DY_OPEN) && (iAutoEnable == WIDE_DY_HANDLE))				//manually
			{
			...
			else if (iWideDynamic == WIDE_DY_VISUAL_ENHANCE)										// visual enhancement
			{
		...
		if(bGetTemplatePara || _iType == HD_PARA_WHITE_BALANCE)	
		{
			...
			else if (pb.iAutoEnable[HD_PARA_WHITE_BALANCE] == 1)
			{
				...
				{
					int iRealItem = iTmpItem + 1;//index +1 except 0
...
{
	// TODO: Sensitivity---only auto has
...
{
	// TODO: Infrared correction Param1 value 0 off 1 on
...
{
	// TODO: Backlight Compensation Level LNM3020: When Backlight Compensation is on, 1-Low, 2-Medium, 3-High
...
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: White balance R adjustment
...
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: White balance B adjustment
...
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: White balance G adjustment
