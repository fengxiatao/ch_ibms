...
private:
	...
	CButton m_chkChannelNo[LEN_64]; //Created according to the maximum number, customers need to suggest dynamic creation @cb150429
	...
	int m_iCurStrategy;			//0: Disk Group Policy, 1: Quota Policy
	DISK_QUOTA_USED m_strDiskQuota[MAX_CHANNEL_NUM];
	int m_iModeType;
	int m_iChannelEnable[LEN_16];	//Linkage channel
	int m_iPageNo;
	int m_iMaxLinkNum;
public:
