...
BEGIN_MESSAGE_MAP(CLS_LANWorkModePage, CLS_BasePage)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_WORKMODE_LANNO, &CLS_LANWorkModePage::OnCbnSelchangeComboWorkmodeLanno)
END_MESSAGE_MAP()
...
BOOL CLS_LANWorkModePage::UI_UpdateMtu()
{
	...
	int iLanNo = m_cboLanNo.GetCurSel()+1;
	int iEnableValue = 0;
	int iRet = NetClient_GetCommonEnable(m_iLogonID, CI_NET_CONNECT_INFO, iLanNo, &iEnableValue);
	if (0 == iRet)
	{
...
void CLS_LANWorkModePage::OnCbnSelchangeComboWorkmodeLanno()
{
	UI_UpdateMtu();
}
