...
void CLS_VideoInterested::OnBnClickedButtonVideoInterestedSet()
{
	...
	if(8 == strInterestedArea.iIndex) //ninth block special treatment
	{
		iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_INTERESTED_AREA_EX, m_iChannelNo, &strInterestedArea, sizeof(TInterestedArea));
	}
	else
	{
		iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_INTERESTED_AREA, m_iChannelNo, &strInterestedArea, sizeof(TInterestedArea));
	}
	if (iRet < 0)
	{
...
void CLS_VideoInterested::UI_UpdateDialog()
{
	...
	m_cboAreaIndex.SetItemData(m_cboAreaIndex.AddString(_T("Area9")), 8);
