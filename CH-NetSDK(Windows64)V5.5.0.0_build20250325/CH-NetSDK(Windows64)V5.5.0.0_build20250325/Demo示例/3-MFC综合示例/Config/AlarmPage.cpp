...
void CLS_AlarmPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SLIDER_VIDEO_OCC_SENSITIVITY, m_sldVideoOcclusionThreshold);
}
...
BEGIN_MESSAGE_MAP(CLS_AlarmPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BUTTON_VIDEO_OCC_SENSITIVITY, &CLS_AlarmPage::OnBnClickedButtonVideoOccSensitivity)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_VIDEO_OCC_SENSITIVITY, &CLS_AlarmPage::OnNMCustomdrawSliderVideoOccSensitivity)
END_MESSAGE_MAP()
...
BOOL CLS_AlarmPage::OnInitDialog()
{
	...
	m_sldVideoOcclusionThreshold.SetRange(0, 255, TRUE);
	m_sldVideoOcclusionThreshold.SetPos(0);
	m_sldVideoOcclusionThreshold.SetTicFreq(5);
...
void CLS_AlarmPage::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_STATIC_VIDEO_OCC_SENSITIVITY,IDS_VIDEO_OCCLUSION_SENSITIVITY);
	SetDlgItemTextEx(IDC_BUTTON_MOTIONDETECTTHRESHOLD,IDS_SET);
	SetDlgItemTextEx(IDC_BUTTON_VIDEO_OCC_SENSITIVITY,IDS_SET);
...
BOOL CLS_AlarmPage::UI_UpdateThreshold()
{
	...
	int iOcclusionThreshold;
	iRet = 	NetClient_GetAlmVdoCovThreshold(m_iLogonID,m_iChannelNo,&iOcclusionThreshold);
	if (0 == iRet)
	{
		m_sldVideoOcclusionThreshold.SetPos(iOcclusionThreshold);
		SetDlgItemInt(IDC_STATIC_SENSITIVITY_PROCESS, iOcclusionThreshold);
		AddLog(LOG_TYPE_SUCC,"","NetClient_GetAlmVdoCovThreshold (%d,%d,%d)",m_iLogonID,m_iChannelNo,iOcclusionThreshold);
	} 
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetAlmVdoCovThreshold (%d,%d,%d)",m_iLogonID,m_iChannelNo,iOcclusionThreshold);
	}
	int iCPUThreshold = 0;
	int iMEMThreshold = 0;
...
//Set up preview capture and delayed capture
...
void CLS_AlarmPage::OnBnClickedButtonVideoOccSensitivity()
{
	// TODO: Set Video Occlusion Sensitivity
	if (m_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
		return;
	...
	int iThreshold = m_sldVideoOcclusionThreshold.GetPos();
	int iRet = NetClient_SetAlmVdoCovThreshold(m_iLogonID, m_iChannelNo, iThreshold);
	if (iRet == 0)
	{
		AddLog(LOG_TYPE_SUCC,"", "NetClient_SetAlmVdoCovThreshold(%d,%d,%d)", m_iLogonID,m_iChannelNo,iThreshold);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"", "NetClient_SetAlmVdoCovThreshold(%d,%d,%d)", m_iLogonID,m_iChannelNo,iThreshold);
	}
}

void CLS_AlarmPage::OnNMCustomdrawSliderVideoOccSensitivity(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	if (m_iLogonID < 0)
	{
	...
	int iThreshold = m_sldVideoOcclusionThreshold.GetPos();
	SetDlgItemInt(IDC_STATIC_SENSITIVITY_PROCESS, iThreshold);
	*pResult = 0;
}