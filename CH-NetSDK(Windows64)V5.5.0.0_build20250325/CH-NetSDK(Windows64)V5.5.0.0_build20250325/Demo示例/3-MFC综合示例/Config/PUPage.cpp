...
BEGIN_MESSAGE_MAP(CLS_PUPage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BTN_VERIFICATION_CODE_SET, &CLS_PUPage::OnBnClickedBtnVerificationCodeSet)
END_MESSAGE_MAP()
...
void CLS_PUPage::OnParamChangeNotify(int _iLogonID, int _iChannelNo, int _iParaType,void* _pPara,int _iUserData)
{
	if (_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG,"","[CLS_PUPage::OnParamChangeNotify]Invalid logon id(%d)", _iLogonID);
		return;
	}
	switch(_iParaType)
	{
	case  PARA_VERIFICATION_CODE:
		{
			AddLog(LOG_TYPE_SUCC, "", "[CLS_PUPage::ParamChangeNotify][PLATE_LIST_TYPE] Set enable success!");
			UI_UpdateVerificationCode();
		}
		break;
	default:
...
BOOL CLS_PUPage::UI_UpdateBarCode()
{
    ...
    BarCode stBarCode = {0};
	int iRetSize = 0;
	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_BARCODE,0, &stBarCode, sizeof(stBarCode), &iRetSize);
	if (0 == iRet)
	{
        SetDlgItemText(IDC_EDT_BARCODE, stBarCode.cBarCode);
        SetDlgItemText(IDC_EDT_SN, stBarCode.cAliSn);
		AddLog(LOG_TYPE_SUCC,"","NetClient_GetDevConfig(%d,%d,%s,%d,%d)",m_iLogonID,NET_CLIENT_BARCODE,stBarCode.cBarCode,sizeof(char),iRetSize);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetDevConfig(%d,%d,%s,%d,%d)",m_iLogonID,NET_CLIENT_BARCODE,stBarCode.cBarCode,sizeof(char),iRetSize);
	}
	return TRUE;
}

BOOL CLS_PUPage::UI_UpdateVerificationCode()
{
	if (m_iLogonID < 0) {
		return FALSE;
	}

	VerificationCode tInfo = {0};
	int iRetSize = 0;
	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_VERIFICATION_CODE,0, &tInfo, sizeof(VerificationCode), &iRetSize);
	if (0 == iRet)
	{
		SetDlgItemText(IDC_EDT_VERIFICATION_CODE_GET, tInfo.cVerificionCode);
		AddLog(LOG_TYPE_SUCC,"","NetClient_GetDevConfig(%d,%d,%s,%d,%d)",m_iLogonID,NET_CLIENT_BARCODE,tInfo.cVerificionCode,sizeof(char),iRetSize);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetDevConfig(%d,%d,%s,%d,%d)",m_iLogonID,NET_CLIENT_BARCODE,tInfo.cVerificionCode,sizeof(char),iRetSize);
	}
	return TRUE;
}
...
void CLS_PUPage::OnBnClickedBtnBarcodeSet()
{
	...
	strcpy(stBarCode.cBarCode, cstr);
    GetDlgItemText(IDC_EDT_SN, cstr);
    strcpy(stBarCode.cAliSn, cstr);
	//cBarCode = cstr;
	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_BARCODE,0, &stBarCode, sizeof(stBarCode));
	if (0 == iRet)
	{
...
void CLS_PUPage::OnBnClickedBtnVerificationCodeSet()
{
	if (m_iLogonID < 0)
		return ;
	...
	VerificationCode tInfo = {0};
	CString cstr = "";
	GetDlgItemText(IDC_EDT_VERIFICATION_CODE, cstr);
	strcpy(tInfo.cVerificionCode, cstr);
	//cBarCode = cstr;
	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_VERIFICATION_CODE,0, &tInfo, sizeof(tInfo));
	if (0 == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","NetClient_SetDevConfig(%d,%d,%s,%d)",m_iLogonID,NET_CLIENT_VERIFICATION_CODE,cstr,sizeof(char));
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_SetDevConfig(%d,%d,%s,%d)",m_iLogonID,NET_CLIENT_VERIFICATION_CODE,cstr,sizeof(char));
	}
}
