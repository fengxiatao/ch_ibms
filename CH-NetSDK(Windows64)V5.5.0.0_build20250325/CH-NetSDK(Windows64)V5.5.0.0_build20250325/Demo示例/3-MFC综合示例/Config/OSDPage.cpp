...
void CLS_OSDPage::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMBO_IPC_PORT_ID, m_cboIPCPortID);
}
...
BEGIN_MESSAGE_MAP(CLS_OSDPage, CLS_BasePage)
    ...
    ON_BN_CLICKED(IDC_BUTTON_PORT_COMMONNAME, &CLS_OSDPage::OnBnClickedButtonPortCommonname)
    ON_CBN_SELCHANGE(IDC_COMBO_IPC_PORT_ID, &CLS_OSDPage::OnCbnSelchangeComboIpcPortId)
END_MESSAGE_MAP()
...
BOOL CLS_OSDPage::OnInitDialog()
{
	...
	m_edtChannelTitle.SetLimitText(LEN_256);
...
void CLS_OSDPage::OnBnClickedButtonChanneltitle()
{
	...
	char cChannelTitle[LENGTH_CHANNEL_TITLE_EX+1]={0};
	GetDlgItemText(IDC_EDIT_CHANNELTITLE, cChannelTitle, LENGTH_CHANNEL_TITLE_EX);
...
void CLS_OSDPage::UI_UpdateDialog()
{
    ...
    SetDlgItemTextEx(IDC_STATIC_ALARM_OUT_PORT, IDS_FUNC_ALARMOUTPPORT_NAME);
    SetDlgItemTextEx(IDC_STATIC_IPC_PORT_ID, IDS_CONFIG_DNVR_ALMLINK_OUTPORT);
    SetDlgItemTextEx(IDC_BUTTON_PORT_COMMONNAME, IDS_SET);
    ...
    const int CONST_INT_MAX_PORT_NUM = 2;
    for (int iPort = 1; iPort <= CONST_INT_MAX_PORT_NUM; iPort++)
    {
        m_cboIPCPortID.SetItemData(m_cboIPCPortID.AddString(IntToStr(iPort).c_str()),iPort);
    }
    m_cboIPCPortID.SetCurSel(0);
...
BOOL CLS_OSDPage::UI_UpdateChannelTitle()
{
...

    {
        ...
        TDevCommonName tDevComName = {0};
        tDevComName.m_iSize = sizeof(TDevCommonName);
        tDevComName.m_iChannelType = DEVCOMNAME_ALARMOUTPORT;

        int iPortID = m_cboIPCPortID.GetItemData(m_cboIPCPortID.GetCurSel());
        int iChannelAndPort = (m_iChannelNo+1)*1000 + iPortID;

        iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_DEV_COMMONNAME, iChannelAndPort, &tDevComName, sizeof(tDevComName), &iBytesReturned);
        SetDlgItemText(IDC_EDIT_DEVCOMNAME_IPC_PORT, (LPCTSTR)(tDevComName.m_cChanneleName));
        if (0 == iRet)
        {
            AddLog(LOG_TYPE_SUCC,"","NetClient_GetDevConfig(%d,%d,%s,%d)",m_iLogonID, m_iChannelNo, tDevComName.m_cChanneleName, tDevComName.m_iChannelType);
        }
...
BOOL CLS_OSDPage::UI_UpdateFontSize()
{
	...
	int iRet = NetClient_GetOSDTypeFontSize(m_iLogonID, iChlNo, iTypeNo, &iSize);
	if (0 == iRet)
	{
		if(iSize <= 5)
		{
...
void CLS_OSDPage::OnBnClickedButtonOsdfontsize()
{
	...
	if (6 == iSize)
	{
		...
		{
			MessageBox("Character width and height exceeded", "Prompt", MB_OK);
			return;
		}
...
#define MAX_TEXT_LEN 512
	...
	for(int i = 0;i < m_edtDrawText.GetLineCount(); ++i)
	{
		...
		cTemp[len] = '\0';//To manually add the terminator
...
void CLS_OSDPage::OnBnClickedButtonPortCommonname()
{
    // TODO: Add your control notification handler code here
    char cChanTitEx[65];
    GetDlgItemText(IDC_EDIT_DEVCOMNAME_IPC_PORT, cChanTitEx, 64);
    TDevCommonName tDevComName = {0};
    tDevComName.m_iSize = sizeof(TDevCommonName);
    ...
    tDevComName.m_iChannelType = DEVCOMNAME_ALARMOUTPORT;
    memcpy_s(tDevComName.m_cChanneleName, sizeof(tDevComName.m_cChanneleName), cChanTitEx, 64);
    int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_DEV_COMMONNAME, iChannelAndPort, &tDevComName, sizeof(tDevComName));
    if (0 == iRet)
    {
        AddLog(LOG_TYPE_SUCC,"","NetClient_SetDevConfig(%d,%d,%s,%d)",m_iLogonID, m_iChannelNo, tDevComName.m_cChanneleName, tDevComName.m_iChannelType);
    }
...
void CLS_OSDPage::OnCbnSelchangeComboIpcPortId()
{
    // TODO: Add your control notification handler code here
    ...
    int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_DEV_COMMONNAME, iChannelAndPort, &tDevComName, sizeof(tDevComName), &iBytesReturned);
    SetDlgItemText(IDC_EDIT_DEVCOMNAME_IPC_PORT, (LPCTSTR)(tDevComName.m_cChanneleName));
    if (0 == iRet)
    {
