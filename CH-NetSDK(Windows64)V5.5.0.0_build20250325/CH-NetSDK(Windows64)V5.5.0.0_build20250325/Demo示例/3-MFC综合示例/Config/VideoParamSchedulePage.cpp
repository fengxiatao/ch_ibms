...
void CLS_VideoParamSchedulePage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_CROPENABLE, m_chkEnable);
	DDX_Control(pDX, IDC_EDIT1, m_edtX);
	DDX_Control(pDX, IDC_EDIT2, m_edtY);
}
...
BEGIN_MESSAGE_MAP(CLS_VideoParamSchedulePage, CLS_BasePage)
	...
	ON_BN_CLICKED(IDC_BUTTON_CROPSET, &CLS_VideoParamSchedulePage::OnBnClickedButtonCropset)
END_MESSAGE_MAP()
...
void CLS_VideoParamSchedulePage::UI_UpdateDialog()
{
	...
	SetDlgItemText(IDC_STATIC_CROPCODING, GetTextByLan(_T("副码流裁剪"), _T("Crop Coding")));
	SetDlgItemText(IDC_CHECK_CROPENABLE, GetTextByLan(_T("使能"), _T("Enable")));
	SetDlgItemText(IDC_STATIC_CONTENT, GetTextByLan(_T("X,Y:万分比坐标,画布大小默认为4:3"), _T("X. Y: 10000 coordinate, canvas size is 4:3 by default")));
	SetDlgItemText(IDC_STATIC_INFO, GetTextByLan(_T("提示:"), _T("Info:")));
	SetDlgItemText(IDC_BUTTON_CROPSET, GetTextByLan(_T("设置"), _T("Set")));
}
...
void CLS_VideoParamSchedulePage::UpdateSchedule()
{
	...
	int iReturnBytes = -1;
	iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_CROPCODING, m_iChannelNo, &tInfo, tInfo.iSize, &iReturnBytes);
	if (iRet == RET_SUCCESS)
	{
		m_chkEnable.SetCheck(tInfo.iEnable);
		CString strLeft = "";
		strLeft.Format("%d", tInfo.tPoint[0].iX);
		m_edtX.SetWindowText(strLeft);
		strLeft.Format("%d", tInfo.tPoint[0].iY);
		m_edtY.SetWindowText(strLeft);
		AddLog(LOG_TYPE_SUCC,"","NetClient_GetDevConfig(NET_CLIENT_CROPCODING) success %d",m_iLogonID);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","NetClient_GetDevConfig(NET_CLIENT_CROPCODING) failed %d",iRet);
	}
}
...
void CLS_VideoParamSchedulePage::OnBnClickedButtonCropset()
{
	int iRet = RET_FAILED;
	CropCoding tInfo = {0};
	tInfo.iSize = (int)sizeof(CropCoding);
	tInfo.iEnable = m_chkEnable.GetCheck();
	CString strLeft;
	m_edtX.GetWindowText(strLeft);
	tInfo.tPoint[0].iX = _ttoi(strLeft);
	m_edtY.GetWindowText(strLeft);
	tInfo.tPoint[0].iY = _ttoi(strLeft);
	//Width 1719 Height 1875
	tInfo.tPoint[1].iX = tInfo.tPoint[0].iX + 1719;
	tInfo.tPoint[1].iY = tInfo.tPoint[0].iY + 1875;
	if (tInfo.tPoint[1].iX > 10000 || tInfo.tPoint[1].iY > 10000)
	{
		MessageBox(GetTextByLan(_T("坐标不合法"),_T("Location is not valid")));
		return;
	}
	iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_CROPCODING, m_iChannelNo,&tInfo, tInfo.iSize);
	if (RET_SUCCESS == iRet)
	{
		AddLog(LOG_TYPE_SUCC,"","NetClient_SetDevConfig(NET_CLIENT_CROPCODING) success %d",m_iLogonID);
	}
	else
		AddLog(LOG_TYPE_SUCC,"","NetClient_SetDevConfig(NET_CLIENT_CROPCODING) failed %d",iRet);
}
