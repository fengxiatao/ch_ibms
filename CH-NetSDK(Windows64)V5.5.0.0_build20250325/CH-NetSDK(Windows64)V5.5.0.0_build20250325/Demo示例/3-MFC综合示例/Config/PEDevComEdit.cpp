...
void CLS_PEDevComEdit::UI_UpdateDialogText()
{
	...
	SetDlgItemText(IDC_STATIC_ALARMUP_TEMP,GetTextEx(IDS_PE_ALARMUP_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_ALARMDOWN_TEMP,GetTextEx(IDS_PE_ALARMDOWN_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_CLEARUP_TEMP,GetTextEx(IDS_PE_CLEARUP_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_CLEARDOWN_TEMP,GetTextEx(IDS_PE_CLEARDOWN_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_HUM_VALUE,GetTextEx(IDS_PE_HUM_LIMIT));
	SetDlgItemText(IDC_STATIC_ALARMUP_HUM,GetTextEx(IDS_PE_ALARMUP_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_ALARMDOWN_HUM,GetTextEx(IDS_PE_ALARMDOWN_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_CLEARUP_HUM,GetTextEx(IDS_PE_CLEARUP_TEMP)+"(degree)");
	SetDlgItemText(IDC_STATIC_CLEARDOWN_HUM,GetTextEx(IDS_PE_CLEARDOWN_TEMP)+"(degree)");
...
void CLS_PEDevComEdit::InitComInfo()
{
	...
	{
		//the first half of the type name
		string strFirstName = "";
		if (m_clsPowerEnPage != NULL)
		{
...
// 		m_cboDevType.SetCurSel(iIndex);
		...
		// Parse the address, alarm and eliminate the upper and lower limits, etc.
		//device address
		CString cstrTemp = "";
		...
		//The lower limit of the temperature alarm

		fValue = (float)(atof(cstrTemp) - 10000) / 100;
		cstrTemp.Format("%0.2f",fValue);
		m_edtTemDisAlarmDownLevel.SetWindowText(cstrTemp);

		// Humidity alarm upper limit
		...
		//Lower limit of humidity elimination alarm
		fValue = (float)(atof(cstrTemp) - 10000) / 100;
		cstrTemp.Format("%0.2f",fValue);
