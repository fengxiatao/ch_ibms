...
BEGIN_MESSAGE_MAP(CStoragePage, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_START_K200, &CStoragePage::OnBnClickedButtonStartK200)
	ON_BN_CLICKED(IDC_BUTTON_STOP_K200, &CStoragePage::OnBnClickedButtonStopK200)
END_MESSAGE_MAP()
...
void CStoragePage::UI_UpdateRecordMode(int* _iCurSel, int _iSize)
{
	for (int i = 0; i < MAX_RECORDMODE_CONTROL; i++)
	{
		...
		InsertString(*(m_pCboRecordMode[i]), 3, IDS_CONFIG_DNVR_MOTIONDETECT);
		InsertString(*(m_pCboRecordMode[i]), 4, IDS_CONFIG_STORAGE_LOSTREC);
		InsertString(*(m_pCboRecordMode[i]), 5, IDS_CONFIG_STORAGE_COVERREC);
		InsertString(*(m_pCboRecordMode[i]), 6, IDS_CONFIG_STORAGE_VCA_ALARM);
		InsertString(*(m_pCboRecordMode[i]), 7, IDS_CONFIG_STORAGE_ALL_ALARM);
		InsertString(*(m_pCboRecordMode[i]), 8, IDS_CONFIG_STORAGE_MPORT);
...
void CStoragePage::UI_UpdateStorageRule()
{
	...
	tStorageRule.iType = 3;//Parameter value iType == 3; 0, disable redundant recording; 1, enable redundant recording;
...
int CStoragePage::UpdateRecordStatusParam()
{
	...
	if(RET_SUCCESS == NetClient_NetFileGetRecordState(m_iLogonID, m_iChannelNo, &iRecordState))
	{
		...
		else                                    //  If the user defines the video, it will be displayed in the edit box below
		{
...
BOOL CStoragePage::UI_UpdateSurface()
{
	...
	SetDlgItemTextEx(IDC_STATIC_K200_ALARM, IDS_K200_ALARM);
	SetDlgItemTextEx(IDC_STATIC_ALARM_TYPE_K200, IDS_CONFIG_LINK_ALARM_TYPE);
	SetDlgItemTextEx(IDC_BUTTON_START_K200, IDS_START_ALARM_VIDEO);
	SetDlgItemTextEx(IDC_BUTTON_STOP_K200, IDS_STOP_ALARM_VIDEO);
...
void CStoragePage::OnCbnSelchangeComboWeekday()
{
	...
	if(iRet == 0)
	{
		...
		for (int i = 0; i < MAX_RECORDMODE_CONTROL; ++i)
		{
			...
			else if(schedule[i]->iRecordMode == 4)
			{
				m_pCboRecordMode[i]->SetCurSel(3);
			}
			else if(schedule[i]->iRecordMode == 8)
			{
				m_pCboRecordMode[i]->SetCurSel(4);
			}
			else if(schedule[i]->iRecordMode == 16)
			{
				m_pCboRecordMode[i]->SetCurSel(5);
			}
			else if(schedule[i]->iRecordMode == 32)
			{
				m_pCboRecordMode[i]->SetCurSel(6);
			}
			else if(schedule[i]->iRecordMode == 64)
			{
				m_pCboRecordMode[i]->SetCurSel(7);
			}
			else if(schedule[i]->iRecordMode == 6)
			{
				m_pCboRecordMode[i]->SetCurSel(8);
			}
		}
...
void CStoragePage::OnBnClickedButtonPrerecSet()
{
	...
	iDelayEnable = 1;    //The delay is always in the enable state
...
void CStoragePage::OnBnClickedCheckredundantrecEnable()
{
	...
	STORAGE_RULE tStorageRule = {sizeof(STORAGE_RULE)};
	tStorageRule.iType = STORAGE_RULE_REDUNDANCE_RECORD;//Parameter value iType == 3; 0, disable redundant recording; 1, enable redundant recording;
...
{
	STORAGE_RULE tStorageRule = {sizeof(STORAGE_RULE)};
	tStorageRule.iType = STORAGE_RULE_SUB_RECORD;//Parameter value iType == 4; 0, do not enable sub-stream recording; 1, enable sub-stream recording;
...
void CStoragePage::OnBnClickedButtonStartK200()
{
	K200StartStopAlarm tInfo = {0};
	int iResult = 0;
	tInfo.iAlarmType =GetDlgItemInt(IDC_EDIT_ALARM_TYPE_K200, &iResult);
	if (0 == iResult)
	{
		AddLog(LOG_TYPE_FAIL, "", "GetDlgItemInt(%d)", m_iLogonID);
		return;
	}
	tInfo.iCMD = ALARM_VIDEO_START;
	K200AlarmPara tResult = {0};
	int iRetValue = NetClient_CmdConfig(m_iLogonID, CMD_K200_START_STOP_ALARM, m_iChannelNo, &tInfo, sizeof(tInfo), &tResult, sizeof(tResult));
	if(RET_SUCCESS == iRetValue)
	{
		AddLog(LOG_TYPE_SUCC, "","CLS_AirLinesPlan::NetClient_CmdConfig[CMD_K200_START_STOP_ALARM] (%d, %d),ret = %d", m_iLogonID, m_iChannelNo, tResult.iRes);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_AirLinesPlan::NetClient_CmdConfig[CMD_K200_START_STOP_ALARM] (%d, %d), ret = %d", m_iLogonID, m_iChannelNo, tResult.iRes);
	}
}

void CStoragePage::OnBnClickedButtonStopK200()
{
	K200StartStopAlarm tInfo = {0};
	int iResult = 0;
	tInfo.iAlarmType =GetDlgItemInt(IDC_EDIT_ALARM_TYPE_K200, &iResult);
