...
void CLS_ItsRecognitionParam::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_EBIKE_CITY, m_cboEbikeCity);
	DDX_Control(pDX, IDC_CHECK__EBIKE_ENABLE, m_chkEbikeEnable);
}
...
BEGIN_MESSAGE_MAP(CLS_ItsRecognitionParam, CLS_BasePage)
	...
	ON_EN_CHANGE(IDC_EDIT_FIRSTCHINESE, &CLS_ItsRecognitionParam::OnEnChangeEditFirstCh)
	ON_BN_CLICKED(IDC_BUTTON_EBIKE_SET, &CLS_ItsRecognitionParam::OnBnClickedButtonEbikeSet)
	ON_CBN_SELCHANGE(IDC_COMBO_EBIKE_CITY, &CLS_ItsRecognitionParam::OnCbnSelchangeComboEbikeCity)
END_MESSAGE_MAP()
...
void CLS_ItsRecognitionParam::UI_UpdateDialog()
{
	...
	SetDlgItemTextEx(IDC_BUTTON_EBIKE_SET,IDS_SET);
	InsertString(m_cboLicenseType, 0, GetTextEx(IDS_NEEDCHECK));
	InsertString(m_cboLicenseType, 1, GetTextEx(IDS_INVALIDLICENSETYPE));	

	m_cboEbikeCity.ResetContent();
	m_cboEbikeCity.SetItemData(m_cboEbikeCity.AddString(GetTextByLan("Ìì", "Tian")),PROVINCE_TJ);
	m_cboEbikeCity.SetItemData(m_cboEbikeCity.AddString(GetTextByLan("Óñ", "Yu")),CITY_GUANGXI_YULIN);
	m_cboEbikeCity.SetCurSel(0);
}
...
{
	CString csProvinceName = "Beijing, Tianjin, Hebei, Hunan, Shanghai, Hubei, Chongqing, Guangdong, Sichuan, Shanxi, Liaoning, Yunnan, Gansu, Heiqing, Zhejiang, Shaanxi, Suzhou, Taiwan, Anhui, Tibet, Fujian, Xingui, Henan, Hong Kong, Macau, Jiangxi, Mongolia, Luqiong";
	if (m_iLogonID < 0)
	{
		AddLog(LOG_TYPE_MSG,"","Invalid logon id(%d)",m_iLogonID);
		return;
...
void CLS_ItsRecognitionParam::OnEnChangeEditFirstCh()
{
	// TODO: If the control is a RICHEDIT control, it will not
	// Send this notification unless overriding CLS_BasePage::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask(),
	// Also OR the ENM_CHANGE flags into the mask.

	// TODO:  Add control notification handler code here
}

void CLS_ItsRecognitionParam::UI_UpdateEBike()
{
	ItsEbikePlate tInfo = {0};
	tInfo.iSize = sizeof(tInfo);
	tInfo.iChanNo = m_iChannel;
	tInfo.iCodeNo = m_cboEbikeCity.GetItemData(m_cboEbikeCity.GetCurSel());

	int iByteReturn = -1;
	int iRet = NetClient_GetDevConfig(m_iLogonID, NET_CLIENT_EBIKE_PLATE, m_iChannel, &tInfo, sizeof(tInfo),&iByteReturn);
	if(RET_SUCCESS == iRet)
	{
		 m_chkEbikeEnable.SetCheck(tInfo.iEnable);
	}
	else
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ItsRecognitionParam::NetClient_GetDevConfig[NET_CLIENT_EBIKE_PLATE] (%d, %d), error(%d)", m_iLogonID, m_iChannel, GetLastError());
	}
}

void CLS_ItsRecognitionParam::OnBnClickedButtonEbikeSet()
{
	ItsEbikePlate tInfo = {0};
	tInfo.iSize = sizeof(tInfo);
	tInfo.iChanNo = m_iChannel;
	tInfo.iCodeNo = m_cboEbikeCity.GetItemData(m_cboEbikeCity.GetCurSel());
	tInfo.iEnable = m_chkEbikeEnable.GetCheck();

	int iRet = NetClient_SetDevConfig(m_iLogonID, NET_CLIENT_EBIKE_PLATE, m_iChannel, &tInfo, sizeof(tInfo));
	if(RET_SUCCESS != iRet)
	{
		AddLog(LOG_TYPE_FAIL,"","CLS_ItsRecognitionParam::NetClient_SetDevConfig[NET_CLIENT_EBIKE_PLATE] (%d, %d), error(%d)", m_iLogonID, m_iChannel, GetLastError());
	}
}

void CLS_ItsRecognitionParam::OnCbnSelchangeComboEbikeCity()
{
	UI_UpdateEBike();
}
