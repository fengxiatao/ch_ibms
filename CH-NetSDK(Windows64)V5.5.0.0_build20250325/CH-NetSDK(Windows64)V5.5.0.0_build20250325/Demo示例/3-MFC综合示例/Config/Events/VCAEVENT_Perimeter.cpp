...
void CLS_VCAEVENT_Perimeter::UpdatePageUI()
{
	...
	iCmd = VCA_CMD_PERIMETER;
	VCAPerimeter tParam = {0};
	tParam.stRule.iRuleID = m_iRuleID;
	tParam.stRule.iSceneID = m_iSceneID;
	iRet = NetClient_VCAGetConfig(m_iLogonID, iCmd, m_iChannelNO, &tParam, sizeof(VCAPerimeter));
	if (iRet >= 0)
	{
		...
		//memcpy(&tr.stDisplayParam, &tParam.stDisplayParam, sizeof(tr.stDisplayParam));
		//memcpy(&pm.stRegion, &tParam.stRegion, sizeof(pm.stRegion));
		if (!tParam.stRule.iValid)
			return;

		m_chkDisplayRule.SetCheck(tParam.stDisplayParam.iDisplayRule);
		m_chkDisplayState.SetCheck(tParam.stDisplayParam.iDisplayStat);

		m_cboTargetCheck.SetCurSel(tParam.iTargetTypeCheck);
		//m_iMinDistance = tw.iMinDistance;
		SetDlgItemInt(IDC_EDIT_MinDis, tParam.iMinDistance);
		SetDlgItemInt(IDC_EDIT_MinTime, tParam.iMinTime);
		m_cboAlarmColor.SetCurSel(tParam.stDisplayParam.iAlarmColor-1);
		m_cboColor.SetCurSel(tParam.stDisplayParam.iColor-1);
		SetDlgItemInt(IDC_EDIT_Dir, tParam.iDirection);

		//m_chkDirEnable.SetCheck(pm.iType);
		m_cboIndentify.SetCurSel(tParam.iMode);
		SetDlgItemInt(IDC_EDIT_PointNum, tParam.stRegion.iPointNum);
		CString szPointBuf;
		for (int i = 0; i < tParam.stRegion.iPointNum; i ++ )
		{
			szPointBuf.AppendFormat("(%d,%d)", tParam.stRegion.stPoints[i].iX, tParam.stRegion.stPoints[i].iY);
		}
...
void CLS_VCAEVENT_Perimeter::OnBnClickedButtonPerimeterset()
{
	...
	tParam.stRule.iRuleID = m_iRuleID;
	tParam.stRule.iSceneID = m_iSceneID;
	int iRet = NetClient_VCAGetConfig(m_iLogonID, VCA_CMD_PERIMETER, m_iChannelNO, &tParam, sizeof(VCAPerimeter));

	if (RET_SUCCESS == iRet)
	{
		tParam.stDisplayParam.iDisplayRule = m_chkDisplayRule.GetCheck();
		tParam.stDisplayParam.iDisplayStat = m_chkDisplayState.GetCheck();
		tParam.stDisplayParam.iColor = m_cboColor.GetCurSel()+1;
		tParam.stDisplayParam.iAlarmColor = m_cboAlarmColor.GetCurSel()+1;

		//pPm->iType = m_chkDoubleDir.GetCheck();
		tParam.iTargetTypeCheck = m_cboTargetCheck.GetCurSel();
		tParam.iMinDistance = GetDlgItemInt(IDC_EDIT_MinDis);
		tParam.iMinTime = GetDlgItemInt(IDC_EDIT_MinTime);
		tParam.iDirection = GetDlgItemInt(IDC_EDIT_Dir);
		tParam.iMode = m_cboIndentify.GetCurSel();

		int iPointNum = GetDlgItemInt(IDC_EDIT_PointNum);
		tParam.stRegion.iPointNum = iPointNum;
		CString strPointStr;
		GetDlgItemText(IDC_EDIT_Points, strPointStr);
		vca_TLine stLineTmp = {0};
		GetPolyFromString(strPointStr, iPointNum, tParam.stRegion);
		iRet = NetClient_VCASetConfig(m_iLogonID, VCA_CMD_PERIMETER, m_iChannelNO, &tParam, sizeof(tParam));
	}
...
void CLS_VCAEVENT_Perimeter::UpdateUIText()
{
	...
	SetDlgItemTextEx(IDC_STATIC_Indentify, IDS_VCA_CHECK_MODE);	//Detection mode
