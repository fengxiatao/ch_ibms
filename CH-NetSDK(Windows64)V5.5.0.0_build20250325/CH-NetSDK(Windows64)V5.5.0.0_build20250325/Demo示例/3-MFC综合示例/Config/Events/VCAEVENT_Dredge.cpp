...
void CLS_VCAEVENT_Dredge::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BTN_DREDGE_SET, m_DredgeSet);
}
...
void CLS_VCAEVENT_Dredge::JudgeVCAStatus()
{
	...
	{
		GetDlgItem(IDC_BTN_DREDGE_SET)->EnableWindow(FALSE);
	}
	else if (0 == tParam.iStatus && 1 == tParam.iResult)//Paused successfully
	{
		GetDlgItem(IDC_BTN_DREDGE_SET)->EnableWindow(TRUE);
	}
}
...
void CLS_VCAEVENT_Dredge::AutoShowParam()
{
	...
	SetDlgItemInt(IDC_EDT_DREDGE_ALARMTIME, 60);
	m_cboAlarmColor.SetCurSel(LIGHT_RED);
	m_cboAreaColor.SetCurSel(LIGHT_GREEN);
}
...
void CLS_VCAEVENT_Dredge::JudgeVCASetState(BOOL _bIsVcaStatus)
{
	m_DredgeSet.EnableWindow(_bIsVcaStatus);
}
