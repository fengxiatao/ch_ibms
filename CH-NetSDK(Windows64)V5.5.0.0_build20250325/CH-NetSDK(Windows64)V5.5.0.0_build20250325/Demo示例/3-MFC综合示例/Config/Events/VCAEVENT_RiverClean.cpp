...
void CLS_VCAEVENT_RiverClean::JudgeVCASetState(BOOL _bIsVcaSet)
{
	GetDlgItem(IDC_BTN_RIVER_SET)->EnableWindow(_bIsVcaSet);
}

void CLS_VCAEVENT_RiverClean::JudgeVCAStatus()
{
	...
	{
		GetDlgItem(IDC_BTN_RIVER_SET)->EnableWindow(FALSE);
	}
	else if (0 == tParam.iStatus && 1 == tParam.iResult)//Paused successfully
	{
		GetDlgItem(IDC_BTN_RIVER_SET)->EnableWindow(TRUE);
	}
}
