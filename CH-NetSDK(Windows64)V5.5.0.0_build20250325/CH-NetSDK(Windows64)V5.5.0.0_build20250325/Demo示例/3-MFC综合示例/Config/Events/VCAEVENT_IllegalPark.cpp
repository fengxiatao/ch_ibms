...
#define LEN_2			2
#define POINT_RANGE		15		//Point range
...
void CLS_VCAEVENT_IllegalPark::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHK_ILLPARK_WARNING_ENABLE, m_chkParkWarningEnable);
	DDX_Control(pDX, IDC_EDT_ILLPARK_WARNING_TIME, m_edtParkingWarningTime);
	DDX_Control(pDX, IDC_CBO_ILLPARK_DETECTION_TYPE, m_cboDetectionType);
	DDX_Control(pDX, IDC_EDT_ILLPARK_STALL_JUDGE_TIME, m_edtStallJudgeTime);
}
...
BOOL CLS_VCAEVENT_IllegalPark::OnInitDialog()
{
	...
	m_edtParkingWarningTime.SetLimitText(LEN_32 - 1);
	m_edtStallJudgeTime.SetLimitText(LEN_32 - 1);
...
void CLS_VCAEVENT_IllegalPark::UpdateUIText()
{
	...
	SetDlgItemTextEx(IDC_CHK_ILLPARK_WARNING_ENABLE, IDS_VCAEVENT_PARK_WARNING_ENABLE);
	SetDlgItemTextEx(IDC_STC_ILLPARK_WARNING_TIME, IDS_VCAEVENT_PARK_WARNING_TIME);
	SetDlgItemTextEx(IDC_STC_ILLPARK_DETECTION_TYPE, IDS_VCAEVENT_DETECTION_TYPE);
	SetDlgItemTextEx(IDC_STC_ILLPARK_STALL_JUDGE_TIME, IDS_VCAEVENT_STALL_JUDGE_TIME);
	...
	m_cboDetectionType.ResetContent();
	const CString cstDetectionType[] = {"1","2","3"};
	for (int i = 0; i < sizeof(cstDetectionType)/sizeof(CString); i ++)
	{
		m_cboDetectionType.InsertString(i, cstDetectionType[i]);
	}
	m_cboDetectionType.SetCurSel(0);
}
...
void CLS_VCAEVENT_IllegalPark::UpdatePageUI()
{
	...
	else
	{
		...
		m_chkParkWarningEnable.SetCheck(iip.iParkWarningEnable);
		SetDlgItemInt(IDC_EDT_ILLPARK_WARNING_TIME, iip.iParkWarningTime);
		m_cboDetectionType.SetCurSel(iip.iDetectionType);
		SetDlgItemInt(IDC_EDT_ILLPARK_STALL_JUDGE_TIME, iip.iStallJudgeTime);
...
void CLS_VCAEVENT_IllegalPark::OnBnClickedBtnIllparkSet()
{
	...
	iip.iParkWarningEnable = m_chkParkWarningEnable.GetCheck();
	iip.iParkWarningTime = GetDlgItemInt(IDC_EDT_ILLPARK_WARNING_TIME);
	iip.iDetectionType = m_cboDetectionType.GetCurSel();
	iip.iStallJudgeTime = GetDlgItemInt(IDC_EDT_ILLPARK_STALL_JUDGE_TIME);
