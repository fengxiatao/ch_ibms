...
private:
    typedef struct _StItem {
       int iChannelAllForGet; // 0:all channel 1:special
       int iChannelAllForSet;
       CString strNameZh;
       CString strNameEn;
       int iModuleIndex;
    } StItem;

    typedef std::map<unsigned int, StItem> MapItem;
    ...
    MapItem m_MapItem;
    std::list<StModule> m_moduleList;
    void ReadFuncFromConfig();
public:
	afx_msg void OnBnClickedButtonGetNewCommonenable();
};
