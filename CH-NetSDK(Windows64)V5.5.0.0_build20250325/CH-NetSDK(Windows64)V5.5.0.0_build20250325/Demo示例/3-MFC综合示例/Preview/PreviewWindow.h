...
#define TIMER_PROT_STATS	2
#define MAX_PROTOCOL_LENGTH	2048
#define MIN_PROTOCOL_PRESETING	1
#define MAX_PROTOCOL_PRESETING	9999

typedef void (*VIDED_CHANGED_NOTIFY)(void* _pVideo,void* _pUserData);
extern BOOL m_bConfigWindowShow;
...
private:
	...
	void StatisticalProtocolData(CString _strFileName);
	void WriteOneProtocolToFile(CFile &_fEditFile, char* data, const int len);
...
private:
	...
	int m_iClickNum;//Number of clicks; Multi-window 0 Single-window 1 Full-screen 2
	HWND m_hReverse;
	CRITICAL_SECTION m_csCriticalSession;
public:
	afx_msg void OnBnClickedButtonDevsnap();
private:
	CButton m_chkFaceTarget;
	CButton m_chkFaceAttr;
	CButton m_chkTempDetect;
	CButton m_chkIllegalPark;
	CButton m_chkProtStats;
	CButton m_radioFitWin;
	CButton m_radioFixRatio;
protected:
	CButton m_chkLightNing;
public:
	void ShowDrawParaConfig(int _iDrawType, int _iEnable);
	afx_msg void OnBnClickedCheckShowFaceTarget();
	afx_msg void OnBnClickedCheckShowFaceAttr();
	afx_msg void OnBnClickedCheckShowTempDetect();
	afx_msg void OnBnClickedCheckShowIllegalPark();
	afx_msg void OnBnClickedCheckShowLightning();
	afx_msg void OnBnClickedCheckShowProtStats();
	afx_msg void OnBnClickedRadioAuto();
	afx_msg void OnBnClickedRadioFixratio();
};
