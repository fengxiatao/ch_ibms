...
### 1.1 全局硬规则：零 Mock（2026-05-08 用户明确指示）

**所有页面、所有大屏、所有 API 必须走真实数据**。具体：

1. **前端禁 mock**：禁用 `builtinData` / `Math.random()` 假数据 / 硬编码假数组
2. **禁 fallback mock**：API 失败时显示空态 / 错误态 / 错误提示，**不允许悄悄塞假数据**让屏看起来"好看"
3. **后端 stub 必须填实**：所有大屏聚合端点返回真实 DB 数据，无数据时返回空集合 / 0
4. **测试数据补充用 SQL 种子**：参考 `.tmp_sql/m2d_demo_seed.sql` 样式（合理时段 / 类型混合 / 字段填充），**不准在代码里 mock**

**每个 M 阶段 DoD 必须新增一条**：该阶段涉及的页面 / 端点 0 mock 残留。
